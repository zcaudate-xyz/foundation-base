...
(defspec.xt throttle-entry-promise
  [:fn [:xt/any] :xt/any])

(defspec.xt throttle-entry
...
(defn.xt throttle-entry-promise
  "normalises a throttle entry to its promise"
  {:added "4.1"}
  [entry]
  (return (:? (xt/x:is-array? entry)
              (xt/x:first entry)
              (xt/x:get-key entry "promise"))))

(defn.xt throttle-entry
  "normalises throttle state to the legacy [promise started] shape"
  {:added "4.1"}
  [entry]
  (return (:? (xt/x:is-array? entry)
              entry
