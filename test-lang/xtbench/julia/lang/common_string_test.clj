(ns xtbench.jl.lang.common-string-test
  (:use code.test)
...
(fact "splits a long string"
  ...
  (!.jl (xts/split-long nil 10))
  => (any [] {})

  (!.jl (xts/split-long "" 10))
  => (any [] {})

  (!.jl (xts/split-long (@! +s+) 10))
  => +out+)
...
(fact "gets the string description for a given tag"
  ...
  (!.jl (xts/tag-string "user.account/login"))
  => "account login")
