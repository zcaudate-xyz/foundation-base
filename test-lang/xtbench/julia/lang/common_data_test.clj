(ns xtbench.jl.lang.common-data-test
...
(fact "merges objects at a nesting level"
  ...
  (!.jl (xtd/obj-assign-nested {:a {:b 1}} {:a {:c 2}}))
  => {"a" {"b" 1, "c" 2}}
...
(fact "finds the intersection between map lookups"
  ...
  (!.jl (xtd/obj-intersection {:a true, :b true} {:b true, :c true}))
  => ["b"])

^{:refer xt.lang.common-data/obj-keys-nested :added "4.1"}
(fact "gets nested keys"

  (!.jl (xtd/obj-keys-nested {:a {:b 1, :c 2}} []))
  => (just [[["a" "b"] 1]
            [["a" "c"] 2]]
           :in-any-order))
...
(fact "finds the difference between two map lookups"
  ...
  (!.jl (xtd/obj-difference {:a true, :b true} {:b true, :c true}))
  => ["c"])
...
(fact "performs reduce on an array"
  ...
  (!.jl (xtd/arr-foldl [1 2 3] (fn [acc x] (return (+ acc x))) 0))
  => 6)

^{:refer xt.lang.common-data/arr-foldr :added "4.1"}
(fact "performs right reduce"

  (!.jl (xtd/arr-foldr [1 2 3] (fn [acc x] (return (+ (* acc 10) x))) 0))
  => 321)
