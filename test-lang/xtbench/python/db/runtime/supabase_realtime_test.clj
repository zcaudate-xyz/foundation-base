(ns xtbench.python.db.runtime.supabase-realtime-test
  (:use code.test)
...
^{:refer xt.db.runtime.supabase-realtime/endpoint-url :added "4.1.3"}
...
^{:refer xt.db.runtime.supabase-realtime/create-client :added "4.1.3"}
(fact "buffers join messages until open and dispatches postgres change payloads"
  ...
  (!.py
   ...
   (. current (emit "open" {}))
   (var join-msg (JSON.parse (xt/x:first sent)))
...
^{:refer xt.db.runtime.supabase-realtime/set-auth :added "4.1.3"}
(fact "sends auth refresh and leave frames for joined channels"
  ...
  (!.py
   ...
   (var auth-msg (JSON.parse (xt/x:get-idx sent 1)))
   (var leave-msg (JSON.parse (xt/x:get-idx sent 2)))
   [(. auth-msg ["event"])
