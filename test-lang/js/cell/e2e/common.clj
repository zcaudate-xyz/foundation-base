    ...
    '(do
      ...
      (when (xt.lang.spec-base/x:nil? existing-db)
        ...
        (:= existing-db
            (xt.lang.common-data/obj-assign
             (xt.db.system/db-create {"::" "db.cache"}
                              schema
                              {"Order" {"position" 0}}
                              nil)
             {"schema" schema
              "views" views}))
        (xt.db.system/sync-event existing-db
                          ["add"
                           {"Order" [{"id" "ord-1" "status" "open"}
                                     {"id" "ord-2" "status" "closed"}]}])
      ...
      (js.cell.kernel.worker-local/actions-init
       {"@e2e/query"
        {"handler"
         (fn [query-plan]
           (var rows (xt.db.system/db-pull-sync
                      (!:G __E2E_REMOTE_DB)
                      {"Order"
                       {"id" {"ident" "id" "type" "text" "order" 0}
                        "status" {"ident" "status" "type" "text" "order" 1}}}
        ...
        {"handler"
         (fn [sync-request]
           ...
           (when (and (xt.lang.spec-base/x:is-object? db-sync)
              ...
              (xt.db.system/sync-event (!:G __E2E_REMOTE_DB) ["add" db-sync]))
           (return sync-request))
         "is_async" false
         "args" ["sync_request"]}}
       worker)
