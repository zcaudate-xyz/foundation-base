...
^{:refer postgres.sample.scratch-v1/as-array :added "4.0"}
(fact "returns a jsonb array"

  (l/emit-as :postgres `[(scratch/as-array "{}")])
  => string?)

^{:refer postgres.sample.scratch-v1/TaskCache :added "4.0"}
(fact "constructs a task cache")

^{:refer postgres.sample.scratch-v1/Task :added "4.0"}
(fact "constructs a task")

^{:refer postgres.sample.scratch-v1/Entry :added "4.0"}
(fact "construcs an entry")

^{:refer postgres.sample.scratch-v1/as-upper :added "4.0"}
(fact "converts to upper case"

  (l/emit-as :postgres `[(scratch/as-upper "abc")])
  => "\"scratch\".as_upper('abc')")

^{:refer postgres.sample.scratch-v1/ping :added "4.0"}
(fact "tests that the db is working"

  (l/emit-as :postgres `[(scratch/ping)])
  => "\"scratch\".ping()")

^{:refer postgres.sample.scratch-v1/ping-ok :added "4.0"}
(fact "tests that the db is working with json"

  (l/emit-as :postgres `[(scratch/ping-ok)])
  => "\"scratch\".ping_ok()")

^{:refer postgres.sample.scratch-v1/echo :added "4.0"}
