...
^{:refer postgres.gen.template-code/route-entry-input :added "4.1"
  :id test-route-entry-input-log-append}
...
^{:refer postgres.gen.template-code/route-entry-input :added "4.1"
  :id test-route-entry-input-custom-root}
...
^{:refer postgres.gen.template-code/route-entries :added "4.1"}
(fact "binds every API function in deterministic descriptor-id order"
  (let [entries (gen/route-entries ['postgres.sample.scratch-v0])]
    (mapv first entries)
    => '[postgres.sample.scratch-v0/echo
         postgres.sample.scratch-v0/log-all
         postgres.sample.scratch-v0/log-append
         postgres.sample.scratch-v0/log-append-public
         postgres.sample.scratch-v0/log-default
         postgres.sample.scratch-v0/ping]
...
(fact "binds select and return views with the correct entry kinds"
  (let [entries (gen/view-entries ['postgres.sample.scratch-v0])]
    (mapv (fn [[sym descriptor]]
            [sym (:id descriptor) (:table descriptor)
             (cond (contains? descriptor :select-entry) :select
                   (contains? descriptor :return-entry) :return)])
          entries)
    => '[[postgres.sample.scratch-v0/log-all "log_all" "Log" :select]
         [postgres.sample.scratch-v0/log-default "log_default" "Log" :return]]
    (get-in entries [1 1 :return-entry :input])
    => [{:symbol "i_log_id" :type "uuid"}]))

^{:refer postgres.gen.template-code/emit-module-entry :added "4.1"}
(fact "emits one unquoted descriptor as a typed def.xt form"
  (gen/emit-module-entry
   :route
   ['sample/source {:id "source" :input [] :return "jsonb"}])
  => #(and (str/starts-with? % "(def.xt ^{:api/type :route} source")
           (str/includes? % "{:id \"source\" :input [] :return \"jsonb\"}")
           (not (str/includes? % "\"{:id"))))
