(ns postgres.core.impl-base-test
  (:require [postgres.core]
            [tahto.runtime.postgres.base.application :as app]
            [postgres.core.impl-base :refer :all]
            [postgres.sample.scratch-v1 :as scratch]
            [tahto.core :as l]
            [tahto.base.book :as book])
  (:use code.test))

(l/script- :postgres
  {:require [[postgres.sample.scratch-v1 :as scratch]]
   :static {:application ["scratch"]
            :all    {:schema   ["scratch"]}}})

(deftype.pg Hello
  [:id   {:type :text :primary true}
   :task {:type :ref :ref {:ns scratch/Task}
          :profiles {:web  {:editable #{:create
                                        :modify}}}}])

(def -tsch- (get-in (app/app "scratch")
                    [:schema
                     :tree
                     :Task]))

^{:refer postgres.core.impl-base/prep-entry :added "4.0"
  :setup [@Hello]}
(fact "prepares data given an entry sym"

  (second (prep-entry '-/Hello (l/rt:macro-opts :postgres)))
  => book/book-entry?)

(fact "prep-entry prefers the live current-module entry"
  (with-redefs [tahto.common.emit-common/emit-symbol-classify (fn [_ _] [:self '-])
                tahto.model.spec-postgres.common/pg-resolve-entry
                (fn [_ _]
                  [{:modules {}}
                   {:id 'Hello
                    :module 'demo
                    :lang :postgres
                    :section :code
                    :static/schema-seed {:vec [:Hello [:id {:type :uuid :primary true}]]}}])]
    (second (prep-entry '-/Hello {:lang :postgres
                                  :snapshot {}
                                  :module {:id 'demo
                                           :code {'Hello {:id 'Hello}}}})))
  => (contains {:id 'Hello
                :lang :postgres
                :section :code}))

^{:refer postgres.core.impl-base/prep-table :added "4.0"}
(fact "prepares data related to the table sym"

  (prep-table '-/Hello false (l/rt:macro-opts :postgres))
  => vector?)

(fact "prep-table overlays module tables onto the application view"
  (with-redefs [tahto.common.emit-common/emit-symbol-classify (fn [_ _] [:self '-])
                tahto.model.spec-postgres.common/pg-resolve-entry
                (fn [_ _]
                  [{:modules {}}
                   {:id 'Entry
                    :module 'demo
                    :lang :postgres
                    :section :code
                    :static/application ["demo"]
                    :static/schema-seed {:vec [:Entry [:id {:type :uuid :primary true}]]}}])
                app/app (fn [_]
                          {:tables {}
                           :pointers {}})]
    (let [module {:id 'demo
                  :code {'Entry {:id 'Entry
                                 :module 'demo
                                 :lang :postgres
                                 :section :code
                                 :static/application ["demo"]
                                 :static/schema-seed {:vec [:Entry [:id {:type :uuid :primary true}]]}}}}
          [_ tsch mopts] (prep-table '-/Entry true {:lang :postgres
                                                    :snapshot {}
                                                    :module module})]
      [(contains? tsch :id)
       (contains? (-> mopts :application :pointers) 'Entry)]))
  => [true true])

^{:refer postgres.core.impl-base/t-input-check :guard true :added "4.0"}
(fact "passes the input if check is ok"

  (t-input-check (get-in (app/app "scratch")
                         [:schema
                          :tree
                          :Hello])
                 {:id "a"})
  => {:id "a"}

  (t-input-check (get-in (app/app "scratch")
                         [:schema
                          :tree
                          :Hello])
                 {})
  => (throws)

  (t-input-check (get-in (app/app "scratch")
                         [:schema
                          :tree
                          :Task])
                 '{:status a
                   :name "hello"
                   :cache hello})
  => map?)

^{:refer postgres.core.impl-base/t-input-collect :added "4.0"}
(fact "adds schema info to keys of map"

  (t-input-collect -tsch-
                   '{:status a})
  => (contains {:status (contains ['a map?])}))

^{:refer postgres.core.impl-base/t-val-process :added "4.0"}
(fact "allows additional filters to be added"

  (t-val-process [['add 5]
                  ['sub 5]]
                 'i)
  => '(sub (add i 5) 5))

^{:refer postgres.core.impl-base/t-val-fn :added "4.0"}
(fact "builds a js val given input"

  (t-val-fn -tsch-
            :status 'a {} {})
  => '(++ a postgres.sample.scratch-v1/EnumStatus)

  (t-val-fn -tsch-
            :cache 'a {}
            (last (prep-table 'scratch/Task true (l/rt:macro-opts :postgres))))
  => '(:uuid a)

  (fact "t-val-fn handles composite primary key reference"
    (with-redefs [book/get-base-entry (fn [_ _ _ _]
                                        {:static/schema-primary [{:id :id :type :uuid}
                                                                 {:id :prospect_id :type :ref}]})
                  prep-table (fn [& _] [nil {:k [{:type :ref :ref {:link {:id :tbl}}}]} {}])]
      (t-val-fn {:k [{:type :ref :ref {:link {:id :tbl}}}]}
                :k
                "some-uuid"
                {}
                {:book {}}))
    => '(:uuid "some-uuid")))

^{:refer postgres.core.impl-base/t-key-attrs-fn :added "4.0"}
(fact "builds a js key"

  (t-key-attrs-fn [:cache  (get -tsch- :cache)])
  => "cache_id"

  (t-key-attrs-fn [:id  (get -tsch- :id)])
  => "id")

^{:refer postgres.core.impl-base/t-key-fn :added "4.0"}
(fact "builds a js key"

  (t-key-fn -tsch-
            :cache)
  => "cache_id"

  (t-key-fn -tsch-
            :id)
  => "id")

^{:refer postgres.core.impl-base/t-sym-fn :added "4.0"}
(fact "builds a json access form"

  (t-sym-fn -tsch-
            :cache
            'e)
  => '(:uuid (coalesce (:->> e "cache_id") (:->> (:-> e "cache") "id")))

  (t-sym-fn -tsch-
            :id
            'e)
  => '(:uuid (:->> e "id")))

^{:refer postgres.core.impl-base/t-build-js-map :added "4.0"}
(fact "builds a js map"

  (t-build-js-map -tsch-
                  (t-input-collect -tsch-
                                   '{:status a
                                     :name "hello"
                                     :cache hello})
                  {:coalesce true}
                  (last (prep-table 'scratch/Task true (l/rt:macro-opts :postgres))))
  => '(jsonb-build-object "id" (postgres.core/uuid-generate-v4)
                          "status" (++ a postgres.sample.scratch-v1/EnumStatus)
                          "name" (:text "hello")
                          "cache_id" (:uuid hello)
                          "__deleted__" false))

^{:refer postgres.core.impl-base/t-create-fn :added "4.0"}
(fact "the flat create-fn"

  (l/with:macro-opts [(l/rt:macro-opts :postgres)]
    (t-create-fn 'scratch/Task '{:status a
                                 :name "hello"
                                 :cache hello}
                 {:coalesce true}))
  => '(jsonb-build-object "id" (postgres.core/uuid-generate-v4)
                          "status" (++ a postgres.sample.scratch-v1/EnumStatus)
                          "name" (:text "hello")
                          "cache_id" (:uuid hello)
                          "__deleted__" false))

^{:refer postgres.core.impl-base/t-returning-cols-default :added "4.0"}
(fact "formats returning cols default"
  (t-returning-cols-default [:id] t-key-attrs-fn)
  => vector?)

^{:refer postgres.core.impl-base/t-join-target-sym :added "4.1"}
(fact "t-join-target-sym builds a namespaced symbol from link info"
  (t-join-target-sym {:module 'scratch :id 'Task}) => 'scratch/Task
  (t-join-target-sym {:ns 'scratch :id 'Task}) => 'scratch/Task
  (t-join-target-sym {:module 'scratch}) => nil)

^{:refer postgres.core.impl-base/t-returning-cols :added "4.0"}
(fact "formats returning cols given"

  (t-returning-cols -tsch-
                    [:*/data]
                    t-key-attrs-fn)
  => [#{"id"} #{"status"} #{"name"} #{"time_created"} #{"time_updated"}]

  (t-returning-cols -tsch-
                    [:-/id]
                    t-key-attrs-fn)
  => [#{"id"}]

  (t-returning-cols -tsch-
                    [:name :time-created]
                    t-key-attrs-fn)
  => [#{"name"} #{"time_created"}]

  (t-returning-cols -tsch-
                    '[hello-id]
                    t-key-attrs-fn)
  => [#{"hello_id"}])

^{:refer postgres.core.impl-base/t-returning :added "4.0"}
(fact "formats the returning expression"

  (t-returning -tsch-
               #{:*/data})
  => '(--- [#{"id"} #{"status"} #{"name"} #{"time_created"} #{"time_updated"}])

  (t-returning -tsch-
               :*/data)
  => '(--- [#{"id"} #{"status"} #{"name"} #{"time_created"} #{"time_updated"}])

  (t-returning -tsch- '*)
  => '*


  (t-returning -tsch- #{{:expr '(count *)}})
  => '(--- [(count *)])

  (t-returning -tsch- '#{:id {:expr (concat #{id} "-" #{time})
                              :as hello}})
  => '(--- [#{"id"} [(concat #{id} "-" #{time}) :as hello]]))

^{:refer postgres.core.impl-base/t-where-hashvec-transform :added "4.0"}
(fact "transforms entries"

  (t-where-hashvec-transform [[:id 1] [:cache 1]] identity)
  => #{[:id 1 :or :cache 1]})

^{:refer postgres.core.impl-base/t-where-hashvec :added "4.0"}
(fact "function for where transform"

  (t-where-hashvec #{[:simple-id [:eq 1]]}
                   identity)
  => #{[:simple-id [:eq 1]]}

  (t-where-hashvec #{[:simple 1
                      :hello 1
                      [:or]
                      :id 1]}
                   identity)
  => #{[:simple 1 :and :hello 1 :or :id 1]})

^{:refer postgres.core.impl-base/t-where-transform :added "4.0"}
(fact "creates a where transform"

  (t-where-transform -tsch- {:id 1} {})
  => {"id" [:eq 1]}

  (t-where-transform -tsch- {:cache 1} {})
  => {"cache_id" [:eq 1]}

  (t-where-transform -tsch- #{[:id 1
                               :or
                               :cache 1]} {})
  => #{["id" [:eq 1] :or "cache_id" [:eq 1]]})

^{:refer postgres.core.impl-base/t-wrap-json :added "4.0"}
(fact "wraps a json return to the statement"

  (t-wrap-json '<FORM> :json 'js-agg 'output nil)
  => '[:with j-ret :as <FORM> \\ :select (js-agg j-ret) :from j-ret]

  (t-wrap-json '<FORM> :json 'js-agg 'output :id)
  => '[:with j-ret :as <FORM> \\ :select (js-agg (. j-ret #{:id})) :from j-ret]

  (t-wrap-json '<FORM> :record 'js-agg 'output nil)
  => '[:with j-ret :as <FORM> \\ :select (js-agg j-ret) :from j-ret])

^{:refer postgres.core.impl-base/t-join-transform :added "4.1"}
(fact "transforms join entries"

  (let [out (t-join-transform {:task [{:type :ref :ref {:link {:ns "scratch" :id "Task"}}}]}
                              [:task]
                              "Hello"
                              {})]
    (first (first out)) => :left-join
    (second (first out)) => 'scratch/Task))

^{:refer postgres.core.impl-base/t-wrap-join :added "4.1"}
(fact "adds a `join` clause"

  (t-wrap-join [] [[:left-join "target" :on [:= "s.id" "t.id"]]] {})
  => [[:left-join "target" :on [:= "s.id" "t.id"]]])

^{:refer postgres.core.impl-base/t-wrap-where :added "4.0"}
(fact "adds a `where` clause"

  (t-wrap-where []
                {:id 1}
                -tsch-
                {}
                {})
  => '[:where {"id" [:eq 1]}])

^{:refer postgres.core.impl-base/t-wrap-order-by :added "4.0"}
(fact "adds an `order-by` clause"

  (t-wrap-order-by []
                   [:id :status]
                   -tsch-
                   {})
  => '[:order-by (quote [#{"id"} #{"status"}])])

^{:refer postgres.core.impl-base/t-wrap-order-sort :added "4.0"}
(fact "determines asc or desc key"

  (t-wrap-order-sort []
                     :asc
                     -tsch-
                     {})
  => '[:asc])

^{:refer postgres.core.impl-base/t-wrap-limit :added "4.0"}
(fact "adds a `limit` clause"

  (t-wrap-limit [] 10 {})
  => [:limit 10])

^{:refer postgres.core.impl-base/t-wrap-offset :added "4.0"}
(fact "adds a `offset` clause"

  (t-wrap-offset [] 10 {})
  => [:offset 10])

^{:refer postgres.core.impl-base/t-wrap-into :added "4.0"}
(fact "adds `into` clause"

  (t-wrap-into [] 'hello {})
  => '[:into hello])

^{:refer postgres.core.impl-base/t-wrap-returning :added "4.0"}
(fact "adds `returning` clause"

  (t-wrap-returning [] [#{"id"} #{"status"}] {})
  => [:returning [#{"id"} #{"status"}]])

^{:refer postgres.core.impl-base/t-wrap-group-by :added "4.0"}
(fact "adds `group-by` clause"

  (t-wrap-group-by [] [#{"id"} #{"status"}] {})
  => [:group-by [#{"id"} #{"status"}]])

^{:refer postgres.core.impl-base/t-wrap-having :added "4.1"}
(fact "adds a `having` clause"

  (t-wrap-having [] {:id 1} -tsch- {} {})
  => '[:having {"id" [:eq 1]}])

^{:refer postgres.core.impl-base/t-wrap-args :added "4.0"}
(fact "adds `additional` args"
  (t-wrap-args [] [:a] {})
  => [:a])

^{:refer postgres.core.impl-base/t-wrap-lock :added "4.1"}
(fact "adds lock clauses and optional newline separators"
  (t-wrap-lock [] [:update] {})
  => [:for :update]

  (t-wrap-lock [:select :*] [:share :nowait] {:newline true})
  => [:select :* \\ :for :share :nowait])
