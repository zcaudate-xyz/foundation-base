...
(fact "gets a memoized hash id"
  ...
  (!.js
   ...
   (var h1 (util/hash-with-cache fresh
                                 (fn [x] (:= calls (+ calls 1)) (return 999))))
   [calls h-preset h0 h1
    (xt/x:get-key preset "_hash")
    (xt/x:get-key fresh "_hash")])
  => [1 123 42 42 123 42])
...
(fact "wraps a hash function with caching"
  ...
  (!.js
   ...
   [editable-calls e0 e1
    cached-calls c0 c1 (xt/x:get-key cached-obj "_hash")
    default-calls d0 d1 (xt/x:get-key default-obj "_hash")])
  => [2 42 42
      1 42 42 42
      1 42 42 42])
