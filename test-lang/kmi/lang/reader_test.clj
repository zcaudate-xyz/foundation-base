(ns kmi.lang.reader-test
  (:require [hara.lang :as l]
            [xt.lang.common-notify :as notify])
  (:use code.test))

^{:seedgen/root {:all true :langs [:lua :python :dart]}}
(l/script- :js
  {:runtime :basic
   :require [[xt.lang.spec-base :as xt]
             [kmi.lang.reader :as rdr]
             [xt.lang.common-repl :as repl]]})

(fact:global
 {:setup [(l/rt:restart)]
 :teardown [(l/rt:stop)]})

^{:refer kmi.lang.reader/unread-char :id kmi-extra-1}
(fact "throws when unreading before any char has been consumed"

  (!.js
   (rdr/unread-char (rdr/create "a") "a"))
  => (throws))

^{:refer kmi.lang.reader/throw-reader :id kmi-extra-2}
(fact "reports unexpected eof when requested"

  (!.js
   (rdr/read-while (rdr/create "abc")
                   (fn:> [ch] (not= ch nil))
                   false))
  => (throws))

^{:refer kmi.lang.reader/impl-char-at :added "4.1"}
(fact "extracts one-character slices from strings"

  (!.js
   [(rdr/impl-char-at "abc" 0)
    (rdr/impl-char-at "abc" 1)
    (== nil (rdr/impl-char-at "abc" 3))])
  => ["a" "b" true])

^{:refer kmi.lang.reader/reader? :added "4.1"}
(fact "creates runtime reader values"

  (!.js
   (var reader (rdr/create "abc"))
   [(rdr/reader? reader)
    (xt/x:get-key reader "::")
    (rdr/reader-position reader)])
  => [true "reader" [1 1]])

^{:refer kmi.lang.reader/create :added "4.1"}
(fact "creates readers with empty buffers and zero offset"

  (!.js
   (var reader (rdr/create "abc"))
   [(xt/x:get-key reader "idx")
    (xt/x:get-key reader "line")
    (xt/x:get-key reader "col")
    (== 0 (xt/x:len (xt/x:get-key reader "pushback")))
    (== 0 (xt/x:len (xt/x:get-key reader "history")))])
  => [0 1 1 true true])

^{:refer kmi.lang.reader/reader-position :added "4.1"}
(fact "tracks line and column positions"

  (!.js
   (var reader (rdr/create "ab"))
   (rdr/read-char reader)
   (rdr/reader-position reader))
  => [1 2])

^{:refer kmi.lang.reader/throw-reader :added "4.1"}
(fact "throws reader errors with position context"

  (!.js
   (var reader (rdr/create "ab"))
   (rdr/read-char reader)
   (rdr/throw-reader reader "boom"))
  => (throws))

^{:refer kmi.lang.reader/peek-char :added "4.1"}
(fact "peeks and reads chars without losing position"

  (!.js
   (var reader (rdr/create "abc"))
   [(rdr/peek-char reader)
    (rdr/read-char reader)
    (rdr/peek-char reader)
    (rdr/read-char reader)
    (rdr/read-char reader)
    (== nil (rdr/read-char reader))
    (rdr/reader-position reader)])
  => ["a" "a" "b" "b" "c" true [1 4]])

^{:refer kmi.lang.reader/read-char :added "4.1"}
(fact "reads chars sequentially"

  (!.js
   (var reader (rdr/create "ab"))
   [(rdr/read-char reader)
    (rdr/read-char reader)
    (== nil (rdr/read-char reader))])
  => ["a" "b" true])

^{:refer kmi.lang.reader/unread-char :added "4.1"}
(fact "unreads chars and restores the exact position"

  (!.js
   (var reader (rdr/create "a\nb"))
   (var a (rdr/read-char reader))
   (var nl (rdr/read-char reader))
   (var after-newline (rdr/reader-position reader))
   (rdr/unread-char reader nl)
   [(rdr/reader-position reader)
    after-newline
    (rdr/read-char reader)
    (rdr/reader-position reader)
    (rdr/read-char reader)])
  => [[1 2] [2 1] "\n" [2 1] "b"])

^{:refer kmi.lang.reader/step-char :added "4.1"}
(fact "supports stepping ignoring and slurping the remaining input"

  (!.js
   (var reader (rdr/create "abcd"))
   (rdr/step-char reader)
   (rdr/ignore-char reader)
   [(rdr/reader-position reader)
    (rdr/slurp reader)
    (== nil (rdr/peek-char reader))])
  => [[1 3] "cd" true])

^{:refer kmi.lang.reader/ignore-char :added "4.1"}
(fact "ignores chars while advancing the reader"

  (!.js
   (var reader (rdr/create "ab"))
   [(== nil (rdr/ignore-char reader))
    (rdr/read-char reader)])
  => [true "b"])

^{:refer kmi.lang.reader/read-while :added "4.1"}
(fact "scans while predicates match"

  (!.js
   (var reader (rdr/create "abc 123"))
   [(rdr/read-while reader
                    (fn:> [ch]
                      (and (not= ch nil)
                           (not= ch " ")))
                    true)
    (rdr/peek-char reader)])
  => ["abc" " "])

^{:refer kmi.lang.reader/read-until :added "4.1"}
(fact "scans until predicates match"

  (!.js
   (var reader (rdr/create "abc 123"))
   [(rdr/read-until reader (fn:> [ch] (== ch "1")) true)
    (rdr/peek-char reader)])
  => ["abc " "1"])

^{:refer kmi.lang.reader/slurp :added "4.1"}
(fact "slurps the remaining input"

  (!.js
   (var reader (rdr/create "abcd"))
   (rdr/read-char reader)
   (rdr/slurp reader))
  => "bcd")
