(ns lua.lib.client-websocket-test
...
(l/script- :lua.nginx
  {:runtime :basic
   ...
   :require [[xt.lang.spec-base :as xt]
             [lua.lib.client-websocket :as lua-ws]
...
^{:refer lua.lib.client-websocket/client :added "4.1.3"}
  (fact "wraps raw lua websocket clients with sync and async client methods"
...
^{:refer lua.lib.client-websocket/driver :added "4.1.3"}
(fact "connects through the lua websocket driver with promise and sync variants"
     ...
     (local driver
            ...
            (fn [url]
              (return {"url" url
                       ...
                       "close" (fn [_code _reason] (return true))
                       "addEventListener" (fn [_event _handler] (return true))}))))
     (local client (ws/connect-sync driver "wss://demo.test/socket"))
     [(ws/client? client)
      (. (. client ["_raw"]) ["url"])])]
  => [true
