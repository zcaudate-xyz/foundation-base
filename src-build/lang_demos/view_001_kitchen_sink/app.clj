(ns lang-demos.view-001-kitchen-sink.app
...
(defn.js App
  ...
  (var active-tab (xt/x:get-idx local-state 0))
  ...
  (var spec (:? (== active-tab "sink")
                (demos/kitchen-sink-spec)
                (demos/web-escape-spec)))
  (var render-fn (:? (== active-tab "sink")
   ...
   [:div {:className "min-h-screen bg-slate-50 text-slate-900"}
    [:div {:className "flex flex-row gap-2 border-b border-slate-200 bg-white p-4"}
     [:% -/TabButton {:label "Kitchen sink"
                      :active (== active-tab "sink")
                      :on-press (fn [] (setTab "sink"))}]
     [:% -/TabButton {:label "Web escape"
                      :active (== active-tab "escape")
                      :on-press (fn [] (setTab "escape"))}]]
    [:% react-view/View {:key active-tab
                         :node node
                         :spec spec
                         :render-fn render-fn
