(ns js.cell-v3.kernel.impl-model
...
(defn.js wrap-cell-args
  "puts the cell link as the first argument"
  {:added "4.0"}
  [handler]
  (return (fn [context]
...
(defn.js refresh-view-dependents
  ...
  (k/for:object [[dmodel-id dview-ids] dependents]
    (var throttle (. models [dmodel-id] ["throttle"]))
...
(defn.js add-model
  "attaches a model and immediately refreshes its views"
  {:added "4.0"}
  [cell model-id views]
...
(defn.js remove-model
  ...
  [cell model-id]
  (var models (. cell ["models"]))
  (var dependents (-/get-model-dependents cell model-id))
...
(defn.js trigger-model-raw
  ...
  (k/for:object [[view-id view] views]
    ...
    (var check (util/check-event trigger topic event {:view view
                                                      ...
                                                      :cell cell}))
    (when check
      (th/throttle-run throttle view-id [event])
...
(defn.js trigger-view
  ...
  (when (util/check-event trigger topic event {:view view
                                               :model model
                                               :cell cell})
    (return (th/throttle-run (. model ["throttle"])
                             view-id
                             [event])))
  (return nil))
...
(defn.js add-raw-callback
  ...
  (return (link/add-callback
           ...
           "@/raw"
           true
           (fn [input signal inner-link]
             (return (-/trigger-all cell signal input))))))
...
(defn.js remove-raw-callback
  ...
  [cell]
  (var raw-link (. cell ["link"]))
  (return (link/remove-callback raw-link "@/raw")))
