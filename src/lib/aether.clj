...
(defn collect-dependencies
  ...
  ([{:keys [^RepositorySystem system session repositories]} coords opts]
   (binding [listener/*progress* (atom [])
             listener/*pairs* (atom {})]
     (let [opts   (collection/merge-nested {:type :defalut
                                   :return :hierarchy
...
(defn resolve-dependencies
  ...
  ([{:keys [^RepositorySystem system session repositories]} coords opts]
   (binding [listener/*progress* (atom [])
             listener/*pairs* (atom {})]
     (let [opts   (collection/merge-nested {:type :default
                                   :return :resolved
                                   :print {:hierarchy true
                                           :title true
                                           :timing true}}
...
(defn outdated?
  ...
  ([aether coords opts]
   (let [opts    (collection/merge-nested opts {:print {:results true}})
