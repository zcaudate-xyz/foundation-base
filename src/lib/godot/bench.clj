...
func _init():
    ...
    for i in range(args.size()):
        if args[i] == \"--port\" and i + 1 < args.size():
            port = int(args[i + 1])
    if port == 0:
        push_error(\"LANG_GODOT_ERROR: no port provided\")
        quit()
        return
    if server.listen(port) != OK:
        push_error(\"LANG_GODOT_ERROR: failed to listen on \" + str(port))
        quit()
        return
    print(\"LANG_GODOT_READY\")
  ...
  (let [file (java.io.File. (str root-dir "/godot.out"))
    ...
    (loop []
      (when (< (System/currentTimeMillis) deadline)
        (if (and (.exists file)
                 (clojure.string/includes? (slurp file) "LANG_GODOT_READY"))
          true
          (do (Thread/sleep 100)
              (recur)))))))
...
(defn- scratch-root
  ...
  {:added "4.1"}
  []
  (let [dir (str (System/getProperty "user.home") "/lang_godot_bench_" (System/currentTimeMillis))]
