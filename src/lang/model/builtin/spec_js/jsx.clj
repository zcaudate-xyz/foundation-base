(ns lang.model.builtin.spec-js.jsx
...
(defn jsx-key-fn
  ...
  ([k]
   (if (keyword? k)
     (keyword (case/camel-case (name k)))
     k)))
...
(defn jsx-standardise-params
  ...
  (let [{:keys [class className]} params
        ...
        value  (if (vector? className)
                 (clojure.string/join " " (map f/strn className))
                 className)
        params (if value
  ...
  ([arr]
   (let [[tag & children] arr
         ...
         [tag params children] (cond (map? params?)
                                     (let [params (->>  params?
                                                        (collection/map-keys jsx-key-fn)
...
(defn emit-jsx-set-params
  ...
  ([params grammar mopts]
   (let [emit-fn (fn [svar mvar fvar]
         ...
         is-sym (fn [x]
                  (and (symbol? x)
                       (not (clojure.string/starts-with? (name x)
                                              "..."))))]
...
(defn emit-jsx-params
  ...
  ([params grammar mopts]
   (let [body-arr (cond (empty? params) []
                        ...
                        :else (f/error "Not valid input." {:input params}))]
     (cond (empty? body-arr)
           ["" ""]

           (data/emit-singleline-array? body-arr)
           [" "  (clojure.string/join " " body-arr)]

           :else
           (let [spc (str (common/newline-indent) "  ")]
             [spc (clojure.string/join spc body-arr)])))))
...
(comment
  ...
  (emit/emit-main (:form (lang.core/ptr-entry js.ext.react/createStoreProvider))
                  (lang.core/grammar:js))
