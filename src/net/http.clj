...
(std.lib.foundation/intern-in client/request
             client/get
             client/post
             client/put
             client/patch
  ...
  (->> params
       (keep (fn [[k v]]
               (cond (nil? v)
                     ...
                     (->> (map-indexed
                           (fn [i x]
                             (str (url-encode (std.lib.foundation/strn k)) "[" (+ i 1) "]" "=" (url-encode (std.lib.foundation/strn x))))
                           v)
                          (interpose "&")
                          (apply str))
                     
                     :else
                     (str (url-encode (std.lib.foundation/strn k)) "=" (url-encode (std.lib.foundation/strn v))))))
       (interpose "&")
       (apply str)))
  ...
  ([url & [opts]]
   (let [events (atom [])
         ...
         thread (future
                  (foreach-fn
                   lines
                   (std.lib.function/fn:consumer [e]
