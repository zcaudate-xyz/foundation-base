(ns tahto.runtime.chromedriver.connection
...
(defn ws-url
  ...
  [{:keys [scheme host port]
    :or {host "localhost"
         port 9222
         scheme "http"}}]
  (let [body (-> (net.http/get (str scheme "://" host ":" port "/json/version"))
                 :body
                 json/read)]
    (or (get body "webSocketDebuggerUrl")
        (get-in (-> (net.http/get (str scheme "://" host ":" port "/json/list"))
                    :body
                    json/read)
