(ns tahto.runtime.jocl.exec
  (:require [std.protocol.component :as protocol.component]
            [std.lib.encode :as encode]
            [tahto.runtime.jocl.common :as common]
            [tahto.runtime.jocl.meta :as meta]
            [tahto.runtime.jocl.type :as type]
...
(defn exec-prep
  ...
  ([platform devices name code]
   (let [devices   (into-array (coll/seqify devices))
...
(defn exec-source
  ...
  (let [code      (or code (impl/emit-script
        ...
        ;; just the entry id (see tahto.model.spec-c/emit-defn).
