...
(defn runtime-library
  ...
  []
  (or  *library*
      (:library (space/space:rt-get :lang.annex))
      (default-library)))
...
(defn emit-direct
  ...
  (let [mopts (provenance/with-provenance
                ...
                {:tahto/phase :emit/direct
                 :tahto/subsystem :tahto.core.impl/emit-direct
                 :tahto/lang lang
...
(defn emit-scaffold-imports
  ...
  (let [[stage grammar book namespace mopts] (emit-options meta)
    ...
    (clojure.string/join "\n" (conj (vec imports-arr)
                         (emit-direct grammar
                                      (list 'do:>
                                            (list 'return (walk/postwalk (fn [x]
                                                                        (if (or (symbol? x)
                                                                                (keyword? x))
                                                                          (name x)
                                                                          x))
