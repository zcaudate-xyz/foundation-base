(ns tahto.model.spec-postgres.tf
  ...
  ([[_ val]]
   (let [obj-fn (fn obj-fn [val]
                  (cond (map? val)
                        ...
                        (keyword? val)
                        (case/snake-case (f/strn val))

                        (set? val)
                        (let [pairs  (loop [acc []
                                            [x & more :as arr] (seq val)]
                                       (cond (empty? arr) acc
                                             (keyword? x) (recur (conj acc [(case/snake-case (f/strn x))
                                                                            (obj-fn (first more))])
                                                                 (rest more))
                                             (symbol? x) (recur (conj acc [(pg-js-idx x) x])
                                                                more)
                                             :else (f/error "Not Valid" {:value x})))]
...
(comment
  (env/pl (tahto.core/pg
         (let [(:jsonb out)  {}
               _  (do:reduce out || :jsonb arr)]
           (return out))))
  (env/pl (tahto.core/pg (do:reduce out || :json [{} {}]))))
