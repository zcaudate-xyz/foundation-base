(ns tahto.model.spec-postgres.form-deftype
  (:require [clojure.string]
            ...
            [tahto.model.spec-postgres.form-defpartition :as form-defpartition]
  ...
  ([form sql]
   (let [{:keys [cascade default constraint generated raw]} sql
         cargs (cond (nil? constraint) []
                     (map? constraint) [:constraint (symbol (f/strn (:name constraint)))
...
(defn pg-deftype-uniques
  ...
  ([cols]
   (let [groups (->> (keep (fn [[k {:keys [type sql ref]}]]
                     ...
                     (mapcat identity)
                     (group-by first)
                     (collection/map-vals (partial map second)))]
     (mapv (fn [keys]
               ...
               (list 'quote
                     (map (fn [{:keys [id type ref]}]
                            (if (= :ref type)
                              ...
                              (symbol (case/snake-case (name id)))))
                          schema-primary))])]))))
...
(defn pg-deftype-indexes
  ...
  ([cols ttok]
   (let [c-indexes (keep (fn [[k {:keys [type sql ref]}]]
         ...
         _ (if (not-empty g-indexes) (f/error "TODO"))]
     s-indexes)))
...
(defn pg-deftype-gen-constraint
  ...
  ([sym [group entries] mopts]
   (let [table-name (case/snake-case (name sym))
  ...
  ([params col-spec]
   (if-let [partition (:partition-by params)]
     (let [[method cols] (cond (map? partition)
                               ...
                               :else
                               (f/error "Not Valid Definition" {:partition partition
                                                                :col-spec col-spec}))
           ...
           cols (doall (map (fn [col]
                              (let [col-key (if (set? col) (first col) col)
                                ...
                                (if (and (not attrs)
                                         (not (set? col)))
                                  (f/error "Partition Column Not Found" {:column col
                                                                         :available (keys col-map)}))
                                (hash-set (if attrs
                                            (if (= (:type attrs) :ref)
                                              (pg-deftype-ref-name col-key (:ref attrs))
                                              (case/snake-case (name col-key)))
                                            (case/snake-case (name col-key))))))
                              cols))]
  ...
  ([form]
   (let [mopts (preprocess/macro-opts)
         ...
         tconstraints (->> (:constraints params)
                           (mapv (fn [[k val]]
                                   (list '% [:constraint (symbol (f/strn k))
                                             :check (list 'quote (list val))]))))

         tcomment (if (and (:doc mdefn) (not (clojure.string/blank? (:doc mdefn))))
                    [[:comment :on :table ttok :is (:doc mdefn)]])

         ccomments (keep (fn [[col attrs]]
                           (if-let [doc (get-in attrs [:sql :comment])]
                             [:comment :on :column (list '. ttok #{(case/snake-case (name col))})
                              :is doc]))
                         col-spec)
  ...
  ([esym]
   (let [[esym & extras] (if (vector? esym) esym [esym])
         ...
         form @(or (resolve esym)
                   (f/error "Cannot resolve symbol." {:input esym}))]
     
     (->> (partition 2 form)
          (mapcat (fn [[k attr]]
                    [k (if-let [m (get extras k)]
                         (collection/merge-nested attr m)
                         attr)]))))))
...
(defn pg-deftype-format
  ...
  (let [[mdefn [op sym spec params]] (grammar-spec/format-defn form)
        ...
        fmeta {:static/tracker (if track (tracker/map->Tracker track))
               ...
               :static/dbtype :table}
        qmeta (collection/qualified-keys msym)]
    [(merge fmeta qmeta)
