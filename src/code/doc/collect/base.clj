...
(defn collect-namespaces
  ...
  ([{:keys [articles] :as interim} name]
   (let [all    (->> (get-in articles [name :elements])
                     (filter #(-> % :type (= :ns-form))))
         meta   (-> all first :meta)
         namespaces (collection/map-juxt [:ns identity] all)]
     (-> interim
         (update-in [:articles name :meta] (fnil collection/merge-nested {}) meta)
         (update-in [:namespaces] (fnil collection/merge-nested {}) namespaces)
...
(defn collect-article-metas
  ...
  ([{:keys [articles] :as interim} name]
   (let [articles (->> (get-in articles [name :elements])
     ...
     (-> interim
         (update-in [:articles name :meta] (fnil collection/merge-nested {}) (dissoc articles :type))
         (update-in [:articles name :elements]
...
(defn collect-global-metas
  ...
  ([{:keys [articles] :as interim} name]
   (let [global (->> (get-in articles [name :elements])
                     ...
                     (apply collection/merge-nested {}))]
     (-> interim
         (update-in [:global] (fnil collection/merge-nested {}) (dissoc global :type))
         (update-in [:articles name :elements]
