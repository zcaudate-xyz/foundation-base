...
(defn get-load-order
  ...
  (let [all-deps   (apply clojure.set/union
        ...
        lu-ns      (std.lib.collection/map-vals :ns input-deps)
        lu-path    (std.lib.collection/transpose lu-ns)
        load-deps  (set (keys lu-path))
        sys-deps   (clojure.set/difference
                    all-deps load-deps)
        mdeps      (std.lib.collection/map-entries
                    (fn [[path {:keys [ns deps]}]]
                      [ns (clojure.set/difference
                           deps sys-deps)])
                    input-deps)
        ordered    (std.lib.sort/topological-sort-order-by-deps
                    mdeps
                    (std.lib.sort/topological-sort mdeps))]
