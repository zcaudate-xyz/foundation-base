...
(defn.js TaskTranslateJsxComponent
  []
  (var pushHistory
       (fn [history item]
         (return
  ...
  (r/return-ui
   {:layout [:*/v
             ...
             :var/output-code    {:%/fn box/useBox
                                  :%/args [-/Global
                                           ["task.translate_jsxc"
                                            "output_code"]]}
             :var/history        {:%/fn ui/useLocalHistory
                                  :%/args ["task.translate_jsxc.history"]}
             :var/history-idx   {:%/fn box/useBox
                                :%/args [-/Global
                                         ["task.translate_jsxc"
                                          "history_idx"]]}}
    ...
    :actions {:action/translate-input
              [{:%/set-async :var/output-code
                :to (. (-/api-post "/api/translate/jsxc"
                                   :var/input-code)
                       (then (fn [output]
                               (navigator.clipboard.writeText output)
                               (:var/history
...
(defn.js TaskTranslatePlpgsql
  ...
  (r/return-ui
   {:layout [:*/v
             ...
             :var/output-code    {:%/fn box/useBox
                                  :%/args [-/Global
                                           ["task.translate_plpgsql"
                                            "output_code"]]}
             :var/history        {:%/fn ui/useLocalHistory
                                  :%/args ["task.translate_plpgsql.history"]}
             :var/history-idx   {:%/fn box/useBox
                                 :%/args [-/Global
                                          ["task.translate_plpgsql"
                                           "history_idx"]]}}
    ...
    :actions {:action/translate-input
              [{:%/set-async :var/output-code
                :to (. (-/api-post "/api/translate/plpgsql"
                                   :var/input-code)
                       (then (fn [output]
                               (navigator.clipboard.writeText output)
                               (:var/history
    ...
    [:% ui/TabComponent
     {:controls controls
      ...
      :pages [{:title "HTML"
              ...
              {:title "Js"
               :content -/TaskTranslateJs}
              {:title "Ui"
               :content -/TaskTranslateJsxComponent}
              {:title "Plpgsql"
               :content -/TaskTranslatePlpgsql}]}]]))
