...
(defn.js OutlinerPanel [{:# [components selectedComponent onSelectComponent onDeleteComponent onMoveComponent]}]
  ...
  (var [hiddenNodes setHiddenNodes] (r/useState (new Set())))
    ...
    [:div {:className "flex flex-col h-full bg-[#252525]"}
      ...
      [:div {:className "h-8 bg-[#2b2b2b] border-t border-[#323232] flex items-center px-3"}
        [:span {:className "text-[10px] text-gray-600"}
          (+ (. components (reduce (fn [count c] (return (+ count (+ (. c.children length) 1)))) 0)) " objects")]]]))

(defn.js TreeNode [{:# [component depth selectedComponent expandedNodes hiddenNodes onSelectComponent onDeleteComponent onMoveComponent toggleExpanded toggleVisibility]}]
    ...
    [:div
      [:div
        ...
        [:div
          ...
          (:? (and component.label (!= component.label component.type))
