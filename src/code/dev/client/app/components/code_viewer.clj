...
(defn.js CodeViewer [{:# [components]}]
  (var generateLayout (fn [component (:= indent 0)]
                        ...
                        (when (> component.children.length 0)
                          (. component.children (forEach (fn [child]
...
"))
  ...
  (var copyToClipboard (async (fn []
                                (try
                                  (await (. navigator.clipboard (writeText fullCode)))
                                  (. toast (success "std.lang code copied to clipboard!"))
                                  (catch err
  ...
  (return
    [:div {:className "flex flex-col h-full bg-white"}
      [:div {:className "px-4 py-2 bg-gray-100 border-b flex items-center justify-between"}
        ...
        [:% fg/Button {:size "sm" :variant "ghost" :onClick copyToClipboard}
