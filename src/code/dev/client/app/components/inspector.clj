(ns smalltalkinterfacedesign.components.inspector
...
(defn.js Inspector [{:# [object]}]
  (var renderObjectDetails (fn []
                             (when (not object)
                               (return [:div {:className "text-sm text-gray-500 italic"} "No object selected"]))

                             (var details [])

                             (if (and (== (typeof object) "object") (!= object nil))
                               (do
                                 (. details (push
                                              [:div {:key "self" :className "mb-2"}
                                                [:span {:className "text-blue-600"} "self"] ": " (. String (or object.result object))])))

                                 (when (and object.result (== (typeof object.result) "object"))
                                   (. (Object.entries object.result) (forEach (fn [[key value]]
                                                                                (. details (push
                               ...
                               (do
                                 (. details (push
                                              [:div {:key "value" :className "mb-2"}
                                                [:span {:className "text-blue-600"} "value"] ": " (. String object.result)])))
                                 (. details (push
