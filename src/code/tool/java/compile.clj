...
(defn javac
  ...
  ([packages {:keys [reload output] :as opts} sources]
   (let [sources   (cond->> sources
     ...
     (if success?
       (javac-process (collection/map-vals #(.toByteArray ^ByteArrayOutputStream %) @cache) opts)
       :not-compiled))))
