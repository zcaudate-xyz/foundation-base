...
(defn verify-seq
  ...
  ([ck data modifiers]
   (cond (= #{} modifiers)
         ...
         :else
         (f/error  "modifiers can only be :gaps-only and :in-any-order"))))

(defn contains-map
  "map check helper function for `contains`"
  {:added "3.0"}
  ([x]
   (let [ck (collection/map-vals common/->checker x)]
     (common/map->Checker
      {:tag :contains
       :doc "Checks if the result is a map having the following conditions:"
       :fn  (fn [res]
              (let [data (res/result-data res)]
                (and (map? data)
                     (verify-map ck data))))
       :expect ck}))))
