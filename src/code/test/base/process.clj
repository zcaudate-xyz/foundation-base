  ...
  ([{:keys [form original meta] :as op}]
   (let [result (-> (evaluate op)
         ...
         _    (intern *ns* (with-meta '*last* {:dynamic true})
                      result)]
     (std.lib.signal/signal {:test :form :result result})
  ...
  ([{:keys [input output meta] :as op}]
   (let [{:keys [guard before after]} context/*eval-check*
         ...
         _    (intern *ns* (with-meta '*last* {:dynamic true})
                      (:data actual))
         _    (after)]
     (std.lib.signal/signal {:test :check :result result})
     (when context/*results*
       (swap! context/*results* conj result))
     (if (and guard (not (:data result)))
       (std.lib.foundation/error "Guard failed" {}))
     result)))
...
(defn collect
  ...
  ([meta results]
   (let [results (if context/*results* @context/*results* results)]
     (std.lib.signal/signal {:id context/*run-id* :test :fact :meta meta :results results})
     (and (->> results
               (filter #(-> % :from (= :verify)))
               (mapv :data)
               (every? true?))
...
(defn skip-check
  ...
  {:added "3.0"}
  ([meta]
   (std.lib.signal/signal {:id context/*run-id* :test :fact :meta meta :results [] :skipped true}) :skipped))
