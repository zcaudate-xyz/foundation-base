              ...
              (for [[op & args] diff]
                (str (apply str (repeat indent " "))
                     (case op
                       ...
                       (pr-str [op args])))))
    (prose/indent (pretty/pprint-str diff) indent)))
...
(defn format-diff
  ...
  (cond (and (map? diff) (or (:+ diff) (:- diff) (:> diff)))
        ...
        :else
        (prose/indent (pretty/pprint-str diff) 4)))
  ...
  ([title color
   ...
   (let [line (if line (str "L:" line " @ ") "")]
     (str (ansi/style (pad-left 8 title) #{color :bold})
          ...
          (str "\n"  (ansi/style (pad-left 8 "Form:") #{color}) "  " (prose/indent-rest (pretty/pprint-str (or original form)) 12))))))
...
(defn print-on-error
  ...
  ([color on-error]
   (if on-error
     (str "\n" (ansi/style (pad-left 9 "On Error:") #{color :bold})
          "  " (prose/indent-rest (on-error-string on-error) 11))
     "")))

(defn print-throw
  "prints throw info"
  {:added "4.1"}
  ([{:keys [name data on-error] :as summary}]
   (print/println
              ...
              "  " (prose/indent-rest
                    (prose/join-lines
                     (take 20 (clojure.string/split-lines
                               (pr-str data))))
                    10))
         (print-on-error :yellow on-error)
  ...
  ([{:keys [name data actual check parent] :as summary}]
   (print/println
    (str (print-preliminary "TIMEOUT" :magenta  summary)
         (if parent (str "\n"  (ansi/style  (pad-left 8 "Parent") #{:magenta}) "  " (prose/indent-rest (pretty/pprint-str parent) 12)))
...
(defn print-failed
  "prints failed info"
  {:added "4.1"}
  ([{:keys [name actual check parent checker on-error] :as summary}]
     ...
     (print/println
      (str (print-preliminary "FAILED" :red summary)
           (if parent  (str "\n" (ansi/style  (pad-left 8 "Parent")
                                              #{:red}) "  " (prose/indent-rest (pretty/pprint-str parent) 12)))
           (if diff
             (str "\n"  (ansi/style (pad-left 8  "Actual:") #{:red}) "  " (prose/indent-rest (pretty/pprint-str result) 12))
             (str "\n"  (ansi/style (pad-left 8  "Check:") #{:red}) "  " check))
           (if diff
             (str "\n" (ansi/style (pad-left 8 "Diff:") #{:red :bold}) "  " (prose/indent-rest (format-diff diff) 10))
             (str "\n"  (ansi/style (pad-left 8 "OUTPUT") #{:red :bold}) "  " (prose/indent-rest
                                                                               (prose/join-lines
                                                                                (take 20 (clojure.string/split-lines
                                                                                          (pr-str result))))
                                                                               10)))
           (print-on-error :red on-error)
