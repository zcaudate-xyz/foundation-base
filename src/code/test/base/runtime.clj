...
(defn remove-link
  "remove ms link"
  {:added "3.0"}
  ([link] (remove-link (env/ns-sym) link))
  ([ns link]
...
(defn get-fact
  ...
  {:added "3.0"}
  ([id]
   (get-fact (env/ns-sym) id))
  ([ns id]
   (if (symbol? id)
     (get-in @context/*registry* [ns :facts id])
     (get-fact (env/ns-sym) ns id)))
  ([ns id k & more]
...
(defn set-in-fact
  ...
  {:added "3.0"}
  ([id ks data]
   (set-in-fact (env/ns-sym) id ks data))
  ([ns id ks data]
...
(defn set-flag
  ...
  {:added "3.0"}
  ([id flag val]
   (set-flag (env/ns-sym) id flag val))
...
(defn teardown-fact
  ...
  {:added "3.0"}
  ([id]
   (teardown-fact (env/ns-sym) id))
  ([ns id]
   (let [teardown-fn  (get-fact ns id :function :teardown)
         out    (env/explode (teardown-fn))
