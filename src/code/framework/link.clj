   ...
   (let [path   (fs/path root file)]
     (if (fs/exists? path)
       (->> (read-string (slurp path))
            (std.lib.collection/map-entries (fn [[k entry]]
...
(defn create-file-lookups
  ...
  {:added "3.0"}
  ([project]
   (std.lib.collection/pmap-vals (fn [suffix]
...
(defn collect-entries
  ...
  {:added "3.0"}
  ([packages lookups]
   (std.lib.collection/pmap-vals (fn [pkg]
                  (->> lookups
  ...
  ([packages lookups]
   (reduce (fn [lookups {:keys [entries]}]
             (reduce (fn [lookups entry]
                       (std.lib.collection/dissoc-nested lookups entry))
                     lookups
                     entries))
...
(defn collect-external-deps
  ...
  {:added "3.0"}
  ([packages]
   (std.lib.collection/pmap-vals (fn [{:keys [dependencies] :as package}]
                  (->> dependencies
                       (map (fn [artifact]
...
(defn collect-linkages
  ...
  {:added "3.0"}
  ([packages lookups]
   (std.lib.collection/pmap-vals (fn [{:keys [entries] :as package}]
                  (->> entries
                       (map (fn [entry]
...
(defn collect-internal-deps
  ...
  {:added "3.0"}
  ([packages]
   (let [packages (std.lib.collection/pmap-vals (fn [{:keys [name imports] :as package}]
        ...
        ;; CHECKS FOR CONSISTENCY
         _  (sort/topological-sort (std.lib.collection/pmap-vals :internal packages))]
     packages)))
...
(defn collect-transfers
  ...
  ([packages lookups project]
   (std.lib.collection/pmap-vals (fn [{:keys [entries bundle] :as package}]
  ...
  ([packages manifest]
   (if (= manifest :all)
     ...
     (let [find-deps (fn find-deps
           ...
           deps   (atom #{})
           lookup (std.lib.collection/map-vals :internal packages)
...
(defn all-linkages
  ...
  ([tag project {:keys [packages] :as deploy}]
   (let [tag    (or tag :public)
         ...
         packages (std.lib.collection/map-entries (fn [[k entry]]
                                   [k (assoc entry :name k)])
                                 packages)
         lookups  (create-file-lookups project)]
