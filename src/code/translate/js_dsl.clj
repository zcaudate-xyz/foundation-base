  ...
  (if (nil? node)
    ...
    (let [type (:type node)]
      (case type
        ...
        "JSXIdentifier" (symbol (:name node))
        ...
        (let [callee (translate-node (:callee node))
              args   (translate-args (:arguments node))]
          (list 'and callee (apply list callee args)))
        ...
        (let [left (translate-node (:left node))
              ...
              body (translate-node (:body node))]
          (list 'for:of (vector left right) body))

        "ForInStatement"
        (let [left (translate-node (:left node))
              right (translate-node (:right node))
              body (translate-node (:body node))]
          (list 'for:in (vector left right) body))
        ...
        "SwitchStatement"
        (let [discriminant (translate-node (:discriminant node))
              cases (:cases node)]
          (concat (list 'case discriminant)
                  (mapcat translate-node cases)))

        "SwitchCase"
        (let [test (if (:test node) (translate-node (:test node)) :else)
              consequent (translate-args (:consequent node))]
          [test (cons 'do consequent)])
        ...
        (let [props (:properties node)
              normal-props (remove #(= "SpreadElement" (:type %)) props)
              spread-props (filter #(= "SpreadElement" (:type %)) props)
              obj-map (apply hash-map
                             (mapcat (fn [p]
                                       [(if (:computed p)
                                     ...
                                     normal-props))]
           (if (seq spread-props)
             (assoc obj-map :.. (if (= 1 (count spread-props))
                                  (translate-node (:argument (first spread-props)))
                                  (vec (map (comp translate-node :argument) spread-props))))
             obj-map))
        ...
        "ImportDeclaration"
        (if (#{"type" "typeof"} (:importKind node))
          nil
        ...
        (let [declaration (:declaration node)]
          (if declaration
            (translate-node declaration)
            nil))

        "ExportSpecifier"
        (let [local (translate-node (:local node))
              exported (translate-node (:exported node))]
          (if (= local exported)
            local
            (list 'as local exported)))

        "ExportDefaultDeclaration"
        (translate-node (:declaration node))
        ...
        "ObjectPattern"
        (let [props (:properties node)
              rest-node (first (filter #(= "RestElement" (:type %)) props))
              normal-props (remove #(= "RestElement" (:type %)) props)
              keys (mapv (fn [p] (translate-node (:value p))) normal-props)]
          (cond-> {:# keys}
            rest-node (assoc :.. (translate-node (:argument rest-node)))))
        ...
        "AssignmentPattern"
        (list ':= (translate-node (:left node)) (translate-node (:right node)))
        ...
        "JSXNamespacedName"
        (let [ns (translate-node (:namespace node))
              name (translate-node (:name node))]
          (symbol (str ns ":" name)))
        ...
        (let [opening (:openingElement node)
              ...
              attrs (reduce (fn [m attr]
                              (if (= "JSXSpreadAttribute" (:type attr))
                                (assoc m :.. (translate-node (:argument attr)))
                                (assoc m
        ...
        "TSParameterProperty" (translate-node (:parameter node))

        ;; TS Nodes - Ignore or Strip
        "TSAsExpression" (translate-node (:expression node))
        "TSNonNullExpression" (translate-node (:expression node))
        "TSTypeAssertion" (translate-node (:expression node))
        "TSTypeParameterInstantiation" nil
        "TSInterfaceDeclaration" nil
        "TSTypeAliasDeclaration" nil
        "TSEnumDeclaration" nil
        "TSTypeAnnotation" nil
