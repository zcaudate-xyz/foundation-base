...
(std.lib.foundation/intern-all js.core.impl)
(std.lib.foundation/intern-in js.core.fetch/fetch
...
(defmacro.js timeout
  ...
  {:added "4.0"}
  [ms]
  (std.lib.template/$ (new Promise (fn [resolve]
    ...
    (fn [statement]
      (cond (vector? statement)
            (cond (= :init  (clojure.core/first statement))
                  ...
                  :else (std.lib.foundation/error "Not valid" {:input statement}))
            
            (or (std.lib.collection/form? statement)
                (symbol? statement))
...
(defmacro.js module:demos
  ...
  (apply vector :<> 
         (->> (ns-interns ns)
              (clojure.core/filter
               (fn [[k v]] (std.string.common/ends-with? (str k) "Demo")))
