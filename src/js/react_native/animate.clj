...
(std.lib.foundation/template-entries [l/tmpl-macro {:base "ReactNative.Animated.Value"
                                   :inst "val"
                                   :tag "js"}]
  [[start [] {:optional [cb]}]
   [stop []]
...
(defmacro.js ^{:standalone true} val
  ...
  {:added "4.0"}
  ([init]
   (std.lib.template/$ (React.useCallback (new ReactNative.Animated.Value ~init) []))))
