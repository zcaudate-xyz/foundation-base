...
(defn.js bind-system
  "binds a transport to a cell-v2 system"
  {:added "4.0"}
  [transport system opts]
  (:= opts (or opts {}))
  (when (and (. transport ["system"])
             (. transport ["listenerId"]))
    (cell-v2/remove-event-listener (. transport ["system"])
                                   (. transport ["listenerId"])))
  (:= (. transport ["system"]) system)
  (when (k/not-nil? (. opts ["forwardAll"]))
    (:= (. transport ["forwardAll"]) (. opts ["forwardAll"])))
  (var listener-id (or (. opts ["listenerId"])
                       (. transport ["listenerId"])
                       (k/cat (. transport ["id"])
                              "::forward")))
  (:= (. transport ["listenerId"]) listener-id)
  (cell-v2/add-event-listener
   system
   listener-id
   true
   (fn [input signal bus]
     (when (and (not (. transport ["suppressForward"]))
                (-/subscribed? transport signal))
       (return (-/send-frame transport
                             (protocol/emit (-/next-id transport "emit")
                                            signal
                                            (k/get-key input "status")
                                            (k/get-key input "body")
                                            (k/get-key input "meta")
                                            nil))))))
  (return transport))

(defn.js unbind-system
  "unbinds a transport from its current system"
  {:added "4.0"}
  [transport]
  (when (and (. transport ["system"])
             (. transport ["listenerId"]))
...
(defn.js handle-call
  ...
  (when (k/nil? system)
    (k/err "ERR - Transport has no bound system"))
  (try
    (var out (cell-v2/call-action system
                                  (protocol/frame-action frame)
...
(defn.js handle-emit
  ...
  (when (k/nil? system)
    (k/err "ERR - Transport has no bound system"))
  (:= (. transport ["suppressForward"]) true)
  (try
    (var out (cell-v2/emit-signal system
                                  (protocol/frame-signal frame)
                                  (k/get-key frame "body")
                                  (k/get-key frame "meta")
                                  (k/get-key frame "status")))
    (:= (. transport ["suppressForward"]) false)
    (return out)
    (catch err
      (:= (. transport ["suppressForward"]) false)
      (throw err))))
...
(defn.js receive-frame
  ...
  (cond (== op "hello")
        ...
        (== op "emit")
        (return (-/handle-emit transport frame))
