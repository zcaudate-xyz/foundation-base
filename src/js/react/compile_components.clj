...
(defn components-expand
  ...
  {:added "4.0"}
  [inputs]
  (let [inputs (std.lib.collection/map-vals
                (fn [tmpl]
                  (cond (vector? tmpl)
    ...
    (std.lib.collection/merge-nested inputs deps)))
...
(defn getter-symbol
  ...
  {:added "4.0"}
  [kw]
  (symbol (std.string.case/camel-case (name kw))))

(defn setter-symbol
  "creates the setter symbol"
  {:added "4.0"}
  [kw]
  (symbol (std.string.case/camel-case (str "set-" (name kw)))))
