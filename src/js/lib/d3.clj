...
(defn d3-macro
  ...
  ([[s props]]
   (with-meta
     (std.lib.template/$ (defmacro.js ~s
            ([& [m]]
...
(defn d3-tmpl
  ...
  ([s]
   (if (vector? s)
     (d3-macro s)
     (std.lib.template/$ (def$.js ~s ~(clojure.core/symbol (clojure.core/str "d3." s)))))))

(std.lib.foundation/template-entries [d3-tmpl {:category "stats"}]
  [min
   minIndex
   max
   maxIndex
...
(std.lib.foundation/template-entries [d3-tmpl {:category "search"} ] 
  [least
    leastIndex
    greatest
    greatestIndex
...
(std.lib.foundation/template-entries [d3-tmpl {:category "transform"}]
  [group
   groups
   index
   indexes
...
(std.lib.foundation/template-entries [d3-tmpl {:category "iterable"}]
  [every
   some
   filter
   map
...
(std.lib.foundation/template-entries [d3-tmpl {:category "set"}]
  [difference
   disjoint
   intersection
   superset
...
(std.lib.foundation/template-entries [d3-tmpl {:category "histogram"}]
  [[bin #{value domain  thresholds}]
    thresholdFreedmanDiaconis
    thresholdScott
    thresholdSturges])

(std.lib.foundation/template-entries [d3-tmpl {:category "interning"}]
  [InternMap
   InternSet])

(std.lib.foundation/template-entries [d3-tmpl {:category "axis"}]
  [axisTop
   axisRight
   axisBottom
   axisLeft
...
(std.lib.foundation/template-entries [d3-tmpl {:category "brush"}]
  [brushX
   brushY
   brushSelection
...
(std.lib.foundation/template-entries [d3-tmpl {:category "chord"}]
  [[chord #{padAngle
            sortGroups
            sortSubgroups
            sortChords}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "color"}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "scale-chromatic"
                              :tag "categorical"}]
  [schemeCategory10
   schemeAccent
   schemeDark2
...
(std.lib.foundation/template-entries [d3-tmpl {:category "contour"}]
  [[contours #{contour size smooth thresholds}]
   contourDensity
   [density #{x y weight size cellSize thresholds bandwidth}]])

(std.lib.foundation/template-entries [d3-tmpl {:category "delaunay"}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "dispatch"}]
  [[dispatch #{on copy call apply}]])

(std.lib.foundation/template-entries [d3-tmpl {:category "drag"}]
  [[drag #{container filter touchable subject clickDistance on}]
   dragDisable
   dragEnable])

(std.lib.foundation/template-entries [d3-tmpl {:category "csv"}]
  [csvParse
   csvParseRows
   csvFormat
   csvFormatBody
...
(std.lib.foundation/template-entries [d3-tmpl {:category "ease"}]
  [easeLinear
   [easePolyIn #{exponent}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "fetch"}]
  [blob
   buffer
   csv
   dsv
...
(std.lib.foundation/template-entries [d3-tmpl {:category "force"}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "format"}]
  [format
   formatPrefix
   formatSpecifier
   precisionFixed
...
(std.lib.foundation/template-entries [d3-tmpl {:category "geo"}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "hierarchy"}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "interpolate"}]
  [interpolate
   interpolateNumber
   interpolateRound
   interpolateString
...
(std.lib.foundation/template-entries [d3-tmpl {:category "path"}]
  [[path #{moveTo closePath lineTo quadraticCurveTo bezierCurveTo arcTo arc rect toString}]])

(std.lib.foundation/template-entries [d3-tmpl {:category "polygon"}]
  [polygonArea
   polygonCentroid
   polygonHull
   polygonContains
...
(std.lib.foundation/template-entries [d3-tmpl {:category "quadtree"}]
  [[quadtree #{x y extent cover add addAll remove removeAll copy root data size find visit visitAfter}]])

(std.lib.foundation/template-entries [d3-tmpl {:category "random"}]
  [randomUniform
   randomInt
   randomNormal
   randomLogNormal
...
(std.lib.foundation/template-entries [d3-tmpl {:category "selection"}]
  [selection
   select
   selectAll
   matcher
...
(std.lib.foundation/template-entries [d3-tmpl {:category "shape"}]
...
(std.lib.foundation/template-entries [d3-tmpl {:category "time"
                              :tag "format"}]
  [timeFormat
   timeParse
   utcFormat
...
(std.lib.foundation/template-entries [d3-tmpl {:category "transition"}]
  [interrupt
