(ns xt.lib.db.supabase
  (:require [std.lang :as l]))

;;
;; Canonical Supabase Realtime `postgres_changes` -> xt.lib.db sync adapter.
;;
;; - Input payload shape follows Supabase Realtime callback payload.
;; - We translate to `xt.lib.db` events and apply them to a local SQLite-backed xt.lib.db instance.
;;
...
(defn.js snake->kebab
  ...
  `pg/bind-schema` + `xt.lib.db` commonly operate on kebab-case keys.
...
(defn.js payload->xdb-events
  "Translates a Supabase `postgres_changes` payload into one (or two) xt.lib.db events.

  Options:
  - `:id-key` (string, default \"id\")

  Returns:
  - JS array of xt.lib.db events: `[\"add\" {\"Table\" [row]}]` or `[\"remove\" {\"Table\" [{\"id\" ...}]}]`."
  {:added "4.1.3"}
  [payload opts]
  (:= opts (or opts {}))
...
(defn.js apply-payload
  "Applies a Supabase payload to a local xt.lib.db instance using `xdb/sync-event`.
...
(defn.js attach-events
  ...
  (var channel-name (or (xt/x:get-key m "channel-name") "xt.lib.db.supabase"))
