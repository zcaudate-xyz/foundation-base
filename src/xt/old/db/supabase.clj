(ns xt.old.db.supabase
  (:require [std.lang :as l]))

;;
;; Canonical Supabase Realtime `postgres_changes` -> xt.old.db sync adapter.
;;
;; - Input payload shape follows Supabase Realtime callback payload.
;; - We translate to `xt.old.db` events and apply them to a local SQLite-backed xt.old.db instance.
;;
...
(defn.js snake->kebab
  ...
  `pg/bind-schema` + `xt.old.db` commonly operate on kebab-case keys.
...
(defn.js payload->xdb-events
  "Translates a Supabase `postgres_changes` payload into one (or two) xt.old.db events.

  Options:
  - `:id-key` (string, default \"id\")

  Returns:
  - JS array of xt.old.db events: `[\"add\" {\"Table\" [row]}]` or `[\"remove\" {\"Table\" [{\"id\" ...}]}]`."
  {:added "4.1.3"}
  [payload opts]
  (:= opts (or opts {}))
...
(defn.js apply-payload
  "Applies a Supabase payload to a local xt.old.db instance using `xdb/sync-event`.
...
(defn.js attach-events
  ...
  (var channel-name (or (xt/x:get-key m "channel-name") "xt.old.db.supabase"))
