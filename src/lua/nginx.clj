...
(defmacro.lua return-ok
  ...
  {:added "4.0"}
  [out]
  (template/$ (do (:= ngx.status ngx.HTTP-OK)
           (ngx.say ~out)
...
(defn ngx-tmpl
  ...
  (let [nsym (-> (name sym)
                 ...
                 (.replaceAll "_" "-")
                 (symbol))]
    (template/$ (def$.lua ~nsym ~(symbol (str "ngx." sym))))))

(f/template-entries [ngx-tmpl]
  +core+
  +http-method+
  +http-status+
  +log+
...
(defmacro.lua path
  ...
  {:added "4.0"}
  [file]
  (template/$ (cat (ngx.config.prefix) ~file)))
