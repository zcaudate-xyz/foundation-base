    ...
    (if entry
      (try
        (let [res (l/emit-as (keyword lang) (list (:form entry)))
              res (if (string? res) (clojure.string/trim res) res)
...
(defn search-components
  ...
  (let [book (l/get-book (l/default-library) (keyword lang))
        modules (book/list-entries book :module)
        q (clojure.string/lower-case query)
        matches (for [mod modules
                      ...
                      comp comps
                      :let [cname (name comp)]
                      :when (clojure.string/includes? (clojure.string/lower-case cname) q)]
...
(defn get-doc-path
  ...
  (try
    ...
    (let [all-pages-fn (resolve 'code.doc.executive/all-pages)
          ...
          ns-slug (clojure.string/replace ns-str "." "-")
          page (some (fn [[k p]]
                       (let [k-str (str k)]
                         (when (or (= k-str ns-slug)
                                   (clojure.string/ends-with? k-str (str "/" ns-slug)))
                           p)))
                     pages)]
      (if page
        {:path (:input page)
            ...
            (when (.exists f)
              (if (.isDirectory f)
                ...
                (do
                  ...
                  (swap! deleted conj (str f))))))
          (if (seq @deleted)
            {:status "ok" :message (str "Deleted " (clojure.string/join ", " @deleted))}
