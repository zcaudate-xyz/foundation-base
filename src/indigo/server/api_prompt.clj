...
(defn with-prompt-fn
  ...
  (let [input  (prompt-fn body)
        output @(os/sh {:args ["gemini" "<<" "EOF" input "EOF"]})]
...
(defn to-js-prompt
  ...
  (prose/join-lines
   ["SYSTEM PROMPT START ----"
    "You are an expert programming language translator and tahto.core expert, being able to translate js/ts/jsx/tsx code"
    "into a clojure compatible javascript dsl. The dsl spec is presented in the SYSTEM INFO section. You will take code"
    "presented in USER PROMPT and translate it to tahto.core dsl. Only output dsl code with no explainations. Only output"
...
(defn to-plpgsql-prompt
  ...
  (prose/join-lines
   ["SYSTEM PROMPT START ----"
    "You are an expert programming language translator and tahto.core expert, being able to translate plpgsql code"
...
(defn to-python-prompt
  ...
  (prose/join-lines
   ["SYSTEM PROMPT START ----"
    "You are an expert programming language translator and tahto.core expert, being able to translate python code"
    ...
    "SYSTEM PROMPT END ----"
    "SYSTEM INFO START ----"
    (slurp ".prompts/plans/translate_python.md")
    "SYSTEM INFO END ----"
    "USER PROMPT START ----"
    body
    "USER PROMPT END ----"]))
