   ...
   (let [from-ns   (symbol (str from "-test"))
     ...
     (spit from-path
           (.replaceAll (slurp from-path)
                        (str (std.lib.foundation/re-create (str from)))
                        (str to)))
...
(comment
  ...
  (fix-tests)
  (str (std.lib.foundation/re-create "hara.util"))
