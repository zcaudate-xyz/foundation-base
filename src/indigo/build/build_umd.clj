...
(defn initialise
  []
  (env/p (os/sh {:root ".build/code-dev-build-umd"
              :args ["npm" "install"]})))
...
(defn generate-umd
  ...
  (do (env/p (os/sh {:root ".build/code-dev-build-umd"
  ...
  (filter identity 
          (for [f  (keys
            ...
            (let [_       (env/p f)
              ...
              (env/p (diff/->string (diff/diff input output)))
              layout)))
...
(comment
  ...
  (os/sh {:root ".build/code-dev-build-umd"
         :args ["npm" "install"]
         :inherit true})
  (os/sh {:root ".build/code-dev-build-umd"
