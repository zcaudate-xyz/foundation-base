...
(defn script-ns-import
  ...
  ([{:keys [require require-impl] :as config}]
   (let [current (std.lib.env/ns-sym)]
     (alias '- current)
     (->> require-impl
...
(defn script-macro-import
  ...
  ([book restrict]
   (let [macros (->> (flatten (:macros book))
         ...
         syms   (map std.lib.foundation/var-sym macros)
         mns    (ut/sym-module (first syms))
         ids    (set (map ut/sym-id syms))
         curr   (std.lib.env/ns-sym)
...
(defn script-fn
  ...
  {:added "4.0"}
  ([lang]
   (script-fn lang (std.lib.env/ns-sym) {}))
  ([lang module]
   (if (map? module)
     (script-fn lang (std.lib.env/ns-sym) module)
...
(defn script-test-prep
  ...
  {:added "4.0"}
  ([lang config]
   (let [module-id (std.lib.env/ns-sym)
  ...
  ([ns tag body meta]
   (let [{:keys [lang module]
          :as rt}    (or (annex/get-annex-runtime ns tag)
                         (std.lib.foundation/error "Annex Not found"
...
(defmacro ^{:style/indent 1}
  ...
  ([tag body]
   (if (vector? tag)
     `(script-ext-run (quote ~(std.lib.env/ns-sym))
                      ~(first tag)
                      (quote ~body)
                      ~(meta &form)))))
...
(defn annex:start
  ...
  {:added "4.0"}
  ([tag]
   (annex:start tag (std.lib.env/ns-sym)))
  ([tag ns]
   (let [{:keys [registry
...
(defn annex:get
  ...
  {:added "4.0"}
  ([tag]
   (annex:get tag (std.lib.env/ns-sym)))
  ([tag ns]
...
(defn annex:stop
  ...
  {:added "4.0"}
  ([tag]
   (annex:stop tag (std.lib.env/ns-sym)))
  ([tag ns]
   (let [{:keys [runtimes]} (annex/get-annex ns)]
     (std.lib.atom/swap-return! runtimes
       (fn [m]
         (if-let [rt (get m tag)]
           [(std.lib.component/stop rt) (dissoc m tag)]))))))

(defn annex:start-all
  "starts all the annex tags"
  {:added "4.0"}
  ([]
   (annex:start-all (std.lib.env/ns-sym)))
  ([ns]
   (let [{:keys [registry]} (annex/get-annex ns)]
     (std.lib.collection/map-entries (fn [[tag {:keys [lang
                                      runtime
                                      config]}]]
...
(defn annex:stop-all
  ...
  {:added "4.0"}
  ([]
   (annex:stop-all (std.lib.env/ns-sym)))
  ([ns]
...
(defn annex:restart-all
  ...
  {:added "4.0"}
  ([]
   (annex:restart-all (std.lib.env/ns-sym)))
  ([ns]
   (annex:stop-all ns)
   (annex:start-all ns)))
