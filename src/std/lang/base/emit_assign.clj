...
(defn emit-def-assign-inline
  ...
  (let [[link-module link-id] (ut/sym-pair link)
        ...
        entry    (or (get-in book [:modules link-module :code link-id])
                     (std.lib.foundation/error "Cannot find entry" {:lang  lang
                                                   :input link}))
        _        (or (empty? (:deps entry))
                     (std.lib.foundation/error "Inline cannot have additional dependencies." {:lang  lang
                                                                             :input link}))
        [_ _ args & body] (:form entry)
        return-ref (volatile! nil)
        body       (std.lib.walk/postwalk (fn [form]
                                 (if (std.lib.collection/form? form)
                                   (cond (= 'return (first form))
                                         (if @return-ref
                                           (std.lib.foundation/error "Inline cannot have multiple returns." {:input @return-ref})
        ...
        _          (or (not (coll? @return-ref))
                       (std.lib.foundation/error "Return should be a token." {:input @return-ref}))
        assign-ref (volatile! nil)
        -          (std.lib.walk/postwalk (fn [form]
                                 (do (when (and (std.lib.collection/form? form)
                                                (= 'var (first form)))
                                       (let [asym (first (filter symbol? (rest form)))]
                                         (or (= @return-ref asym)
                                             (std.lib.foundation/error "Inlined with unaccounted for declarations"
                                                      {:link link
                                                       :form body}))
        ...
        body       (std.lib.walk/prewalk-replace smap body)
        #_#_
        _          (std.lib.env/prn smap @assign-ref @return-ref
                          body)]
    (if (and rsym? asym?)
      (apply list 'do* body)
         ...
         argstrs  (map (fn [{:keys [value symbol] :as arg}]
                         (let [{:assign/keys [inline template] :as aopts} (meta value)
                               custom (cond (:assign/fn aopts) [:raw  ((:assign/fn aopts) symbol)]
                                            template [:template (std.lib.walk/prewalk-replace {template symbol} value)]
                       ...
                       args)
         vstr    (std.string.common/join (str sep space) argstrs)
