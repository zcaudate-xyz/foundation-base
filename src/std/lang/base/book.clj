...
(std.lib.foundation/intern-in module/book-module
             module/book-module?
             entry/book-entry
             entry/book-entry?
             meta/book-meta
  ...
  ([{:keys [modules] :as book} id]
   (let [[mid sid] (ut/sym-pair id)]
     (or (get-base-entry book mid sid :code)
         ...
         (std.lib.foundation/error "ENTRY NOT FOUND"
                  {:module mid
                   :symbol sid})))))
...
(defn book-string
  ...
  ([{:keys [lang
            ...
            scope]}]
   (str "#book " [lang] " "
        (std.lib.collection/map-vals (fn [module]
                      (->> (select-keys module [:code :fragment])
                           (std.lib.collection/map-vals count)))
                    modules))))
...
(defn book-merge
  ...
  (let [_  (or (= (:parent book)
                  (:lang parent))
               (std.lib.foundation/error "Not mergable" {:book   (select-keys book   [:lang :parent :merged])
...
(defn assert-compatible-lang
  ...
  (or (check-compatible-lang book lang)
      (std.lib.foundation/error "Not compatible" {:book (select-keys book   [:lang :parent :merged])
                                 :lang lang})))
...
(defn put-module
  ...
  (let [_ (assert-compatible-lang book (:lang module))
        {:keys [id]} module]
    (atom/atom-put-fn book [:modules id] (std.lib.collection/filter-vals identity module))))
...
(defn assert-module
  ...
  (or (has-module? book module-id)
      (std.lib.foundation/error "No module found." {:available (set (list-entries book :module))
                                   :module module-id})))
...
(defn put-entry
  ...
  (let [_ (assert-compatible-lang book (:lang entry))
        _ (assert-module book (:module entry))
        {:keys [id module section]} entry]
    (atom/atom-put-fn book [:modules module section id] (std.lib.collection/filter-vals identity entry))))
...
(defn assert-entry
  ...
  (or (has-entry? book module-id section symbol-id)
      (and (assert-module book module-id)
           (std.lib.foundation/error "No entry found." {:available (set (keys (get (get-entry book module-id)
                                                                  section)))
                                       :section section
                                       :module module-id}))))
...
(defn module-create-filename
  ...
  (let [suffix   (or (get-in book [:file :suffix])
    ...
    (str (last (std.string.common/split (name module-id) #"\."))
         "." suffix)))
...
(defn module-create-check
  ...
  (let [missing (clojure.set/difference (set (vals link))
    ...
    (if (not-empty missing)
      (std.lib.foundation/error "Missing namespaces" {:ns missing
                                     :link link
                                     :all (keys modules)})
      true)))
...
(defn module-create-requires
  ...
  {:added "4.0"}
  [require]
  (std.lib.collection/map-juxt [first #(apply hash-map (cons :id %))] require))

(defn module-create
  "creates a module given book and options"
  {:added "4.0"}
  ([{:keys [lang] :as book} module-id options]
   (let [_ (or book      (std.lib.foundation/error "Book is required." {:lang lang
                                                       :module module-id}))
         _ (or module-id (std.lib.foundation/error "Module id is required." {:lang lang
                                                            :book book
                                                            :module module-id}))
         {:keys [require
                 require-impl
         ...
         internal (-> (std.lib.collection/map-entries (fn [[id {:keys [as]}]] [id (or as id)])
                                     requires)
                      (assoc module-id '-))
         link     (-> (std.lib.collection/map-entries (fn [[id {:keys [as]}]]
                                       [(if (vector? as)
                                          (last as)
                                          (or as id))
                                        id])
                                     requires)
                      (assoc '- module-id))
         internal   (std.lib.collection/transpose link)
         native     (std.lib.collection/map-juxt [first (fn [[_ & args]]
                                         (let [{:keys [bundle]
                                                :as opts} (apply hash-map args)
                                               bundle (if bundle
                                                        (std.lib.collection/map-juxt [first #(apply hash-map (rest %))]
                                                                    bundle))]
                                           (cond-> opts
         ...
         native-lu  (->> native
                         (mapcat (fn [[import {:keys [as]}]]
                                   (->> (disj (set (flatten (seq (std.lib.collection/seqify as)))) '*)
                                        (filter symbol?)
                                        (map (fn [sym] [sym import])))))
                         (into {}))
         alias    (merge (std.lib.collection/map-juxt [#(nth % 2) first] alias)
                         (->> (vals native)
                              (map :as)
                              (keep (fn [x] (if (vector? x) (last x) x)))
                              (std.lib.collection/map-juxt [identity identity])))
         includes   (->> requires
