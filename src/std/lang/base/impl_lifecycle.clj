    ...
    (case type
      ...
      (let [entry (get links link-ns)
        ...
        {:ns (->> [root-prefix rel label]
                  ...
                  (std.string.common/join path-separator))
         :suffix suffix
         :as link-as}))))
...
(comment
  ...
  #_(when (and (= :module (:layout mopts))
               ...
               (not (-> emit :export :suppress)))
      (when-not (:as export)
        (std.lib.env/prn   "Missing export `:as` field" {:input export
                                               :module module-id})
        (std.lib.foundation/error "Missing export `:as` field" {:input export
                                               :module module-id}))
      (when-not (:entry export)
        (std.lib.env/prn   "Missing export `:entry` field" {:input export
                                                  :module module-id})
        (std.lib.foundation/error "Missing export `:entry` field" {:input export
                                                  :module module-id}))
