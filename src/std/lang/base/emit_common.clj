     ...
     (if (prose/multi-line? output)
       (let [indent-fn (if full-body
         ...
         (str prefix " "
              (indent-fn output
                         0
                         {:custom (str (prose/spaces (+ (or prev 0)
                                                      (or indent 0)))
                                       prefix
                                       " ")})))
       (str prefix " " output)))))
...
(defn emit-squash
  ...
  (clojure.string/join (emit-array more grammar mopts)))
...
(defn emit-infix
  ...
  ([raw args grammar mopts]
   ...
   (let [{:keys [start end space]} (get-in grammar [:default :common])]
     (clojure.string/join (str space raw space)
...
(defn emit-infix-if-single
  ...
  ([[_ check then else] grammar mopts]
   (let [{:keys [start end space]} (get-in grammar [:default :common])
     ...
     (f/-> (emit-array [check then else] grammar mopts emit-wrapping)
...
(defn emit-between
  ...
  {:added "3.0"}
  ([raw args grammar mopts]
   (clojure.string/join raw (emit-array args grammar mopts emit-wrapping))))
...
(defn emit-assign
  ...
  ([raw args grammar mopts]
   ...
   (let [{:keys [space assign]} (get-in grammar [:default :common])]
     (clojure.string/join (str space (or raw assign) space)
               (emit-array args grammar mopts emit-wrapping)))))
...
(defn emit-return-base
  ...
  ([raw args grammar mopts]
   (let [{:keys [space sep]} (get-in grammar [:default :common])
         ...
         out (->> (emit-array args grammar mopts emit-value)
                   (clojure.string/join (str sep space)))]
     (cond (and (collection/form? (first args))
...
(defn emit-with-global
  ...
  (if-let [transform (get-in grammar [:token :symbol :global])]
    (emit-value (transform token grammar mopts) grammar mopts)
   ...
   (let [[type ns] (emit-symbol-classify sym mopts)
         ...
         link-symbol (fn [sym]
                       (cond (= layout :flat)   (name sym)
                             ...
                             #_#_
                             :else (f/error "Not Implemented." {:input sym
                                                               :layout layout
                                                                :classify [type ns]})))
...
(defn emit-with-rand
  ...
  (let [token (case type
                ...
                (rand))]
    (emit-value token grammar mopts)))
...
(defn invoke-kw-parse
  ...
  {:added "3.0"}
  ([args]
   (let [idx (collection/index-at keyword? args)]
     (if (neg? idx)
       [args []]
...
(defn emit-invoke-layout
  ...
  ([str-array grammar mopts]
   (let [{:keys [sep space]} (helper/get-options grammar [:default :invoke])]
     (cond (or (some prose/multi-line? str-array)
               (and (not *multiline*)
                    (< (apply + (map count str-array)) *max-len*)))
           (-> (clojure.string/join (str sep space) str-array)
               (wrapped-str [:default :invoke] grammar))
           
           :else
           (f/-> (clojure.string/join (str sep "\n" (prose/spaces 2)
                                )
                           str-array)
                 (str "\n" (prose/spaces 2) % "\n")
                 (wrapped-str [:default :invoke] grammar)
                 (prose/indent-rest *indent*))))))
   ...
   (cond (symbol? v)
         ...
         (let [sym  (first v)
           ...
           (emit-invoke-raw (str apply
                                  ...
                                  (if (not-empty braces)
                                    (emit-value braces grammar mopts)
                                    ""))
  ...
  ([key [sym & args :as form] {:keys [reserved] :as grammar} mopts & [expansion]]
   (let [{:keys [op emit transform raw default type] :as props} (get reserved sym)]
     (case emit
       ...
       ;; RESTRICTED SYMBOLS
       :throw         (f/error "Not allowed as op" {:symbol sym :form form})

       ;; HANDLE ERROR
       (if-let [f (get expansion emit)]
         (f key props form grammar mopts)
         (f/error "No matching clause"
                  {:emit emit
                   :key key
                   :symbol sym
                   :form form
   ...
   (let [key (helper/form-key-base form)
         ...
         [key type check?]  (cond (vector? key) key
                                 ...
                                 (check-fn (first form)
                                           [:invoke :invoke]))
         _ (if (get banned key)
             (f/error "Disallowed in form" {:form form :key key :type type}))]
     [key type check?])))
