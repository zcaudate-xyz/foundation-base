(ns std.lang.base.emit-top-level
  (:require [std.lang.base.emit-assign :as assign]
...
(defn emit-declare
  ...
  ([key [tag & syms] grammar mopts]
   (let [{:keys [raw space sep]}  (helper/get-options grammar [:define key])
         pre (str (or raw tag))]
     (str pre space (std.string.common/join sep (common/emit-array (vec syms) grammar mopts))))))
   ...
   (let [module-id (or (-> mopts :entry :module)
         ...
         sym (if (and
                  ...
                  (or (#{:full :host} (:layout mopts))
                      (= key :defglobal)))
               (do (or module-id
                       (std.lib.foundation/error "Module not found"
       ...
       (cond (or (fn? emit)
             ...
             (case type
               ...
               :hard-link  (common/*emit-fn* (cons (:raw props) (rest form))
                                             grammar mopts)
               (std.lib.foundation/error "Missing key" {:key key
                                       :symbol sym
                                       :props props
                                       :entry (get reserved sym)})))
       (catch Throwable t
         (if common/*explode*
           (std.lib.env/prn :EMIT-ERROR form t))
         (throw t))))))
