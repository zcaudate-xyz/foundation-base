...
(defn bash-map
  ...
  (let [arr (reduce-kv (fn [acc k v]
                        ...
                        []
                        form)]
    (std.string.common/join " " (common/emit-array arr grammar mopts))))
...
(defn bash-invoke
  ...
  (let [args (map (fn [x]
                    (if (and (std.lib.collection/form? x)
                             (not (get-in grammar [:reserved (first x)])))
                      (list '$ x)
                      x))
                  args)]
    (std.string.common/join " " (common/emit-array (cons f args) grammar mopts))))
...
(def +template+
  (->> {:banned #{:regex}
        ...
        :token   {:nil       {:as "''"}
                  ...
                  :keyword   {:custom #'std.lib.foundation/strn}}
