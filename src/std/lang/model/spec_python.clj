       ...
       `(\\ ~(apply list
                    \\ (mapcat (fn [d]
                                 [\\ (list :%
                                           (list :- "@")
                                           (if (keyword? d)
                                             (list :- (std.lib.foundation/strn d))
                                             d))])
                               decorators))
         \\ ~body)))))
         ...
         (let [[args body] args]
           (apply list :- :lambda
                  (concat (if (not-empty args)
                            [(list 'quote args) ":"]
                            [":"])
                          [(if (and (std.lib.collection/form? body)
                                    (= 'return (first body)))
                             (second body)
                             body)]))))))
...
(defn tf-for-return
  ...
  [[_ [[res err] statement] {:keys [success error]}]]
   (std.lib.template/$ (try (var ~res ~statement)
             ~success
...
(def +meta+
  (book/book-meta
   {:module-current (fn []
                      (std.lib.template/$ (list (b:& (set [(str x) :for x :in (locals)])
                                      (set [(str m) :for m :in sys.modules])))))
    :module-export  (fn [{:keys [as refer]} opts])
    :module-import  (fn [name {:keys [as refer]} opts]  
                      (if as
                        (std.lib.template/$ (:- :import ~name :as ~as))
                        (std.lib.template/$ (:- :import ~name))))
    :module-unload  (fn [name as]
                      (std.lib.template/$ (do (del (. sys.modules [~name]))
                               (del ~(symbol name)))))}))
