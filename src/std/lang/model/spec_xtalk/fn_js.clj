...
(defn js-tf-x-type-native
  [[_ obj]]
  (std.lib.template/$ (do (when (== ~obj nil)
             (return nil))
           (var t := (typeof ~obj))
           (if (== t "object")
...
(defn js-tf-x-lu-get
  ...
  {:added "4.0"}
  ([[_ lu obj]]
   (std.lib.template/$ (. ~lu (get ~obj)))))

(defn js-tf-x-lu-set
  "converts map to array"
  {:added "4.0"}
  ([[_ lu obj gid]]
   (std.lib.template/$ (. ~lu (set ~obj ~gid)))))

(defn js-tf-x-lu-del
  "converts map to array"
  {:added "4.0"}
  ([[_ lu obj]]
   (std.lib.template/$ (. ~lu (delete ~obj)))))
...
(defn js-tf-x-iter-eq
  ([[_ it0 it1 eq-fn]]
   (std.lib.template/$ (do (for [:let x0 :of ~it0]
              (var r1 (. ~it1 (next)))
              (cond (. r1 done)
                    (return false)
...
(defn js-tf-x-cache
  ([[_ name]]
   (if (= (std.lib.foundation/strn name) "GLOBAL")
     'window.localStorage
...
(defn js-tf-x-cache-incr
  ([[_ cache key num]]
   (std.lib.template/$ (do:> (var prev (Number (. ~cache (getItem ~key))))
              (var curr (+ prev ~num))
...
(defn js-tf-x-thread-spawn
  ([[_ thunk]]
   (std.lib.template/$ (new Promise (fn [resolve reject]
                       (resolve (~thunk)))))))
...
(defn js-tf-x-with-delay
  ([[_ thunk ms]]
   (std.lib.template/$ (setTimeout (fn []
...
(defn js-tf-x-start-interval
  ([[_ thunk ms]]
   (std.lib.template/$ (setInterval (fn []
                      (new Promise (fn [resolve reject]
                                     (resolve (~thunk)))))
                    ~ms))))
...
(defn js-tf-x-stop-interval
  ([[_ instance]]
   (std.lib.template/$ (clearInterval instance))))
