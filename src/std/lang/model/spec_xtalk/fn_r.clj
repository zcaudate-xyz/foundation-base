...
(defn r-tf-x-type-native
  [[_ obj]]
  (std.lib.template/$ (do (var t := (typeof ~obj))
           (cond (== t "list")
...
(defn r-tf-x-is-object?
  [[_ e]]
  (std.lib.template/$ (and (== "list" (typeof ~e))
            (not (is.null (names ~e))))))
...
(defn r-tf-x-lu-get
  ...
  {:added "4.0"}
  ([[_ lu obj]]
   (std.lib.template/$ (. ~lu [(tracemem ~obj)]))))

(defn r-tf-x-lu-set
  "converts map to array"
  {:added "4.0"}
  ([[_ lu obj gid]]
   (std.lib.template/$ (:= (. ~lu [(tracemem ~obj)]) ~gid))))
...
(defn r-tf-x-arr-push
  [[_ arr item]]
  (std.lib.template/$ (:= (. ~arr [(+ (length ~arr) 1)])
            ~item)))
