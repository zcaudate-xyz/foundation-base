...
(defn lua-tf-x-err
  [[_ s & [data]]]
  (if data
    (std.lib.template/$ (error (cjson.encode ~[s data])))
    (std.lib.template/$ (error ~s))))

(defn lua-tf-x-shell
  ([[_ s cm]]
   (std.lib.template/$ (do* (var handle (io.popen ~s))
...
(defn lua-tf-x-hash-id
  [[_ obj]]
  (std.lib.template/$ (return nil)))

(defn lua-tf-x-type-native
  [[_ obj]]
  (std.lib.template/$ (do (local t := (type ~obj))
           (if (== t "table")
             (if (== nil (. '(~obj) [1]))
               (return "object")
               (return "array"))
...
(defn lua-tf-x-is-object?
  [[_ e]]
  (std.lib.template/$ (or (and (== "table" (type ~e))
                (== nil (. '(~e) [1])))
           (== "object" (type ~e)))))
...
(defn lua-tf-x-lu-get
  ([[_ lu obj]]
   (std.lib.template/$ (. ~lu [(tostring ~obj)]))))

(defn lua-tf-x-lu-set
  ([[_ lu obj gid]]
   (std.lib.template/$ (:= (. ~lu [(tostring ~obj)]) ~gid))))

(defn lua-tf-x-lu-del
  ([[_ lu obj]]
   (std.lib.template/$ (:= (. ~lu [(tostring ~obj)]) nil))))
...
(defn lua-tf-x-return-encode
  ([[_ out id key]]
   (std.lib.template/$ (do (do (local ret nil)
                (local '[r-ok r-err]
                       (pcall (fn []
                                (cond (== nil ~out)
...
(defn lua-tf-x-socket-connect
  ([[_ host port opts]]
   (std.lib.template/$ (do* (local '[conn err])
...
(defn lua-tf-x-iter-from-obj
  ([[_ obj]]
   (std.lib.template/$ (coroutine.wrap
...
(defn lua-tf-x-iter-has?
  ([[_ obj]]
   (std.lib.template/$ (and (== "table" (type ~obj))
