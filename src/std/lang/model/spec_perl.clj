...
(defn perl-var
  ...
  (let [val (last args)
        sym-str (if (keyword? sym)
                  (std.lib.foundation/strn sym)
                  (str "$" (std.lib.foundation/strn sym)))]
...
(defn perl-symbol
  ...
  [sym grammar mopts]
  (let [s0 (name sym)
        s  (if (or (std.string.common/includes? s0 "::")
                   (std.string.common/includes? s0 "->"))
             s0
             (std.string.common/replace s0 "-" "_"))]
    (cond (:perl/func mopts)
          s

          (or (std.string.common/starts-with? s "$")
...
(defn perl-invoke-args
  [args grammar mopts]
  (std.string.common/join ", " (common/emit-invoke-args args grammar mopts)))
...
(defn perl-array
  ...
  [arr grammar mopts]
  (str "[" (std.string.common/join ", " (common/emit-array arr grammar mopts)) "]"))
...
(defn perl-map
  ...
  (let [entries (map (fn [[k v]]
                       (str (if (keyword? k)
                              (common/*emit-fn* (std.lib.foundation/strn k) grammar mopts)
