  ...
  (let [[_ type & args] arr]
     (cond (= 'slice type)
           ...
           (str (emit/emit-main type grammar mopts)
                "[" (std.string.common/join ", " (map #(emit/emit-main % grammar mopts) args)) "]"))))
  ...
  ([arr grammar mopts]
   (let [sym (first arr)]
     (cond (= :> sym)
           ...
           :else
           (str "[]interface{}{"
                (std.string.common/join ", " (map #(emit/emit-main % grammar mopts) arr))
                "}")))))
       ...
       (std.string.common/join "\n"
                 (map (fn [field]
                        (if (vector? field)
                          (let [[n t & tags] field
                                tag-str (if (seq tags)
                                          (str " `" (std.string.common/join " " tags) "`")
                                          "")]
