...
(defn to-erlang-var [sym]
  (if (symbol? sym)
    (let [s (name sym)]
      (if (re-find #"^[a-z]" s)
        (symbol (str (std.string.common/upper-case (subs s 0 1)) (subs s 1)))
        sym))
    sym))
...
(defn emit-erlang-defn
  ...
  (wrap-raw
   (str (ut/sym-default-str sym)
        "("
        (std.string.common/join ", " (map emit-ast params))
        ") -> "
        (std.string.common/join ", " (map emit-ast body))
        ".")))
...
(defn emit-erlang-tuple
  ...
  [[_ & elements]]
  (wrap-raw
   (str "{" (std.string.common/join ", " (map emit-ast elements)) "}")))
