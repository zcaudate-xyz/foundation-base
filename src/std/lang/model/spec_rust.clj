...
(defn rst-typesystem
  ...
  (let [{:keys [sep] :as opts} (helper/get-options grammar [:data :vector])
        [sym & more] (rest arr)]
    (str sym "<" (std.string.common/join ","
                           (map (fn [input]
                                  (cond (string? input)
                                        input
...
(defn rst-deftrait
  ...
  {:added "4.0"}
  [[_ sym & body]]
  (let [body (std.lib.walk/postwalk (fn [x]
                           (if (and (list? x)
                                    (= 'fn (first x))
                                    (= 3 (count x)))
...
(defn rst-new
  ...
  (let [items (map (fn [[k v]]
                     (str (std.lib.foundation/strn k) ": " (emit/emit-main v grammar mopts)))
                   (partition 2 args))]
    (str (emit/emit-main sym grammar mopts)
         "{" (std.string.common/join ", " items) "}")))
       ...
       (cond (empty? items)
             ...
             (let [strs (mapcat (fn [args]
                                  (fn/emit-fn-preamble-args
                                   ...
                                   grammar mopts))
                                items)]
               (str "struct " sym "{" (std.string.common/join ", " strs) "}")))))
