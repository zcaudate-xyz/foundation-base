...
(defn tf-local
  ...
  (if (empty? args)
    (list 'var* :local decl)
    (let [bound (last args)]
      (cond (and (std.lib.collection/form? bound)
                 (= 'fn (first bound)))
...
(defn lua-map-key
  ...
  {:added "3.0"}
  ([key grammar mopts]
   (cond (not (or (std.lib.collection/form? key)
                  (symbol? key)
                  (number? key)))
...
(defn tf-for-return
  ...
  (std.lib.template/$ (do (var '[~res ~err] ~statement)
           (if (not ~err)
             ~success
             ~error))))
...
(defn tf-for-try
  ...
  {:added "4.0"}
  [[_ [[res err] statement] {:keys [success error]}]]
  (std.lib.template/$ (do (var '[ok out] (pcall (fn []
                                   (return ~statement))))
           (if ok
             (do* (var ~res := out)
                  ~@(if success [success]))
...
(defn tf-for-async
  ...
  (std.lib.template/$ (ngx.thread.spawn
        (fn []
...
(defn lua-module-link
  ...
  ([ns graph]
   (let [{:keys [target root-ns]} graph
         root-path (->> (std.string.common/split (name root-ns) #"\.")
                        (butlast)
                        (std.string.common/join "/"))
         
         ns-path   (std.string.common/replace (name ns) #"\." "/")]
     (if (std.string.common/starts-with? ns-path (str root-path))
       (std.lib.foundation/->> (fs/relativize root-path ns-path)
              (str)
              (str "./"))
       (str "./" ns-path)))))
