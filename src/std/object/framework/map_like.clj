   ...
   `[(defmethod protocol.object/-meta-read ~cls
       ([~'_]
        ~(let [custom (:custom read)
               ...
               read (if custom
                      `(update-in ~read [:methods] collection/merge-nested ~custom)
                      read)]
           read)))
           ...
           ([~'_]
            ~(let [write (extend-map-write cls opts)
                   ...
                   write (if-let [custom (:custom write)]
                           `(update-in ~(dissoc write :custom) [:methods] collection/merge-nested ~custom)
                           write)]
               write))))

     (do (memoize/memoize:remove read/meta-read-exact ~cls)
         (memoize/memoize:remove write/meta-write-exact ~cls)
