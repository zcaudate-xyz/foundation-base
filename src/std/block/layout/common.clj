...
(defn layout-multiline-hiccup
  ...
  [form {:keys [spec]
         :as opts}]
  (let [spec  (merge {:col-from (if (vector? (second form))
                                  0
                                  1)
                      :col-start 1}
                     spec)
...
(defn layout-with-bindings
  ...
  ([form {:keys [indents]
          :or {indents 0}
          :as opts}]
   (let [[start-sym nindents] (layout-multiline-call-setup form
                                                           (assoc opts :indents (inc indents)))
