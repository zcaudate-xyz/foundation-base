...
(defn group-blocks-prep
  ...
  (let [delimiters  (parse/parse-delimiters content)
        lines       (std.string.common/split-lines content)
...
(defn get-block-lines
  ...
  #_(std.lib.env/prn {:start start
            :end end
            :start-col start-col
            :end-col end-col})
  (cond (= start end)
        (let [line (str (std.string.prose/spaces (dec start-col))
                        (-> (get lines (dec start))
                            (subs (dec start-col))))]
          (if end-col
            [(subs line 0 end-col)]
        ...
        (let [start-line  (-> (get lines (dec start))
                              (subs (dec start-col)))
                               ...
                               (catch Throwable t
                                 (std.lib.env/prn {:count (count lines)
                                         :end end
                                         :start start
                                         :start-col start-col
                                         :end-col end-col})
                                 (throw t)))]
          (vec (concat [(str (std.string.prose/spaces (dec start-col))
                             start-line)]
                       (subvec lines
                               start
                               (dec end))
...
(defn check-errored-suspect
  ...
  (let [{:keys [line col]
        ...
        args   [[row-offset
                ...
                (dec col)]        
        #_#_#_#_#_#_
        _ (std.lib.env/prn args)
        _ (std.lib.env/prn (dissoc block :children))
        _ (std.lib.env/prn error)
        snippet       (std.string.prose/join-lines
  ...
  (let [{:keys [children]} block
        ...
        block-error      (if (seq all-errors)
                           {:errors (mapv #(update % :line + (dec (first (:line block)))) all-errors)
                            :lines  (std.string.common/split-lines snippet)
                            :at     (dissoc block :children)})
        #_#_
        _                (std.lib.env/pl snippet)
        #_#_
        _                (std.lib.env/prf {:is-suspect is-suspect
                                 :child child-error
...
(defn wrap-print-diff
  ...
  (fn [content]
    (let [new-content (print-fn content)
          ...
          _ (std.lib.env/p (diff/->string deltas))]
      new-content)))
