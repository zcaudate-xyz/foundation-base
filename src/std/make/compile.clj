  ...
  ([{:keys [main root] :as opts}]
   (mapv (fn [[input file]]
           (let [out-path (compile-out-path (assoc opts :file file))
                 in-path  (or (std.lib.env/sys:resource input)
                              (std.lib.foundation/error "Resource not found:" {:value input}))]
             (if *mock-compile*
               [in-path out-path]
...
(defn compile-resolve
  ...
  (loop [x x]
    (cond (or (var? x)
              (std.lib.context.pointer/pointer? x))
          (recur @x)
...
(defn compile-ext
  ...
  ([{:keys [header footer main format root target name file wrapper] :as sopts
   ...
   (let [{:keys [suffix lang] :as m} (compile-ext-fn format)
         ...
         f      (:fn m)
         f      (if (symbol? f)
                  (do (std.lib.env/require (symbol (namespace f)))
                      @(resolve f))
                  f)
         body   (f main)
         full   ((wrapper compile-fullbody sopts) body sopts)
         file   (or file
                    (if name (str name "." (std.lib.foundation/strn (or suffix format))))
                    (:file m)
                    (std.lib.foundation/error (str "Need a :file or :name entry for "
...
(defn compile-directive
  ...
  (let [glob-fn (fn [patterns sopts]
                  (some (fn match-fn [pattern]
                          (cond (or (std.lib.foundation/regexp? pattern)
                                    (string? pattern))
                                ...
                                (every? (fn [[k comp]]
                                          ...
                                          (let [v (get sopts k)]
                                            (std.lib.foundation/suppress (cond (std.lib.foundation/regexp? comp) (re-find comp v)
                                                              (or (fn? comp)
                                                                  (var? comp)
                                                                  (set? comp)) (comp v)
                                                              :else (= comp v)))))
                                ...
                                :else (std.lib.foundation/error "Not valid" {:pattern pattern})))
                        patterns))]
  ...
  (let [{:keys [sections hooks default] :as m}  @instance
        ...
        _ (when (:post hooks)
            (doseq [hook (:post hooks)]
              (try ((:fn hook) m)
                   (catch Throwable t
                     (std.lib.env/p "Hook " (:id hook) " failed to run")))))]
    output))
