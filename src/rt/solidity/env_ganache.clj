...
(defn start-ganache-server
  ...
  []
  (or @*server*
      (when (not (std.lib.foundation/suppress
                  (std.lib.network/wait-for-port "127.0.0.1" +default-port+
                                   {:timeout 1000})))
        (let [_    (fs/create-directory +default-dir+)
          ...
          (-> (if (not @*server*)
                (swap! *server*
                       (fn [m]
                         (let [process (std.lib.os/sh {:args ["/bin/bash" "-c"
                                                     "ganache --wallet.seed 'test' --host '0.0.0.0'"]
                                              :wait false
                                              :root +default-dir+})
                               thread  (-> (std.lib.future/future
                                             (std.lib.os/sh-wait process))
                                           (std.lib.future/on:complete
                                            (fn [_ _]
                                              (try (let [out (std.lib.os/sh-output process)]
                                                   ...
                                                   (catch Throwable t))
                                              (reset! *server* nil))))]
                           (std.lib.network/wait-for-port (network/local-ip) +default-port+
                                            {:timeout 10000})
                           {:type "ganache"
                            :port +default-port+
                            :root +default-port+
