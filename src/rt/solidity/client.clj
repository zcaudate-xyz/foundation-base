...
(defn check-node-connection
  ...
  (or (server/get-relay
      ...
      (std.lib.foundation/error "Not Connected")))
...
(defn contract-fn-name
  ...
  (let [fn-name  (str (or (:id ptr)
                          (std.lib.foundation/error "Free pointers not supported")))]
...
(defn create-web3-node
  ...
  (let [rt-node (basic/rt-basic
                 {:id (std.lib.foundation/sid)
                  :lang :js
                  :runtime :basic
                  :layout :full})
        url     (common/get-url rt)
        form  (std.lib.template/$ [(:= solc (require "solc"))])
...
(defn stop-web3
  ...
  (let [_ (std.lib.component/stop (:node rt))
...
(defn raw-eval-web3
  ...
  ([{:keys [id lang] :as rt} body]
   (std.lib.foundation/error "NOT AVAILABLE" {})))
...
(defn invoke-ptr-web3-check
  ...
  (let [spec  (first (filter #(-> (get % "name")
        ...
        retype (or (get-in spec ["outputs" 0 "type"])
                   "")]
    (when (not spec)
      (std.lib.foundation/error "No spec found: " {:name fn-name
                                  :options (map #(get % "name") (:abi contract))}))
    (when (not= (count args)
                (count types))
      (std.lib.foundation/error "Args not the same: " {:args args
                                      :types types}))
    (and (std.string.common/starts-with? retype "uint")
         (not (std.string.common/ends-with? retype "[]")))))
    ...
    (let [prefix (if (empty? interfaces)
          ...
          form   (list `web3/contract-test-prep
                       ...
                       (list `xt.lang.base-lib/join
                             "\n"
                             (std.string.common/split-lines code))
