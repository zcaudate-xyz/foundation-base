...
(defmacro.pg
  ...
  {:added "4.0"}
  [m field]
  (std.lib.template/$ (coalesce (:->> ~m ~(str field "_id"))
                 (:->> (:-> ~m ~field) "id"))))
...
(defmacro.pg ^{:- [:jsonb]}
  ...
  {:added "4.0"}
  ([f arr & args]
   (std.lib.template/$ (% [(coalesce (jsonb-agg (~f o-ret ~@args))
                      (js []))
...
(defmacro.pg ^{:- [:block]}
  ...
  {:added "4.0"}
  [enum]
  (std.lib.template/$ [:select p :from (unnest (enum-range (++ nil ~enum))) :as p :order-by (random) :limit 1]))
