...
(defn t-update-symbol
  ...
  ([tsch sym {:keys [columns
   ...
   (let [trkm (tracker/tracker-map-modify params)
         ...
         _  (if err (std.lib.foundation/error "Not Valid." (assoc err :data all-columns)))
         ...
         body (mapv (fn [k]
                      (let [trkv    (get trkm k)
                            ...
                            out (cond trkv
                                    ...
                                    (and (std.lib.collection/form? (first inputs))
...
(defn t-update-map
  ...
  ([tsch m params mopts]
   (let [m  (merge (tracker/tracker-map-modify params) m)
         ...
         _  (if err (std.lib.foundation/error "Not Valid." (assoc err :data m)))
         entry-fn  (fn [_ k] (get m k))
         m       (std.lib.collection/map-entries (fn [[k v]]
                                  [k (base/t-val-fn tsch k v params mopts)])
                                m)
         m       (std.lib.collection/filter-keys #(not (-> % tsch first :ignore)) m)
