  ...
  (let [book (snap/get-book snapshot :postgres)
        {:static/keys [dbtype]
         :as entry}  (book/get-base-entry book
                                          ...
                                          (:section link))]
    (cond (not entry)
          (std.lib.foundation/error "Entry not found." {:input link})

          (not= dbtype type)
          (std.lib.foundation/error "Entry type not correct." {:type type
                                              :input entry})
  ...
  ([sym module {:keys [ns] :as ref}]
   (if (and (= "-" (namespace ns))
     ...
     [(select-keys @(or (resolve ns)
                        (std.lib.foundation/error "Not found" {:input ref}))
...
(defn pg-deftype-hydrate-process-sql
  ...
  ([sql k attrs]
   (if (:process sql)
     (assoc sql :process
            (std.lib.walk/prewalk
             (fn [x]
               (if (symbol? x)
                 (std.lib.foundation/var-sym (or (resolve x)
                                (std.lib.foundation/error "Cannot resolve symbol"
                                         {:symbol x
                                          :col k
                                          :attrs attrs})))
                 x))
...
(defn pg-deftype-hydrate-process-foreign
  ...
  ([foreign resolve-link-fn snapshot]
   (std.lib.collection/map-vals (fn [f-spec]
                 (if (:ns f-spec)
...
(defn pg-deftype-hydrate-process-enum
  ...
  ([k attrs snapshot]
   (let [enum-var  (or (resolve (-> attrs :enum :ns))
                       (std.lib.foundation/error "Not found" {:input (:enum attrs)}))
   ...
   (let [resolve-link-fn (partial pg-deftype-hydrate-link sym module)
         ...
         hmeta   (assoc (common/pg-hydrate-module-static module)
                        ...
                        :static/schema-primary (cond (empty? @capture)
                                                     (std.lib.foundation/error "Primary not available")
