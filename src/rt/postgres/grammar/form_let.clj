...
(defn pg-tf-let-assign
  ...
  (let [data-fn (fn [e]
    ...
    (cond (= '_ e)    [v]
          (symbol? e) (data-fn e)
          (std.lib.collection/form? e) (data-fn (last e))
          (set? e)    (mapv (fn [ei]
                              (cond (symbol? ei)
                                    `(:= ~ei (:-> ~v ~(tf/pg-js-idx ei)))
                                    
                                    (std.lib.collection/form? ei)
                                    `(:= ~(last ei) (~@(butlast ei)
                                                     (:->> ~v ~(tf/pg-js-idx (last ei)))))
                                    
                                    :else (std.lib.foundation/error "Not Allowed" {:value ei})))
                            e)
          (vector? e)  (vec
                        (map-indexed (fn [i ei]
                                       (cond (symbol? ei)
                                             ...
                                             :else (std.lib.foundation/error "Not Allowed" {:value ei})))
                                     e))
          :else (std.lib.foundation/error "Not Allowed" {:value e}))))
  ...
  ([dsyms body]
   (let [not-checked (volatile! #{})
         ...
         collect-syms (fn [form]
                        (std.lib.walk/walk:find
                         (fn [x]
                           (and (symbol? x)
                                (nil? (namespace x))))
                         form))
         check-fn (fn check-fn [csyms x]
                    (cond (std.lib.collection/form? x)
                          (let [f (first x)]
                            (cond (= 'let f)
     ...
     (if (not-empty @not-checked)
       (std.lib.foundation/error "Unknown symbols in form" {:symbols @not-checked
                                           :dsyms dsyms})))))
  ...
  ([[_ bindings & body]]
   (let [pairs (map vec (partition 2 bindings))
         ...
         [declu dsyms] (loop [dsyms '#{_}
                         ...
                         (if (empty? forms)
                           [out dsyms]
                           (let [csyms (std.lib.walk/walk:find
                                        (fn [x]
                                          (and (symbol? x)
                                               (nil? (namespace x))))
                                        form)]
         ...
         decl  (mapcat (fn dec-fn [e]
                         (cond (= '_ e)    []
                               ...
                               (std.lib.collection/form? e) [(if (= '++ (first e))
