...
(invoke/definvoke collect-module-globals
  ...
  ([module]
   (let [{:keys [native static]} module]
     (disj (clojure.set/union
            (set (mapcat #(mapcat collection/seqify (vals %))
                         (vals native)))
         ...
         sym-fn (fn [form]
                  (let [[ftag] form
                    ...
                    (cond (#{'var 'const 'fn:> 'fn 'local} ftag)
                          ...
                          (or (clojure.string/starts-with? (str ftag) "for:")
                              (= (str ftag) "forange"))
               ...
               (fn [form]
                 (cond (collection/form? form)
                       ...
                       (cond (or (= '_ form)
                                 ...
                                 (reserved form)
                                 (namespace form)
                                 (clojure.string/starts-with? (str form)
                                                   "x:")
                                 (clojure.string/starts-with? (str form)
                                                   "..."))
                             form
                             ...
                             :else
                             (let [s  (first (clojure.string/split (str form)
                                                        #"\."))
                                   s  (if s (symbol s))]
                               (cond (or (nil? s)
                                         (globals s)
           ...
           (let [{:keys [vars syms]} (collect-sym-vars entry module lang-globals)
             ...
             (when (not-empty unused)
               (when (= :print (:unused options))
                 (env/p (str "UNUSED VAR @ " (:module entry) "/" id)
                      {:unused unused})))
             (when (not-empty unknown)
               (when (= :print (:unknown options))
                 (env/p (str "UNKNOWN VARIABLES @ " (:module entry) "/" id)
                      {:unknown unknown}))
               (when (= :error (:unknown options))
                 (f/error (str "UNKNOWN VARIABLES @ " (:module entry) "/" id)
                          {:unknown unknown
                           :form (or (:form entry)
                                     (:form-input entry))})))
             :pass)))))
