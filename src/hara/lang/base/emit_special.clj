(ns hara.lang.base.emit-special
...
(defn emit-with-module-all-ids
  ...
  (let [book   (book/book-from snapshot lang)
        ...
        _   (if (not export-sym)
              (f/error "No Export for Module" {:module module-id
...
(defn emit-with-deref
  ...
  (if (= 'var (first form))
    ...
    (f/error "Needs to be a clojure.lang.Var")))
...
(defn emit-with-lang
  ...
  (let [[_ {:keys [lang module] :as opts
            :or {module (env/ns-sym)}}
         body] form
        book   (or (book/book-from snapshot lang)
                   (f/error "Lang not found." {:lang lang
