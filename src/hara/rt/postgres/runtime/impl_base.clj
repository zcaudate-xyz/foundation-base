(ns hara.rt.postgres.runtime.impl-base
  (:require [hara.rt.postgres.base.grammar.common :as common]
            [hara.rt.postgres.base.grammar.form-deftype-hydrate :as form-deftype-hydrate]
            [hara.rt.postgres.base.application :as app]
            [hara.lang :as l]
            [hara.lang.base.emit-common :as emit-common]
            [hara.lang.base.emit-data :as emit-data]
   ...
   (let [cols    (keys m)
         [_ err] (if valid
                   (schema/check-valid-columns
                    tsch
                    cols))
         _ (and err  (f/error "Invalid keys." (assoc err :data m)))
         [_ err]  (if missing
                    (schema/check-missing-columns
                     ...
                     (fn [{:keys [primary required sql ignore]}]
                       (and (not ignore)
                            (or required primary)
                            (nil? (:default sql))))))
         _ (and err  (f/error "Missing keys." (assoc err :data m)))
         err (if format
               (schema/check-fn-columns tsch m))
         _ (if (not-empty err)
             (f/error "Column errors." err))]
     m)))
...
(defn t-build-js-map
  ...
  ([tsch input {:keys [defaults]
   ...
   (let [m    (merge (if defaults
                     ...
                     (collection/map-entries (fn [[k [v attrs]]]
...
(defn t-returning-cols
  ...
  ([tsch returning key-fn]
   (let [map-fn (fn [{:keys [expr as]}]
         ...
         _        (if err
                    (f/error "Not valid." (assoc err :data map-ks)))
  ...
  ([tsch returning key-fn]
   (let [key-fn (or key-fn
     ...
     (cond (or (#{'*} returning)
           ...
           :else
           (f/error "Not valid" {:value returning})))))
   ...
   (let [val-fn   (fn [k v op]
                    (let [[{:keys [type ref enum] :as attr}] (get tsch k)]
                      (cond (nil? v)
                            ...
                            (and (= :enum type)
                                 (not (and (collection/form? v)
                                           (= (first v) '++))))
                            [k [op (list '++ v (:ns enum))]]

                            (or (map? v) (ut/hashvec? v))
                            (let [_  (if (not= type :ref) (f/error "Not of type :ref" {:keys k
                                                                                       :attribute attr}))
                                  rtsch (get-in schema
                                                [:tree
                                                 (:ns ref)])]
         ...
         tf-fn   (fn [m]
                   (->> m
                        (collection/map-entries entry-fn)
                        (collection/map-keys (fn [k] (t-key-fn tsch k)))))]
     (cond (ut/hashvec? where)
           ...
           (let [[_ err]      (schema/check-valid-columns tsch (keys where))
                 _   (if (and err
                              (not *skip-checks*))
                       (f/error "Invalid columns." (assoc err :data where)))]
             (tf-fn where))
           
           :else (f/error "Not Allowed" {:value where})))))
   ...
   (let [entry-fn (fn [k]
                    (let [[{:keys [type ref] :as attr}] (get tsch k)]
                      (cond (= :ref type)
                            ...
                            :else
                            (f/error "Key is not a ref." {:key k :attr attr}))))]
     (vec
      (mapcat (fn [x]
                (cond (keyword? x)
                      [(entry-fn x)]
...
(defn t-wrap-order-by
  ...
  ([form cols tsch {:keys [newline]}]
   (let [[_ err] (if (vector? cols)
         ...
         _ (if err (f/error "Not valid." (assoc err :data cols)))]
     (cond-> form
  ...
  ([form lock {:keys [newline]}]
   (if-not lock
     ...
     (let [_ (if (not (vector? lock))
               (f/error "Lock must be a vector" {:input lock}))
           ...
           clause (or (modes mode)
                      (f/error "Invalid lock mode" {:input mode :allowed (keys modes)}))
