(ns hara.rt.basic
  (:require [hara.rt.basic.server-basic :as server-basic]
            [hara.rt.basic.server-websocket :as server]
            [hara.rt.basic.type-basic :as basic]
            [hara.rt.basic.type-container :as container]
            [hara.rt.basic.type-oneshot :as oneshot]
            [hara.rt.basic.type-remote-port :as remote-port]
            [hara.rt.basic.type-twostep :as twostep]
            [hara.rt.basic.type-websocket :as websocket]
            [std.concurrent :as cc]
            [std.lib.foundation :as f]))

(f/intern-in
 basic/rt-basic-port
 basic/rt-basic
 basic/rt-basic:create
 
 oneshot/rt-oneshot
 oneshot/rt-oneshot:create
 twostep/rt-twostep
 twostep/rt-twostep:create
 remote-port/rt-remote-port
 remote-port/rt-remote-port:create
 websocket/rt-websocket
 websocket/rt-websocket:create)

(defn clean-relay
  "cleans the relay on the server"
  {:added "4.0"}
  [rt]
  (let [record (server-basic/get-server (:id rt)
                                        (:lang rt))]
    (if-let [relay (server-basic/get-relay record)]
      (cc/send relay {:op :clean}))))
