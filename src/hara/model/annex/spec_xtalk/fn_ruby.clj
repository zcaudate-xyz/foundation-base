(ns hara.model.annex.spec-xtalk.fn-ruby
       ...
       (template/$
        (do (require "json")
            ...
            (if (== 1 (. ~encode-fn arity))
              (:= ~encoded (. ~encode-fn (call ~result)))
              (:= ~encoded (. ~encode-fn (call ~result nil nil))))
            (return ~encoded)))))))
