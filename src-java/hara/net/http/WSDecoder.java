...
public class WSDecoder {
    ...
    public Frame decode(ByteBuffer buffer) throws WSException {
            ...
            switch (state) {
                case FRAME_START:
                    ...
                    if (opcode != -1 && tmpOp != opcode) {
                        ...
                        throw new WSException("opcode mismatch: pre: " + opcode + ", now: "
                                + tmpOp);
                    }
                    opcode = tmpOp;
                ...
                case READ_LENGTH:
                    ...
                    if (!masked) {
                        throw new WSException("unmasked client to server frame");
                    }
                ...
                case READ_8_LENGTH:
                    if (isAvailable(buffer, 8)) {
                        ...
                        if (length < 65536) {
                            throw new WSException("invalid data frame length. most significant bit is not zero or length fits in unsigned short.");
                        }
                    ...
                    if (payloadRead == payloadLength) {
                        if (finalFlag) {
                            ...
                            switch (opcode) {
                                ...
                                default:
                                    throw new WSException("not impl for opcode: " + opcode);
                            }
                        } else {
    ...
    public void abortIfTooLarge(long length) throws WSException {
        if (length > maxSize) { // drop if message is too big
            throw new WSException("Max payload length 4m, get: " + length);
        }
    }
