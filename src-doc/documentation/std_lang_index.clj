(ns documentation.hara.lang-index)

[[:hero {:title "hara.lang"
         :subtitle "A language-oriented compiler for multi-runtime systems."
         :lead "`hara.lang` treats code generation as **authoring in data**, **emitting through grammars**, and **running against real runtimes**. The result is not just transpilation, but a reusable library model for building polyglot systems from a shared Lisp DSL."
...
[[:callout {:tone :info
            ...
            :content "Most tools explain code generation from the perspective of a compiler. This site explains `hara.lang` from the perspective of **systems work**: shared abstractions, target-specific emission, runtime integration, and a single maintenance workflow across languages."}]]

[[:card-grid {:title "What makes `hara.lang` different"
...
[[:quote {:text "The interesting part of `hara.lang` is not that it can print JavaScript or Lua. It is that the same authoring model can keep a multi-language system coherent."
           :source "hara.lang design perspective"}]]
