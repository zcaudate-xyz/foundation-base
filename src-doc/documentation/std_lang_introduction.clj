(ns documentation.lang.core-introduction
  (:require [lang.core :as l])
  (:use code.test))

[[:hero {:title "Introduction"
         :subtitle "Understanding the book, the grammar, and the runtime model."
         :lead "`tahto.core` is best understood as a **language-oriented templating system** built for real multi-language applications. It stores code in a reusable intermediate form, emits that form through a target grammar, and can then hand the result to a matching runtime."
         :badges ["Book model" "Grammar model" "Shared DSL"]}]]

[[:chapter {:title "The core idea"}]]

"Instead of writing and maintaining every target language by hand, `tahto.core` lets you author code in a Lisp DSL and describe each target language as a grammar. That grammar controls **how forms are emitted**, **which macros expand**, **which operators exist**, and **what dependencies or native fragments are required**."
...
"The fastest way to see the book and grammar model is to emit the same small function to two targets. `l/emit-as` takes a language keyword and a quoted lang.core form, and returns the emitted string."
...
(fact "install a script context and call a generated function"
  ^{:refer lang.core/script- :added "4.0"}
  (do
    (l/script- :js
...
"Straight transpilation is usually about **converting one source language into one target language**. `lang.core` aims at a broader problem: **shared authoring and maintenance across many targets and runtimes**."
...
[[:chapter {:title "lang.core.base.book and lang.core.base.library Summary" :link "merged-plans-slop-summary-std-lang-base-book-summary-md"}]]

"The `lang.core.base.book*` and `lang.core.base.library*` namespaces define the data structures for storing, managing, and accessing code within the `foundation-base` ecosystem. A \"library\" is a collection of \"books,\" each of which represents a language and contains the code and grammar for that language."

"**Core Concepts:**"

"*   **Library (`tahto.core.base.library`):** A `Library` is the top-level container for all language definitions. It holds a \"snapshot\" of the current state of all books.\n*   **Snapshot (`tahto.core.base.library-snapshot`):** A `Snapshot` is an immutable map of language IDs to book definitions. It represents a specific version of the library's content.\n*   **Book (`tahto.core.base.book`):** A `Book` holds all the code for a particular language. It contains a map of modules, a grammar, and metadata about the language.\n*   **Module (`lang.core.base.book-module`):** A `Module` represents a single source file or a collection of related code. It contains a map of code entries, as well as information about the module's dependencies, exports, and other properties.\n*   **Entry (`tahto.core.base.book-entry`):** An `Entry` represents a single piece of code, such as a function, a variable, or a macro. It contains the code itself (as a Clojure form), as well as metadata about the code."
...
(fact "lang.core.base.book and lang.core.base.library Summary example"
  (require '[lang.core.base.library :as lib])
  (require '[lang.core.base.book :as book])
  (require '[lang.core.base.book-module :as module])
  (require '[lang.core.base.book-entry :as entry])
...
"By providing a structured way to store and manage code, the `lang.core.base.book*` and `lang.core.base.library*` namespaces play a crucial role in the `foundation-base` transpiler."
...
(fact "lang.core.base.emit Summary example"
  (def +my-grammar+
    (h/merge-nested
     helper/+default+
...
[[:chapter {:title "lang.core.base.grammar Summary" :link "merged-plans-slop-summary-std-lang-base-grammar-summary-md"}]]

"The `lang.core.base.grammar*` namespaces are responsible for defining the structure and semantics of a language that can be emitted by the `foundation-base` transpiler. The grammar is a key component of the emit pipeline, providing the necessary information to translate a Clojure-like form into a string of code in the target language."

"**Core Concepts:**"

"*   **Grammar:** A grammar is a Clojure map that defines the syntax and semantics of a target language. It is created using the `lang.core.base.grammar/grammar` function, which takes a tag, a map of reserved words, and a template as arguments.\n*   **Reserved Words:** The `:reserved` map in the grammar defines how specific symbols are treated by the emitter. For each symbol, you can specify its `op` (operation), `emit` type (e.g., `:infix`, `:prefix`, `:macro`), `raw` string representation, and other properties.\n*   **Operators (`+op-*`):** The `lang.core.base.grammar-spec`, `lang.core.base.grammar-macro`, and `lang.core.base.grammar-xtalk` namespaces define a large set of operators that can be used to build a grammar. These operators cover a wide range of functionalities, from basic arithmetic and logic to more advanced features like macros and cross-language interoperability (`xtalk`).\n*   **Building a Grammar:** The `lang.core.base.grammar/build` function is used to construct a grammar by selecting a set of operators from the available `+op-*` definitions. You can include or exclude specific operator groups to create a grammar that is tailored to your needs."
...
"**Language-Specific Grammars in `lang.core.model`:**"

"The `lang.core.model.*` files provide concrete examples of how to define grammars for different languages. These files demonstrate how to:"
...
"By studying the language-specific grammars in `lang.core.model`, you can get a good understanding of how to create your own grammars and customize the emit pipeline to support new languages or extend existing ones."
...
(fact "lang.core.base.runtime Summary example"
...
"The `lang.core.base.runtime` namespace defines a `RuntimeDefault` record using `defimpl`. This record provides a default implementation for the `IContext` and `IComponent` protocols."

"*   It serves as a base for many of the language-specific runtimes in `rt.basic`.\n*   It includes logic for proxying calls to another runtime via the `redirect` field.\n*   The `default-*` functions in `tahto.core.base.runtime` provide the actual implementations for the protocol functions. For example, `default-invoke-ptr` handles the logic for invoking a function pointer."

"**3. Customization:**"

"*   **Extending `RuntimeDefault`:** The easiest way to create a new runtime is to extend `RuntimeDefault` and override the functions that need to be customized.\n*   **Creating a New Runtime from Scratch:** For more advanced use cases, you can create a new runtime from scratch by implementing the `IContext` and `IComponent` protocols yourself.\n*   **Runtime Proxies:** The `tahto.core.base.runtime-proxy` namespace allows you to create a proxy for an existing runtime. This is useful for adding functionality or for creating a different interface to a runtime."
...
[[:chapter {:title "lang.core.base.script Summary" :link "merged-plans-slop-summary-std-lang-base-script-summary-md"}]]

"The `lang.core.base.script*` namespaces provide a high-level interface for interacting with the `foundation-base` language ecosystem. They tie together the `grammar`, `emit`, `book`, and `runtime` components to provide a seamless experience for defining, compiling, and executing code in different languages."

"**Core Concepts:**"

"*   **Script:** A \"script\" is a self-contained unit of code that can be executed in a specific language runtime.\n*   **`script` macro:** The `script` macro is the main entry point for defining a script. It takes a language keyword, a module name, and a configuration map as arguments.\n*   **Runtime Management:** The `tahto.core.base.script-control` namespace provides functions for managing the lifecycle of language runtimes, including `script-rt-get`, `script-rt-stop`, and `script-rt-restart`.\n*   **Annex:** An \"annex\" (`tahto.core.base.script-annex`) is a way to extend an existing language with new functionality. It allows you to define new macros and functions that can be used in the extended language."

"**How `lang.core.base.script` Ties Everything Together:**"
...
(fact "lang.core.base.script Summary example"
  (require '[lang.core.base.script :as script])
...
"*   From `tahto.core.base.util`: `sym-full`, `sym-id`, `sym-module`, `sym-pair`, `sym-default-str`, `ptr`.\n*   From `tahto.core.base.emit-common`: `with:explode`, `with-trace`.\n*   From `tahto.core.base.emit`: `with:emit`, `emit*`.\n*   From `tahto.core.base.emit-helper`: `basic-typed-args`, `emit-type-record`.\n*   From `tahto.core.base.emit-preprocess`: `macro-form`, `macro-opts`, `macro-grammar`, `with:macro-opts`.\n*   From `tahto.core.base.impl`: `emit-script`, `emit-str`, `emit-as`, `emit-symbol`, `default-library`, `default-library:reset`, `runtime-library`, `with:library`, `grammar`.\n*   From `tahto.core.base.impl-entry`: `emit-entry`, `with:cache-none`, `with:cache-force`.\n*   From `tahto.core.base.impl-deps`: `emit-entry-deps`.\n*   From `tahto.core.base.pointer`: `with:print`, `with:print-all`, `with:clip`, `with:input`, `with:raw`, `with:rt`, `with:rt-wrap`, `rt:macro-opts`.\n*   From `tahto.core.base.script`: `script`, `script-`, `script+`, `!`, `annex:get`, `annex:start`, `annex:stop`, `annex:restart-all`, `annex:start-all`, `annex:stop-all`, `annex:list`.\n*   From `tahto.core.base.script-def`: `tmpl-entry`, `tmpl-macro`.\n*   From `tahto.core.base.library`: `get-book-raw`, `get-book`, `get-module`, `get-snapshot`, `delete-module!`, `delete-modules!`, `delete-entry!`, `purge-book!`.\n*   From `tahto.core.base.script-lint`: `lint-set`, `lint-clear`.\n*   From `tahto.core.base.util`: `rt`, `rt:list`, `rt:default`.\n*   From `tahto.core.base.script-control`: `rt:restart`, `rt:stop`.\n*   From `lang.core.base.workspace`: `sym-entry`, `module-entries`, `emit-ptr`, `emit-module`, `print-module`, `ptr-clip`, `ptr-print`, `ptr-setup`, `ptr-teardown`, `ptr-setup-deps`, `ptr-teardown-deps`, `rt:module`, `rt:module-purge`, `rt:inner`, `rt:restart`, `rt:setup`, `rt:setup-to`, `rt:setup-single`, `rt:scaffold`, `rt:scaffold-to`, `rt:scaffold-imports`, `rt:teardown`, `rt:teardown-at`, `rt:teardown-single`, `rt:teardown-to`, `intern-macros`.\n*   From `tahto.core.base.manage`: `lib:overview`, `lib:module`, `lib:entries`, `lib:purge`, `lib:unused`."
...
[[:section {:title "lang.core.base.book (Language Book Management)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-book-language-book-management"}]]
...
[[:section {:title "lang.core.base.compile (Code Compilation and Output)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-compile-code-compilation-and-output"}]]
...
[[:section {:title "lang.core.base.emit (Code Emission Pipeline)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-code-emission-pipeline"}]]
...
[[:section {:title "lang.core.base.emit-assign (Assignment Emission)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-assign-assignment-emission"}]]
...
[[:section {:title "lang.core.base.emit-block (Block Emission)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-block-block-emission"}]]
...
[[:section {:title "lang.core.base.emit-common (Common Emission Utilities)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-common-common-emission-utilities"}]]
...
[[:section {:title "lang.core.base.emit-data (Data Structure Emission)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-data-data-structure-emission"}]]
...
[[:section {:title "lang.core.base.emit-fn (Function Emission)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-fn-function-emission"}]]
...
[[:section {:title "lang.core.base.emit-helper (Emission Helper Functions)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-helper-emission-helper-functions"}]]
...
[[:section {:title "lang.core.base.emit-preprocess (Emission Preprocessing)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-preprocess-emission-preprocessing"}]]
...
[[:section {:title "lang.core.base.emit-special (Special Form Emission)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-special-special-form-emission"}]]
...
[[:section {:title "lang.core.base.emit-top-level (Top-Level Form Emission)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-emit-top-level-top-level-form-emission"}]]
...
[[:section {:title "lang.core.base.grammar (Language Grammar Definition)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-grammar-language-grammar-definition"}]]
...
[[:section {:title "lang.core.base.grammar-macro (Macro Transformations)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-grammar-macro-macro-transformations"}]]
...
[[:section {:title "lang.core.base.grammar-spec (Grammar Specification)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-grammar-spec-grammar-specification"}]]
...
[[:section {:title "lang.core.base.grammar-xtalk (Cross-Language Operators)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-grammar-xtalk-cross-language-operators"}]]
...
[[:section {:title "lang.core.base.impl (Core Implementation Utilities)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-impl-core-implementation-utilities"}]]
...
[[:section {:title "lang.core.base.impl-deps (Dependency Management Implementation)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-impl-deps-dependency-management-implementation"}]]
...
[[:section {:title "lang.core.base.impl-entry (Entry Implementation)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-impl-entry-entry-implementation"}]]
...
[[:section {:title "lang.core.base.impl-lifecycle (Lifecycle Implementation)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-impl-lifecycle-lifecycle-implementation"}]]
...
[[:section {:title "lang.core.base.library (Library Management)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-library-library-management"}]]
...
[[:section {:title "lang.core.base.library-snapshot (Library Snapshot Management)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-library-snapshot-library-snapshot-management"}]]
...
[[:section {:title "lang.core.base.manage (Language Management Tasks)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-manage-language-management-tasks"}]]
...
[[:section {:title "lang.core.base.pointer (Language Pointers)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-pointer-language-pointers"}]]
...
[[:section {:title "lang.core.base.registry (Language Registry)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-registry-language-registry"}]]
...
[[:section {:title "lang.core.base.runtime (Language Runtimes)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-runtime-language-runtimes"}]]
...
[[:section {:title "lang.core.base.runtime-proxy (Runtime Proxy)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-runtime-proxy-runtime-proxy"}]]
...
[[:section {:title "lang.core.base.script (Scripting and Runtime Control)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-script-scripting-and-runtime-control"}]]
...
[[:section {:title "lang.core.base.script-annex (Script Annex Management)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-script-annex-script-annex-management"}]]
...
[[:section {:title "lang.core.base.script-control (Script Runtime Control)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-script-control-script-runtime-control"}]]
...
[[:section {:title "lang.core.base.script-def (Script Definition Helpers)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-script-def-script-definition-helpers"}]]
...
[[:section {:title "lang.core.base.script-lint (Script Linting)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-script-lint-script-linting"}]]
...
[[:section {:title "lang.core.base.script-macro (Script Macro Interning)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-script-macro-script-macro-interning"}]]
...
[[:section {:title "lang.core.base.util (Language Utilities)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-util-language-utilities"}]]
...
[[:section {:title "lang.core.base.workspace (Workspace Management)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-base-workspace-workspace-management"}]]
...
[[:section {:title "lang.core.interface.type-notify (Notification Server)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-interface-type-notify-notification-server"}]]
...
[[:section {:title "lang.core.interface.type-shared (Shared Runtime Management)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-interface-type-shared-shared-runtime-management"}]]
...
[[:section {:title "lang.core.model. (Language Specifications)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-model-language-specifications"}]]
...
[[:section {:title "lang.core.strict.basestate (Strict Mode State)" :link "merged-plans-slop-summary-std-lang-summary-md-lang-lang-strict-basestate-strict-mode-state"}]]
...
"The `lang.core` module is the cornerstone of the `foundation-base` project, enabling:"
...
"By providing a powerful and extensible framework for language definition and code generation, `lang.core` empowers developers to build highly flexible and adaptable software systems."
