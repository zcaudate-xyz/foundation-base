...
[[:hero {:title "tahto.model"
         :subtitle "Target language specifications and xtalk function libraries."
         :lead "`tahto.model` contains target specs for JS, Lua, Python, Go, Dart, SQL, Solidity, xtalk, and annex languages. These models are the bridge between structured tahto.core forms and emitted target code."}]]
...
"`tahto.core/emit-as` is the main entry point for turning a tahto form into target source code. The same expression can be rendered for different language models just by changing the target keyword."
