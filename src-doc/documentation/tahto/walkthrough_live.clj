...
"The walkthrough shows how lang.core scripts define target contexts, how forms are emitted or executed, and how generated pointers connect Clojure authoring to target language code."
...
"What each section demonstrates: the first shows `!.js` returning *values* instead of code strings; the second makes the emission visible with `l/with:print-all`; the third shows the identical tahto.core body running under Lua and Python; the last mixes live results from all three runtimes in one Clojure expression."
...
(fact "define and call a JS function"
  ^{:refer lang.core/emit-ptr :added "4.0"}
  (do
    (l/script- :js
      {:runtime :basic
       :require [[xt.lang.spec-base :as xt]]})
...
(fact "capture emitted JS with print-all"
  ^{:refer lang.core/with:print-all :added "4.0"}
  (do
    (l/script- :js
      {:runtime :basic
       :require [[xt.lang.spec-base :as xt]]})
...
"The `:basic` runtime works for Lua and Python too. The same lang.core function body runs in each target process."
