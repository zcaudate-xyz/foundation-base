...
[[:hero {:title "Walkthrough: basic"
         ...
         :lead "The basic walkthrough introduces lang.core script contexts: how a target language is installed into a Clojure namespace, how forms are emitted as target code, and how saved entries link together as a dependency graph."}]]

[[:chapter {:title "Motivation"}]]

"tahto.core lets you write target language code — JavaScript, Lua, Python, R, SQL — as ordinary Clojure forms. A *script context* installs the macros for one target into the current namespace. Inside that context you can emit code for inspection, or save entries into a *book* that tracks every definition, its dependencies, and its emitted output."
...
(fact "!.js returns emitted JavaScript as a string"
  ^{:refer lang.core/script- :id wt-basic/emit}
  (l/script- :js
...
(fact "def.js saves a :def entry in the book"
  ^{:refer lang.core/script- :id wt-basic/def-js}
  (def.js answer (+ 1 2 3))
...
(fact "defn.js saves a :defn entry and emits calls"
  ^{:refer lang.core/script- :id wt-basic/defn-js}
  (defn.js greet
    [a b]
    (return (+ a b)))
...
(fact "-/ references are recorded as dependencies"
  ^{:refer lang.core/script- :id wt-basic/linking}
  (def.js base (+ 1 2 3))
...
(fact "def$.js creates an inlineable fragment"
  ^{:refer lang.core/script- :id wt-basic/fragment}
...
(fact "defmacro.js expands forms during emission"
  ^{:refer lang.core/script- :id wt-basic/defmacro}
