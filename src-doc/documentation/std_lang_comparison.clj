(ns documentation.tahto.core-comparison
...
[[:card-grid {:items [{:meta "Straight transpiler"
                      ...
                      {:meta "`tahto.core`"
                       ...
                       :text "`tahto.core` starts from a shared DSL and expects the grammar to decide how each target should realize that abstraction. The point is not exact syntax preservation, but reusable cross-target structure."}
                      {:meta "Tradeoff"
...
[[:card-grid {:items [{:meta "Haxe / Nim / ReScript"
                      ...
                      {:meta "Racket"
                       ...
                       :text "Racket is stronger as a host for new languages and macro systems. `tahto.core` is more directly aimed at emitting mainstream target languages and integrating them into shared runtime flows."}
                      {:meta "JetBrains MPS / Spoofax"
                       :title "Language workbenches"
                       :text "Those systems are deeper on language engineering, projectional editing, and formal tooling. `tahto.core` is lighter, code-first, and much more comfortable inside a REPL-driven Clojure workflow."}]}]]
...
[[:chapter {:title "What is unusual about `tahto.core`?"}]]
...
[[:quote {:text "The best way to think about `tahto.core` is not as a prettier transpiler. It is a reusable authoring and emission system for code that has to live in several languages at once."
