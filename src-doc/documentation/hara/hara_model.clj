tahto/tahto_model.clj:1:(ns documentation.tahto-model
  (:require [tahto.core :as l]
tahto/tahto_model.clj:3:            [tahto.model.spec-js :as js]
tahto/tahto_model.clj:4:            [tahto.model.spec-lua :as lua])
  (:use code.test))

tahto/tahto_model.clj:7:[[:hero {:title "tahto.model"
         :subtitle "Target language specifications and xtalk function libraries."
tahto/tahto_model.clj:9:         :lead "`tahto.model` contains target specs for JS, Lua, Python, Go, Dart, SQL, Solidity, xtalk, and annex languages. These models are the bridge between structured tahto.core forms and emitted target code."}]]
...
tahto/tahto_model.clj:20:"`tahto.core/emit-as` is the main entry point for turning a tahto form into target source code. The same expression can be rendered for different language models just by changing the target keyword."
