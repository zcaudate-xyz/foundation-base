...
"The `src-build/projects/play/go_001_xtalk_user_directory` and `src-build/projects/play/ts_001_single_source_user_directory` examples are the clearest single-source xtalk projects. They define typed records and functions once, then emit target artifacts for Go or TypeScript/JavaScript."
...
"Generated projects are authored as xtalk source files. The `lang.typed` loader turns a source file into a context that exposes specs, functions, and values declared in the file."

(fact "load the Go user directory example as a typed context"
  (-> (typed/load-file "src-build/projects/play/go_001_xtalk_user_directory/main.clj")
      :domain)
  => :xtalk)

(fact "inspect the User record and lookupUser function"
  (let [ctx (typed/load-file "src-build/projects/play/go_001_xtalk_user_directory/main.clj")]
    [(-> (typed/spec-def ctx 'lang-demos.go-001-xtalk-user-directory.main/User)
         :type
         :kind)
     (mapv (comp str :name)
           (typed/function-input ctx 'lang-demos.go-001-xtalk-user-directory.main/lookupUser))
     (-> (typed/function-output ctx 'lang-demos.go-001-xtalk-user-directory.main/lookupUser)
         :kind)])
...
(fact "emit Go type declarations"
  (-> (xtalk-parse/analyze-file "src-build/projects/play/go_001_xtalk_user_directory/main.clj")
      (go-typed/emit-analysis-declarations)
      (str/includes? "type User"))
  => true)

(fact "emit TypeScript type declarations"
  (-> (xtalk-parse/analyze-file "src-build/projects/play/ts_001_single_source_user_directory/main.clj")
