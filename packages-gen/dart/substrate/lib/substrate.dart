...
node_configure(node, opts) {
  ...
  for(var entry_41957 in (opts["spaces"] ?? <dynamic, dynamic>{}).entries){
    var space_id = entry_41957.key;
    var config = entry_41957.value;
  ...
  for(var entry_41958 in (opts["handlers"] ?? <dynamic, dynamic>{}).entries){
    var action = entry_41958.key;
    var config = entry_41958.value;
    var entry = base_util.config_normalize_handler(action,config);
    register_handler(node,action,entry["fn"],entry["meta"]);
  };
  for(var entry_41959 in (opts["triggers"] ?? <dynamic, dynamic>{}).entries){
    var signal = entry_41959.key;
    var config = entry_41959.value;
