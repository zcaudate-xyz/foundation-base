import 'package:xtalk_lang/common-tree.dart' as xtt;
import 'package:xtalk_lang/common-data.dart' as xtd;
import 'package:xtalk_db/text/base-scope.dart' as base_scope;
import 'package:xtalk_db/text/base-check.dart' as check;
import 'package:xtalk_db/text/sql-util.dart' as sql_util;
import 'package:xtalk_db/text/sql-graph.dart' as sql_graph;







tree_control_array(control) {
  if((() {
    var dart_truthy__52472 = xtd.is_emptyp(control);
    return (null != dart_truthy__52472) && (false != dart_truthy__52472);
  })()){
    return <dynamic>[];
  }
  var out = <dynamic>[];
  var limit = control["limit"];
  var offset = control["offset"];
  var order_by = control["order_by"];
  var order_sort = control["order_sort"];
  if((order_by.runtimeType).toString().startsWith("List") || (order_by.runtimeType).toString().startsWith("_GrowableList")){
    out.add(sql_util.ORDER_BY(order_by));
  }
  if(null != order_sort){
    out.add(sql_util.ORDER_SORT(order_sort));
  }
  if(("int" == (limit.runtimeType).toString()) || ("double" == (limit.runtimeType).toString()) || ("num" == (limit.runtimeType).toString())){
    out.add(sql_util.LIMIT(limit));
  }
  if(("int" == (offset.runtimeType).toString()) || ("double" == (offset.runtimeType).toString()) || ("num" == (offset.runtimeType).toString())){
    out.add(sql_util.OFFSET(offset));
  }
  return out;
}

tree_base(schema, table_name, sel_query, clause, returning, opts) {
  var tarr = base_scope.merge_queries(sel_query,clause);
  var tree = xtd.arr_assign(<dynamic>[table_name],tarr);
  if((() {
    var dart_truthy__52471 = xtd.not_emptyp(returning);
    return (null != dart_truthy__52471) && (false != dart_truthy__52471);
  })()){
    tree.add(returning);
  }
  return sql_graph.select_tree(schema,tree,opts);
}

tree_count(schema, entry, clause, opts) {
  var control = entry["control"];
  var view = entry["view"];
  var query = view["query"];
  var table = view["table"];
  return tree_base(schema,table,query,clause,xtd.arr_assign(
    <dynamic>[<dynamic, dynamic>{"::":"sql/count"}],
    tree_control_array(control)
  ),opts);
}

tree_select(schema, entry, clause, opts) {
  var control = entry["control"];
  var view = entry["view"];
  var query = view["query"];
  var table = view["table"];
  return tree_base(
    schema,
    table,
    query,
    clause,
    xtd.arr_assign(<dynamic>["id"],tree_control_array(control)),
    opts
  );
}

tree_return(schema, entry, sel_query, clause, opts) {
  var view = entry["view"];
  var query = view["query"];
  var table = view["table"];
  return tree_base(schema,table,sel_query,clause,query,opts);
}

tree_combined(schema, sel_entry, ret_entry, ret_omit, clause, opts) {
  var control = sel_entry["control"];
  var sel_table = sel_entry["view"]["table"];
  var ret_table = ret_entry["view"]["table"];
  var sel_query = sel_entry["view"]["query"];
  var ret_query = ret_entry["view"]["query"];
  if(null == sel_query){
    sel_query = <dynamic, dynamic>{};
  }
  if(null == ret_query){
    ret_query = <dynamic, dynamic>{};
  }
  var ret_clause = ((null != xtd.not_emptyp(ret_omit)) && (false != xtd.not_emptyp(ret_omit))) ? <dynamic>[
    <dynamic, dynamic>{"id":<dynamic, dynamic>{"not_in":<dynamic>[ret_omit]}}
  ] : <dynamic>[];
  var combined_clause = base_scope.merge_queries(clause,ret_clause);
  return tree_base(
    schema,
    sel_table,
    sel_query,
    combined_clause,
    xtd.arr_assign(xtd.arr_clone(ret_query),tree_control_array(control)),
    opts
  );
}

tree_fill_input(tree, args, input_spec, drop_first) {
  var arg_map = <dynamic, dynamic>{};
  if((null != drop_first) && (false != drop_first)){
    input_spec.removeAt(0);
  }
  if(0 == input_spec.length){
    return tree;
  }
  var arr_52473 = input_spec;
  for(var i = 0; i < arr_52473.length; ++i){
    var e = arr_52473[i];
    arg_map["{{" + e["symbol"] + "}}"] = args[i];
  };
  var out = xtt.tree_walk(tree,(x) {
    return x;
  },(x) {
    if(("String" == (x.runtimeType).toString()) && arg_map.containsKey(x)){
      return arg_map[x];
    }
    return x;
  });
  return out;
}

plan_select(schema, entry, args, opts) {
  var input = entry["input"];
  var itree = tree_select(schema,entry,<dynamic, dynamic>{},opts);
  var qtree = tree_fill_input(itree,args,xtd.arr_clone(input),false);
  return qtree;
}

plan_count(schema, entry, args, opts) {
  var input = entry["input"];
  var itree = tree_count(schema,entry,<dynamic, dynamic>{},opts);
  var qtree = tree_fill_input(itree,args,xtd.arr_clone(input),false);
  return qtree;
}

plan_return(schema, entry, id, args, opts) {
  var input = entry["input"];
  var itree = tree_return(
    schema,
    entry,
    <dynamic, dynamic>{"id":id},
    <dynamic, dynamic>{},
    opts
  );
  var qtree = tree_fill_input(itree,args,xtd.arr_clone(input),true);
  return qtree;
}

plan_return_bulk(schema, entry, ids, args, opts) {
  var input = entry["input"];
  var itree = tree_return(
    schema,
    entry,
    <dynamic, dynamic>{"id":<dynamic>["in",<dynamic>[ids]]},
    <dynamic, dynamic>{},
    opts
  );
  var qtree = tree_fill_input(itree,args,xtd.arr_clone(input),true);
  return qtree;
}

plan_combined(schema, sel_entry, sel_args, ret_entry, ret_args, ret_omit, opts, as_tree) {
  var sel_input = sel_entry["input"];
  var ret_input = ret_entry["input"];
  var itree = tree_combined(schema,sel_entry,ret_entry,ret_omit,<dynamic>[],opts);
  var qtree = tree_fill_input(
    itree,
    xtd.arr_assign(xtd.arr_clone(ret_args),sel_args),
    xtd.arr_assign(xtd.arr_clone(ret_input),sel_input),
    (ret_input.length > 0) ? true : false
  );
  return qtree;
}

plan_view_check(entry, args, drop_first) {
  var targs = entry["input"];
  if((null != drop_first) && (false != drop_first)){
    targs = <dynamic>[...targs];
    targs.removeAt(0);
  }
  var value_52494 = check.check_args_length(args,targs);
  var l_ok = value_52494[0];
  var l_err = value_52494[1];
  if(!((null != l_ok) && (false != l_ok))){
    return <dynamic>[l_ok,l_err];
  }
  var value_52495 = check.check_args_type(args,targs);
  var t_ok = value_52495[0];
  var t_err = value_52495[1];
  if(!((null != t_ok) && (false != t_ok))){
    return <dynamic>[t_ok,t_err];
  }
  return <dynamic>[true,null];
}

plan_view(schema, query_spec) {
  var return_args = query_spec["return_args"];
  var return_bulk = query_spec["return_bulk"];
  var return_count = query_spec["return_count"];
  var return_entry = query_spec["return_entry"];
  var return_id = query_spec["return_id"];
  var return_omit = query_spec["return_omit"];
  var select_args = query_spec["select_args"];
  var select_entry = query_spec["select_entry"];
  var table = query_spec["table"];
  if((null != select_entry) && (null != return_entry)){
    var value_52496 = plan_view_check(select_entry,select_args,false);
    var s_ok = value_52496[0];
    var s_err = value_52496[1];
    if(!((null != s_ok) && (false != s_ok))){
      return <dynamic>[s_ok,s_err];
    }
    var value_52497 = plan_view_check(return_entry,return_args,true);
    var r_ok = value_52497[0];
    var r_err = value_52497[1];
    if(!((null != r_ok) && (false != r_ok))){
      return <dynamic>[r_ok,r_err];
    }
    return <dynamic>[
      true,
      plan_combined(
          schema,
          select_entry,
          select_args,
          return_entry,
          return_args,
          return_omit,
          <dynamic, dynamic>{},
          false
        )
    ];
  }
  else if(null != select_entry){
    var value_52498 = plan_view_check(select_entry,select_args,false);
    var s_ok = value_52498[0];
    var s_err = value_52498[1];
    if(!((null != s_ok) && (false != s_ok))){
      return <dynamic>[s_ok,s_err];
    }
    return <dynamic>[
      true,
      ((null != return_count) && (false != return_count)) ? plan_count(schema,select_entry,select_args,<dynamic, dynamic>{}) : plan_select(schema,select_entry,select_args,<dynamic, dynamic>{})
    ];
  }
  else if(null != return_id){
    var rargs = <dynamic>[return_id,...return_args];
    var value_52499 = plan_view_check(return_entry,rargs,false);
    var r_ok = value_52499[0];
    var r_err = value_52499[1];
    if(!((null != r_ok) && (false != r_ok))){
      return <dynamic>[r_ok,r_err];
    }
    return <dynamic>[
      true,
      plan_return(schema,return_entry,return_id,return_args,<dynamic, dynamic>{})
    ];
  }
  else if(null != return_bulk){
    var value_52500 = plan_view_check(return_entry,return_args,true);
    var r_ok = value_52500[0];
    var r_err = value_52500[1];
    if(!((null != r_ok) && (false != r_ok))){
      return <dynamic>[r_ok,r_err];
    }
    return <dynamic>[
      true,
      plan_return_bulk(
          schema,
          return_entry,
          return_bulk,
          return_args,
          <dynamic, dynamic>{}
        )
    ];
  }
  else{
    return <dynamic>[true,null];
  }
}