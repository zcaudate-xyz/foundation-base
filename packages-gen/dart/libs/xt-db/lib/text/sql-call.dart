...
call_format_input(spec, args) {
  ...
  var out = <dynamic>[];
  var arr_52224 = args;
  for(var i = 0; i < arr_52224.length; ++i){
    var arg = arr_52224[i];
    var input = targs[i];
    var dbarg = null;
    if(input["type"] == "jsonb"){
...
call_raw(client, spec, args) {
  var targs = spec["input"];
  var value_52245 = check.check_args_length(args,targs);
  var l_ok = value_52245[0];
  var l_err = value_52245[1];
  if(!((null != l_ok) && (false != l_ok))){
    throw "ERR: - " + jsonEncode(l_err);
  }
  var value_52246 = check.check_args_type(args,targs);
  var t_ok = value_52246[0];
  var t_err = value_52246[1];
...
call_api(client, spec, args) {
  var targs = spec["input"];
  var value_52247 = check.check_args_length(args,targs);
  var l_ok = value_52247[0];
  var l_err = value_52247[1];
  if(!((null != l_ok) && (false != l_ok))){
    return jsonEncode(<dynamic, dynamic>{"status":"error","data":l_err});
  }
  var value_52248 = check.check_args_type(args,targs);
  var t_ok = value_52248[0];
  var t_err = value_52248[1];
  if(!((null != t_ok) && (false != t_ok))){
  ...
  var error_fn = (err) {
    if((() {
      var dart_truthy__52223 = err["status"];
      return (null != dart_truthy__52223) && (false != dart_truthy__52223);
    })()){
