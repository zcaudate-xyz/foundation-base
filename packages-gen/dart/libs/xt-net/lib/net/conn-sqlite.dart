...
raw_query(db, query) {
  if((() {
    var dart_truthy__51593 = query_returns_rowsp(query);
    return (null != dart_truthy__51593) && (false != dart_truthy__51593);
  })()){
    var result = db.select(query);
    var columns = result.columnNames;
    var values = null;
    values = <dynamic>[];
    var iter_51594 = result.iterator;
    while(iter_51594.moveNext()){
      var row = iter_51594.current;
      var row_values = null;
      row_values = <dynamic>[];
      var arr_51607 = columns;
      for(var i = 0; i < arr_51607.length; ++i){
        var _ = arr_51607[i];
