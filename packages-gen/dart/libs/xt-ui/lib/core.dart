...
registry_compose(layers) {
  ...
  var arr_52744 = layers ?? <dynamic>[];
  for(var i52745 = 0; i52745 < arr_52744.length; ++i52745){
    var layer = arr_52744[i52745];
    for(var entry_52766 in (layer["contracts"] ?? <dynamic, dynamic>{}).entries){
      var component_id = entry_52766.key;
      var contract = entry_52766.value;
      registry_register_contract(out,contract);
    };
    for(var entry_52767 in (layer["renderers"] ?? <dynamic, dynamic>{}).entries){
      var component_id = entry_52767.key;
      var renderer = entry_52767.value;
...
validate_props(contract, props) {
  ...
  for(var prop_id in (props ?? <dynamic, dynamic>{}).keys){
    if(!(() {
      var dart_truthy__52742 = xtd.arr_some(allowed,(allowed_id) {
        return allowed_id == prop_id;
      });
      return (null != dart_truthy__52742) && (false != dart_truthy__52742);
    })() && !(() {
      var dart_truthy__52743 = xtd.arr_some(events,(event_id) {
        return event_id == prop_id;
      });
      return (null != dart_truthy__52743) && (false != dart_truthy__52743);
    })()){
...
validate_node(registry, ui_node) {
  ...
  if((ui_node.runtimeType).toString().startsWith("List") || (ui_node.runtimeType).toString().startsWith("_GrowableList")){
    var arr_52768 = ui_node;
    for(var i52769 = 0; i52769 < arr_52768.length; ++i52769){
      var child = arr_52768[i52769];
  ...
  var arr_52790 = ui_node["children"] ?? <dynamic>[];
  for(var i52791 = 0; i52791 < arr_52790.length; ++i52791){
    var child = arr_52790[i52791];
    validate_node(registry,child);
  };
  return true;
}
...
base_registry() {
  ...
  for(var i52813 = 0; i52813 < arr_52812.length; ++i52813){
    var component_id = arr_52812[i52813];
