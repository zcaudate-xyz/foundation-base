...
table_get_data(entry) {
  ...
  for(var entry_42800 in entry["ref_links"].entries){
    var link = entry_42800.key;
    var m = entry_42800.value;
...
table_emit_flat(emit_fn, schema, lookup, flat, opts) {
  ...
  var emit_pair_fn = (pair) {
    var value_42801 = pair;
    var table_name = value_42801[0];
    var data = value_42801[1];
    var cols = base_schema.table_columns(schema,table_name);
...
prepare_remove_input(data, schema, lookup, opts) {
  ...
  var statements = xtd.arr_mapcat(ordered,(entry) {
    var value_42804 = entry;
    var table_name = value_42804[0];
    var ids = value_42804[1];
