...
(defn.lua native-request
  "dispatches a request through resty.http"
  {:added "4.1.3"}
  [raw request]
  (var client (or (xt/x:get-key raw "client")
                  (http/new)))
  (var response
  ...
  (return (-/normalise-response response)))
...
(defn.lua default-request-sync
  "dispatches a sync request using raw request_sync, request, or resty.http"
  {:added "4.1.3"}
  [raw input opts]
  (:= raw (or raw {}))
  ...
  (when (xt/x:is-function? request-fn)
    ...
    (when (and (xt/x:is-object? output)
      ...
      (xt/x:err "Lua client fetch request_sync cannot unwrap async request"))
    (return (-/normalise-response output)))
  (return (-/native-request raw request)))
