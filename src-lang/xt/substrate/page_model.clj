...
(defn.xt ensure-space-state
  ...
  (when (not (and (xt/x:not-nil? state)
                  (== (xt/x:get-key state "::") "substrate.page.state")))
...
(defn.xt resolve-view
  ...
  (var resolver (or (xt/x:get-key view "resolver") {}))
  (var type (or (xt/x:get-key resolver "type")
                "fn/local"))
  (var ctx {"node" node
            "space_id" space-id
            "state" state
...
(defn.xt view-refresh
  ...
  {:added "4.1"}
  [node space-id model-id view-id]
  (return (-/view-refresh-result
           (-/resolve-view node space-id model-id view-id))))
