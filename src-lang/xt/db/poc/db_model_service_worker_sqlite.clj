(ns xt.db.poc.db-model-service-worker-sqlite
...
(defn.js create-impl
  "Creates a db impl. Memory is created directly; sqlite/postgres impls should
   be pre-created and passed via :impl in the service config."
  {:added "4.1"}
  [type schema lookup]
  (cond (== type "memory")
...
(defn.js init-services
  "Initialises db/primary and db/caching services on a node.

   `config` entries can be either {:type \"memory\"} or {:impl <pre-created-impl>}.
   The latter is used for sqlite because the client needs to be created and
   initialised asynchronously before the service is attached."
  {:added "4.1"}
