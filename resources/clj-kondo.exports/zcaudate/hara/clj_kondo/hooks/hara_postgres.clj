...
(def ^:private +language-suffix+
  {:postgres "pg" :javascript "js" :typescript "ts" :xtalk "xt"
...
(defn function-shape [node]
  (let [children (rest (:children node))
    ...
    (when (and name-node args-index)
      {:name-node name-node :args-node (nth (vec remainder) args-index)
       :body-nodes (drop (inc args-index) remainder)
       :sql? (or (= :sql (:%% metadata)) (= :sql (:%% (meta name-node))))})))
(defn- normalized-args-node [args-node]
  (api/vector-node
   (->> (:children args-node)
        (keep (fn [child]
                (let [value (api/sexpr child)]
                  (when (symbol? value) (api/token-node value))))))))
(defn lint-function! [{:keys [name-node args-node body-nodes sql?]}]
  (let [name (api/sexpr name-node) args (api/sexpr args-node) body (map api/sexpr body-nodes)
        ...
        direct-returns (filter #(and (= "return" (form-head %)) (seq? (second %)))
                              (all-nodes (cons 'do body)))]
    (when (and (not (or sql? (and (zero? top-let-count)
                                  (some #(and (seq? %)
...
(defn defn-pg [{:keys [node]}]
  (if-let [{:keys [name-node args-node] :as shape} (function-shape node)]
    (do (lint-function! shape)
        {:node (api/list-node
                [(api/token-node 'clojure.core/defn) name-node
