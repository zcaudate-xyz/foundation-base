          ...
          (let [run (vec (take-while (fn [entry]
            ...
            (when (> (count run) 1)
              (let [targets (mapv (comp :target var-dot-binding api/sexpr) run)]
                (report! node :lang.xtalk/merge-dot-destructure :warning
   ...
   (when node
     (let [form (api/sexpr node)
       ...
       (when (and (not dot-object?)
         ...
         (report! node :lang.xtalk/nested-dot-access :warning
      ...
      (cond
        ...
        (let [raw-head (first form)
          ...
          (when-let [{:keys [target object key]} (when (= head 'var)
            ...
            (report! node :lang.xtalk/redundant-dot-destructure :warning
          ...
          (when (and (= head 'x:get-key)
                     (or (= 3 (count form))
                         (and (= 4 (count form))
                              (nil? (nth form 3)))))
            (report! node :lang.xtalk/redundant-get-key :warning
