          ...
          (let [run (vec (take-while (fn [entry]
            ...
            (when (> (count run) 1)
              (let [targets (mapv (comp :target var-dot-binding api/sexpr) run)]
                (report! node :tahto.xtalk/merge-dot-destructure :warning
   ...
   (when node
     (let [form (api/sexpr node)
       ...
       (when (and (not dot-object?)
         ...
         (report! node :tahto.xtalk/nested-dot-access :warning
       ...
       (when (and (api/list-node? node)
         ...
         (when-let [suggestion (fn-arrow-suggestion form)]
           (report! node :tahto.xtalk/redundant-fn-arrow :warning
  ...
  (let [target-node (nth (vec (rest (:children node))) 0 nil)
    ...
    (cond
      ...
      (let [invalid (remove symbol? target)
        ...
        (when (seq invalid)
          (report! target-node :tahto.xtalk/invalid-destructuring :error
                   "set destructuring targets must contain only symbols"))
        (doseq [[field bindings] fields]
          (when (> (count bindings) 1)
            (report! target-node :tahto.xtalk/field-collision :error
      ...
      (cond
        ...
        (let [head (canonical-head (first form))]
          (when (and (= :value context) (block-head? head))
            (report! node :tahto.xtalk/block-in-value :error
                     (str "block form " head
          ...
          (when-let [{:keys [target object key]} (when (= head 'var)
            ...
            (report! node :tahto.xtalk/redundant-dot-destructure :warning
          ...
          (when (and (= head 'x:get-key)
                     (or (= 3 (count form))
                         (and (= 4 (count form))
                              (nil? (nth form 3)))))
            (report! node :tahto.xtalk/redundant-get-key :warning
