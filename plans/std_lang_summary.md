...
### 1.1. Book
A "Book" (`tahto.core.base.book`) is the central data structure that holds the definition of a language environment. It contains:
...
### 1.2. Grammar
The grammar (`tahto.core.base.grammar`) defines how Lisp forms are translated into the target language's syntax. It includes:
...
### 1.3. Emission
The emission process (`tahto.core.base.emit`) takes a Lisp form and converts it to a string string based on the grammar.
...
### 1.4. Scripting
The `tahto.core` DSL allows defining code within Clojure namespaces using `l/script`.
...
*   **JavaScript (`:js`)**: Defined in `spec_js.clj`. Supports JSX (via `tahto.core.model.spec-js.jsx`), QML, and standard JS constructs.
...
2.  **Loading**: The `tahto.core` runtime loads these definitions into the global registry (or a specific Book).
