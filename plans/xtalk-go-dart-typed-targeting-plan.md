...
1. **Typed AST/registry and namespace analysis already exist**
   - `lang.core.typed.xtalk` exposes registration and analysis APIs (`analyze-namespace`, `analyze-and-register!`, registry lookups).
...
3. **A working declaration backend exists (TypeScript)**
   - `lang.core.model.spec-js.ts` already demonstrates:
...
4. **Go runtime/codegen already exists for xtalk language model**
   - `lang.core.model.spec-go` and `lang.core.model.spec-xtalk.fn-go` already provide syntax/runtime mappings for xtalk -> Go code generation.
...
Create a small common module (e.g. `lang.core.model.spec-xtalk.typed.emit`) that encapsulates:
