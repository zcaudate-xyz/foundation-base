...
(fact "starts the redis server in a given directory"
  (with-redefs [network/port:check-available (constantly 17001)
                fs/create-directory (constantly nil)
                fs/exists? (constantly true)
                os/sh (constantly (delay "process"))
                os/sh-wait (constantly nil)
...
(fact "stop the redis server"
  (with-redefs [os/sh-close (constantly nil)
                os/sh-exit (constantly nil)
                os/sh-wait (constantly nil)]
