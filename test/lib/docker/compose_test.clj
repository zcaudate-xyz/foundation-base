  ...
  {:compose   [[:image (or image "redis")]
               ...
               [:healthcheck
                {:test ["CMD" "redis-cli" "ping"]
                 :interval "1m"}]]
   :export    {:APP_EV_HOST (or ip (std.lib.foundation/strn name))
               :APP_EV_PORT 6379}})
  ...
  {:compose  [[:image (or image "minio/minio")]
              ...
              [:healthcheck
               {:test ["CMD" "curl" "-f" (format "http://%s:9000/minio/health/live"
                                                 name)]
                :interval "1m"}]]
   :export   {:APP_MINIO_HOST  (or ip (std.lib.foundation/strn name))
              :APP_MINIO_PORT  9000
  ...
  {:compose   [[:image  (or image "app-server")]
               ...
               [:healthcheck
                {:test ["CMD" "curl" "-f" (str "http://" name)]
                 :interval "1m"}]]
   :export    {:APP_WEB_HOST  (or ip (std.lib.foundation/strn name))
               :APP_WEB_PORT  80
               :APP_WEB_SECURED  "no"}})
