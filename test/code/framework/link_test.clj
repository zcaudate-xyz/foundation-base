...
(fact "splits runtime namespaces into dedicated packages"
  (let [entries          (collect-entries -packages- -lookups-)
        lang-core-entries (set (get-in entries '[xyz.zcaudate/lang.core :entries]))
        lang-annex-entries (set (get-in entries '[xyz.zcaudate/lang.annex :entries]))
        xt-entries       (set (get-in entries '[xyz.zcaudate/xt.lang :entries]))
        ...
        nginx-entries    (set (get-in entries '[xyz.zcaudate/lang.runtime.annex.nginx :entries]))
    ...
    [(boolean (lang-core-entries [:clj 'lang.model.builtin.spec-xtalk]))
     (boolean (lang-core-entries [:clj 'lang.runtime.basic.type-basic]))
     (boolean (lang-core-entries [:clj 'lang.runtime.annex.postgres]))
     (boolean (lang-core-entries [:clj 'lang.runtime.graal]))
     (boolean (lang-core-entries [:clj 'lang.runtime.annex.nginx]))
     (boolean (lang-core-entries [:clj 'xt.lang.common-lib]))
     (boolean (lang-annex-entries [:clj 'lang.model.annex.spec-r]))
     (boolean (lang-annex-entries [:clj 'lang.runtime.annex.haxe]))
     (boolean (lang-annex-entries [:clj 'lang.runtime.annex.postgres]))
     (boolean (lang-annex-entries [:clj 'lang.runtime.annex.redis]))
     (boolean (xt-entries [:clj 'xt.lang.common-lib]))
     (boolean (postgres-entries [:clj 'postgres.core]))
     (boolean (postgres-entries [:clj 'lang.runtime.annex.postgres]))
     (boolean (solidity-entries [:clj 'solidity.core]))
     (boolean (solidity-entries [:clj 'lang.runtime.annex.solidity]))
     (boolean (solidity-entries [:clj 'lang.runtime.annex.solidity.client]))
     (boolean (nginx-entries [:clj 'lang.runtime.annex.nginx]))
     (boolean (nginx-entries [:clj 'lang.runtime.annex.nginx.config]))
     (boolean (graal-entries [:clj 'lang.runtime.graal]))
     (boolean (jep-entries [:clj 'lang.runtime.annex.jep]))
     (boolean (redis-entries [:clj 'lang.runtime.annex.redis]))
     (contains? entries 'xyz.zcaudate/lang.core)
     (contains? entries 'xyz.zcaudate/lang.annex)
     (contains? entries 'xyz.zcaudate/xt.lang)
     (contains? entries 'xyz.zcaudate/lang)])
  => [true true false false false false true true false false true true true true true true true true true true true true true false])
