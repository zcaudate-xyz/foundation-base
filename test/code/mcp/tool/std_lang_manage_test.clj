...
(fact "dispatches std.lang.manage tasks with default autopilot params"
  (let [calls (atom nil)]
    (with-redefs [code.mcp.tool.std-lang-manage/+task-aliases+ {:inventory (fn [params]
                                                                             (reset! calls params)
                                                                             "inventory summary")}]
...
(fact "passes EDN vector arguments directly to std.lang.manage tasks"
  (let [calls (atom nil)]
    (with-redefs [code.mcp.tool.std-lang-manage/+task-aliases+ {:support-matrix (fn [params]
                                                                                  (reset! calls params)
                                                                                  "support summary")}]
