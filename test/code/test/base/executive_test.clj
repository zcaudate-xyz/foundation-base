    ...
    (binding [context/*root* "/tmp/proj"]
      (with-redefs [executive/save-artifact (fn [label path _]
                    ...
                    executive/save-run-history (fn [& _]
                                                 "/tmp/proj/.lang/runs/run-history.csv")
        ...
        [(executive/save-report-paths {:passed [] :failed [] :throw [] :timeout []}
         ...
         (first @captured)])))
  => [{:report-path nil
       :run-path "/tmp/proj/.lang/runs/run-1.run.edn"
       :history-path "/tmp/proj/.lang/runs/run-history.csv"}
      1
      ["Run helper" "/tmp/proj/.lang/runs/run-1.run.edn"]])
...
(fact "appends a csv row describing the run"
  ...
  (let [calls (atom [])]
    (with-redefs [std.fs/exists? (fn [_] false)
      ...
      [(executive/save-run-history {:failed [{:meta {}}] :throw [] :timeout []}
                                   ...
                                   {:run-path "/tmp/.lang/runs/run-1.run.edn"})
  ...
  => ["/tmp/.lang/runs/run-history.csv"
      [[:mkdir "/tmp/.lang/runs"] true true true]])
