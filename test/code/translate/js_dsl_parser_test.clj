(ns code.translate.js-dsl-integration-test
  (:use code.test)
...
(comment
  ...
  (let [source   "../Smalltalkinterfacedesign/src"
        target   "test-data/code.translate/sample/smalltalkinterfacedesign"
  ...
  (let [target   "test-data/code.translate/sample/smalltalkinterfacedesign"
        files  (keys (std.fs/list
                      target
                      {:recursive true
                       :include [".tsx.json$"]}))]
    (doseq [f files]
      (let [ast  (std.json/read
        ...
        (std.lib/prn f)
        (code.translate.js-dsl/translate-node ast)))))
