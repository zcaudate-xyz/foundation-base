(ns lang.seedgen.common-xtalk-test
  (:use code.test)
  (:require [lang.seedgen.common-xtalk :refer :all]))
...
^{:refer lang.seedgen.common-xtalk/grammar-entries :added "4.1"}
(fact "returns grammar xtalk entries"
  (vector? (grammar-entries))
  => true)

^{:refer lang.seedgen.common-xtalk/categories :added "4.1"}
(fact "returns xtalk categories"
  (vector? (categories))
  => true)

^{:refer lang.seedgen.common-xtalk/op-map :added "4.1"}
(fact "returns op map keyed by op"
  (map? (op-map))
  => true)

^{:refer lang.seedgen.common-xtalk/symbols :added "4.1"}
(fact "returns xtalk symbols vector"
  (vector? (symbols))
  => true)

^{:refer lang.seedgen.common-xtalk/installed-languages :added "4.1"}
(fact "returns installed languages vector"
  (vector? (installed-languages))
  => true)

^{:refer lang.seedgen.common-xtalk/parent-languages :added "4.1"}
(fact "returns xtalk parent languages vector"
  (vector? (parent-languages))
  => true)

^{:refer lang.seedgen.common-xtalk/languages :added "4.1"}
(fact "returns audit language selection"
  (vector? (languages))
  => true)

^{:refer lang.seedgen.common-xtalk/feature-status :added "4.1"}
(fact "feature-status returns known state keyword"
  (keyword? (feature-status :js 'x:get-key))
  => true)

^{:refer lang.seedgen.common-xtalk/support :added "4.1"}
...
^{:refer lang.seedgen.common-xtalk/model-inventory :added "4.1"}
(fact "returns xtalk model inventory"
  (map? (model-inventory))
  => true)

^{:refer lang.seedgen.common-xtalk/test-inventory :added "4.1"}
(fact "returns xtalk test inventory"
  (map? (test-inventory))
  => true)

^{:refer lang.seedgen.common-xtalk/runtime-inventory :added "4.1"}
...
^{:refer lang.seedgen.common-xtalk/spec-inventory :added "4.1"}
(fact "returns xtalk spec inventory"
  (map? (spec-inventory))
  => true)

^{:refer lang.seedgen.common-xtalk/language-status :added "4.1"}
(fact "returns merged xtalk language status"
  (map? (language-status))
  => true)

^{:refer lang.seedgen.common-xtalk/coverage-summary :added "4.1"}
(fact "returns xtalk coverage summary"
  (map? (coverage-summary))
  => true)

^{:refer lang.seedgen.common-xtalk/missing-by-language :added "4.1"}
(fact "missing-by-language returns map"
  (map? (missing-by-language))
  => true)

^{:refer lang.seedgen.common-xtalk/missing-by-feature :added "4.1"}
(fact "missing-by-feature returns map"
  (map? (missing-by-feature))
  => true)

^{:refer lang.seedgen.common-xtalk/inventory-path :added "4.1"}
(fact "builds ops path"
  (string? (inventory-path {:project {:root "."}}))
  => true)

^{:refer lang.seedgen.common-xtalk/read-inventory :added "4.1"}
(fact "returns nil for missing ops file"
  (read-inventory "target/missing-xtalk-ops.edn")
  => nil)

^{:refer lang.seedgen.common-xtalk/inventory-entries :added "4.1"}
(fact "builds xtalk inventory entries from grammar tables"
  ...
  => {:op :x-get-key
      ...
      :macro 'lang.base.grammar-xtalk/tf-get-key
      :emit :macro})

^{:refer lang.seedgen.common-xtalk/render-inventory :added "4.1"}
(fact "renders entries as string"
  (string? (render-inventory [{:op :x}]))
  => true)

^{:refer lang.seedgen.common-xtalk/generate-inventory :added "4.1"}
(fact "generates xtalk ops inventory"
  (map? (generate-inventory {:write false}))
  => true)

^{:refer lang.seedgen.common-xtalk/render-support :added "4.1"}
