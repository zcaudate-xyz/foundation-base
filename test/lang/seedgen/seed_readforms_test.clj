...
^{:refer lang.seedgen.form-parse/class-empty :added "4.1"}
(fact "returns an empty classification map"
  (seed-readforms/class-empty) => {:root [] :derived [] :scaffold []})

^{:refer lang.seedgen.form-parse/class-explicit :added "4.1"}
...
^{:refer lang.seedgen.form-parse/form-script? :added "4.1"}
(fact "recognizes lang.core/script- forms"
  (seed-readforms/form-script? #{'lang.core/script-} '(lang.core/script- :js {})) => true
  (seed-readforms/form-script? #{'lang.core/script-} '(+ 1 2 3)) => false
  (seed-readforms/form-script? #{'l/script-} '(l/script- :js {})) => true)

^{:refer lang.seedgen.form-parse/class-form :added "4.1"}
(fact "classifies forms as root, derived or scaffold"
  (seed-readforms/class-form :js #{'lang.core/script-} '(lang.core/script- :js {})) => :root
  (seed-readforms/class-form :js #{'lang.core/script-} '(lang.core/script- :lua {})) => :derived
  (seed-readforms/class-form :js #{'lang.core/script-} '(!.js 1)) => :root
  (seed-readforms/class-form :js #{'lang.core/script-} '(!.lua 1)) => :derived
  (seed-readforms/class-form :js #{'lang.core/script-} '(+ 1 2 3)) => :scaffold)

^{:refer lang.seedgen.form-parse/class-navs :added "4.1"}
(fact "classifies a sequence of navigable forms"
  (let [text "(lang.core/script- :js {:runtime :basic})\n\n(!.js 1)\n\n(+ 1 2 3)"
        root (nav/parse-root text)
        navs (form-common/nav-top-levels root)]
    (seed-readforms/class-navs :js #{'lang.core/script-} navs)
    => (contains {:root (fn [v] (= 2 (count v)))
                  :derived []
                  :scaffold (fn [v] (= 1 (count v)))})))

^{:refer lang.seedgen.form-parse/class-merge :added "4.1"}
...
^{:refer lang.seedgen.form-parse/fact-config-nav :added "4.1"}
...
^{:refer lang.seedgen.form-parse/global-context :added "4.1"}
...
^{:refer lang.seedgen.form-parse/entry-enrich :added "4.1"}
(fact "enriches an entry with fact classification"
  (let [text "^{:setup [(!.js (setup))]}\n(fact \"hello\"\n  (!.js 1)\n  => 1)"
        root (nav/parse-root text)
        fact-nav (nav/down root)
        entry {}]
    (seed-readforms/entry-enrich :js #{'lang.core/script-} fact-nav entry)
    => (contains {:checks map?
                  :fact-setup map?
                  :fact-teardown map?})))

^{:refer lang.seedgen.form-parse/seedgen-readforms :added "4.1"}
