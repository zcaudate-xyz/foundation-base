(ns lang.runtime.basic.type-oneshot-test
  (:require [lang.runtime.basic.impl.process-js :as js]
             [lang.runtime.basic.impl.process-lua :as lua]
             [lang.runtime.basic.impl.process-python :as py]
             [lang.runtime.annex.basic.impl.process-r :as r]
             [lang.runtime.basic.type-oneshot :as p]
             [std.json :as json]
             [lang.base.book :as book]
            [lang.base.emit-prep-lua-test :as prep]
            [lang.core.impl-entry :as entry]
            [lang.core.library :as lib]
            [lang.core.library-snapshot :as snap]
            [lang.core.pointer :as ptr]
             [lang.base.util :as ut]
             [lang.model.builtin.spec-js :as spec-js]
             [lang.model.builtin.spec-lua :as spec-lua]
             [lang.model.builtin.spec-python :as spec-py]
             [lang.model.annex.spec-r :as spec-r]
             [std.lib.env :as env]
             [std.string.prose :as prose])
  (:use code.test))

(def +library-ext+
  (doto (lib/library:create
         {:snapshot (snap/snapshot {:lua {:id :lua
                                          :book prep/+book-min+}})})
    (lib/install-book! (dissoc spec-lua/+book+ :parent))
    (lib/install-book! (dissoc spec-r/+book+ :parent))
    (lib/install-book! (dissoc spec-js/+book+ :parent))
    (lib/install-book! (dissoc spec-py/+book+ :parent))
    (lib/install-module! :lua 'L.util
                         {:require '[[L.core :as u]]
                          :import '[["cjson" :as cjson]]})
    (lib/add-entry-single!
     (entry/create-code-base
      '(defn sub-fn
         [a b]
         (return ((u/identity-fn u/sub) a b)))
      {:lang :lua
       :namespace (env/ns-sym)
       :module 'L.util
       :line 1}
      {}))
    (lib/add-entry-single!
     (entry/create-code-base
      '(defn add-fn
         [a b]
         (return ((u/identity-fn u/add) a (-/sub-fn b 0))))
      {:lang :lua
       :namespace (env/ns-sym)
       :module 'L.util
       :line 2}
      {}))))

(def +ptr-identity+
  (ut/lang-pointer :lua
                   {:module 'L.core
                    :id 'identity-fn
                    :section :code
                    :library +library-ext+}))

(def +ptr+
  (ut/lang-pointer :lua
                   {:module 'L.util
                    :id 'add-fn
                    :section :code
                    :library +library-ext+}))

^{:refer lang.runtime.basic.type-oneshot/invoke-ptr-oneshot.compile :adopt true :added "4.0"}
(fact "compiles the pointer body"

  (ptr/with:input []
    (-> (p/rt-oneshot {:lang :lua
                       :layout :full})
        (p/invoke-ptr-oneshot +ptr+ [1 2])))
  => (prose/|
   "local function L_core___identity_fn(x)"
   "  return x"
   "end"
   ""
   "local function L_util___sub_fn(a,b)"
   "  return L_core___identity_fn(function (x,y)"
   "    return x - y"
   "  end)(a,b)"
   "end"
   ""
   "local function L_util___add_fn(a,b)"
    "  return L_core___identity_fn(function (x,y)"
    "    return x + y"
    "  end)(a,L_util___sub_fn(b,0))"
    "end"
    ""
    "local function OUT_FN()"
    "  return L_util___add_fn(1,2)"
    "end"
    "return OUT_FN()")

  (ptr/with:raw
   (ptr/with:input []
     (-> (p/rt-oneshot {:lang :lua
                        :layout :flat})
         (p/invoke-ptr-oneshot +ptr-identity+ [1]))))
  => string?)

^{:refer lang.runtime.basic.type-oneshot/sh-exec :added "4.0"}
(fact "basic function for executing a shell process"

  (p/sh-exec ["python3" "-c"] "print(1 + 1)" {})
  => "2"

  (p/sh-exec ["luajit" "-e"] "print(1 + 1)" {})
  => "2"

  (p/sh-exec ["R" "-s" "-e"] "1 + 1" {})
  => "[1] 2")

^{:refer lang.runtime.basic.type-oneshot/raw-eval-oneshot :added "4.0"}
(fact "evaluates a raw statement with oneshot"

  (-> (p/rt-oneshot {:lang :lua})
      (p/raw-eval-oneshot "print(1 + 1)"))
  => "2"

  (-> (p/rt-oneshot {:lang :r})
      (p/raw-eval-oneshot "1 + 1"))
  => "[1] 2")

^{:refer lang.runtime.basic.type-oneshot/invoke-ptr-oneshot :added "4.0"}
(fact "gets the oneshow invoke working"

  (-> (p/rt-oneshot {:lang :lua})
      (p/invoke-ptr-oneshot +ptr-identity+ [123])
      (ptr/with:raw)
      (json/read))
  => {"return" "number", "value" 123, "type" "data"}

  (-> (p/rt-oneshot {:lang :lua
                     :program :luajit})
      (p/invoke-ptr-oneshot +ptr-identity+ [123]))
  => 123

  (-> (p/rt-oneshot {:lang :lua
                     :program :luajit
                     :layout :full})
      (p/invoke-ptr-oneshot +ptr-identity+ ['(error "Hello")])
      (ptr/with:raw)
      (json/read))
  => {"value" "[string \"local function L_core___identity_fn(x)...\"]:6: Hello", "type" "error"}


  (-> (p/rt-oneshot {:lang :lua
                     :program :luajit
                     :layout :full})
      (p/invoke-ptr-oneshot +ptr+ [10 20])
      (ptr/with:raw)
      (json/read))
  => {"return" "number", "value" 30, "type" "data"})

^{:refer lang.runtime.basic.type-oneshot/rt-oneshot-setup :added "4.0"}
(fact "helper function for preparing oneshot params"

  (p/rt-oneshot-setup :lua :resty {} nil)
  => vector?)

^{:refer lang.runtime.basic.type-oneshot/rt-oneshot:create :added "4.0"}
(fact "creates a oneshot runtime"

  (->> (p/rt-oneshot:create {:lang :lua
                             :program :luajit})
       (into {}))
  => map?)

^{:refer lang.runtime.basic.type-oneshot/rt-oneshot :added "4.0"
  :setup [(defn rt-oneshot
            [lang & [opts]]
            [(-> (p/rt-oneshot (merge {:lang lang} opts))
                 (p/invoke-ptr-oneshot (ut/lang-pointer lang {:library +library-ext+})
                                       ['(+ 1 2 3 4)])
                 (ptr/with:input))
             (-> (p/rt-oneshot (merge {:lang lang} opts))
                 (p/invoke-ptr-oneshot (ut/lang-pointer lang {:library +library-ext+})
                                       ['(+ 1 2 3 4)])
                 (ptr/with:raw)
                 (json/read))
             (-> (p/rt-oneshot (merge {:lang lang} opts))
                 (p/invoke-ptr-oneshot (ut/lang-pointer lang {:library +library-ext+})
                                       ['(+ 1 2 3 4)]))])]}
(fact "creates a oneshot runtime"

  (rt-oneshot :r)
  => ["(function (){\n  return(1 + 2 + 3 + 4);\n})()"
      {"return" "number", "value" 10, "type" "data"}
      10]

  (rt-oneshot :lua)
  => ["local function OUT_FN()\n  return 1 + 2 + 3 + 4\nend\nreturn OUT_FN()"
      {"value" 10, "type" "data", "return" "number"}
      10]

  (rt-oneshot :js)
  => ["(function (){\n  return 1 + 2 + 3 + 4;\n})()"
      {"return" "number", "value" 10, "type" "data"}
      10]

  (rt-oneshot :python)
  => [(prose/|
       "def OUT_FN():"
       "  import traceback"
       "  err = None"
       "  try:"
       "    return 1 + 2 + 3 + 4"
       "  except Exception:"
       "    err = traceback.format_exc()"
       "  raise Exception(err)"
       "(globals())[\"OUT\"] = OUT_FN()")
      {"key" nil, "id" nil, "return" "number", "value" 10, "type" "data"}
      10])

(comment
  (./import)
  (./create-tests))
