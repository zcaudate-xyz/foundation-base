(ns lang.runtime.annex.redis.eval-script-test
  (:require [clojure.string :as str]
            ...
            [lang.runtime.annex.redis.eval-script :refer :all]
...
^{:refer lang.runtime.annex.redis.eval-script/raw-compile-form :added "4.0"}
(fact "converts a ptr into a form"

  (raw-compile-form redis/scan-sub)
  => '(return (kmi.redis/scan-sub (. KEYS [1]))))

^{:refer lang.runtime.annex.redis.eval-script/raw-compile :added "4.0"}
...
^{:refer lang.runtime.annex.redis.eval-script/raw-prep-in-fn :added "4.0"}
(fact "prepares the arguments for entry"

  (raw-prep-in-fn {:rt/redis {:nkeys 1}} [:key :arg])
  => [[:key] [:arg]])

^{:refer lang.runtime.annex.redis.eval-script/raw-prep-out-fn :added "4.0"}
(fact "prepares arguments out"

  (raw-prep-out-fn {:rt/redis {:encode {:out true}}} "{\"a\":1}")
  => {:a 1})

^{:refer lang.runtime.annex.redis.eval-script/rt-install-fn :added "4.0"}
...
^{:refer lang.runtime.annex.redis.eval-script/redis-invoke-sha :added "4.0"
