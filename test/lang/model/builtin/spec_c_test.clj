...
^{:refer lang.model.builtin.spec-c/tf-define :added "4.0"}
...
^{:refer lang.model.builtin.spec-c/tf-struct :added "4.1"}
...
^{:refer lang.model.builtin.spec-c/tf-enum :added "4.1"}
(fact "transforms enum definition"

  (tf-enum '(enum Color [RED GREEN BLUE]))
  => '(:- "enum" Color (:- "{") (quote [RED GREEN BLUE]) (:- "};")))

^{:refer lang.model.builtin.spec-c/tf-typedef :added "4.1"}
(fact "transforms typedef"

  (tf-typedef '(typedef :int :my_int))
  => '(:- "typedef" :int "my_int;"))

^{:refer lang.model.builtin.spec-c/to-c-type :added "4.1"}
...
^{:refer lang.model.builtin.spec-c/tf-arrow :added "4.1"}
(fact "transforms arrow ->"

  (tf-arrow '(arrow ptr field))
  => '(:% ptr (:- "->" field)))

^{:refer lang.model.builtin.spec-c/tf-sizeof :added "4.1"}
