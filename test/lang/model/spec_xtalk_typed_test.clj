...
(fact "defspec.xt registers a spec declaration"
  ...
  (eval '(lang.typed/defspec.xt LocalId :xt/str))
  (-> (typed/spec-def (typed/load-registry) 'lang.model.spec-xtalk-typed-test/LocalId)
      :type
      types/type->data)
  => {:kind :primitive :name :xt/str})
...
(fact "defspec.xt resolves aliased type names during registration"
  (types/clear-registry!)
  (eval '(lang.typed/defspec.xt AliasMaybeFixture
           [:xt/maybe types/User]))
  (-> (typed/spec-def (typed/load-registry) 'lang.model.spec-xtalk-typed-test/AliasMaybeFixture)
      :type
      types/type->data)
  => '{:kind :maybe
       :item {:kind :named
              :name lang.typed.xtalk-common/User}})
...
(fact "infers fn:> as a zero-arg constant function"
  (-> (infer/infer-type '(lang.typed.xtalk-intrinsic/const-fn "ok")
                        {:ns 'sample.route
                         :aliases {}})
      :type
      types/type->data)
