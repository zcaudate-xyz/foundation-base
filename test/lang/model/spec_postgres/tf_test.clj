(ns lang.model.spec-postgres.tf-test
  (:require [lang.model.spec-postgres.tf :refer :all])
  (:use code.test))

^{:refer lang.model.spec-postgres.tf/pg-js-idx :added "4.0"}
(fact "ignores single letter prefix"

  (pg-js-idx 'i-hello)
  => "hello")

^{:refer lang.model.spec-postgres.tf/pg-tf-js :added "4.0"}
...
^{:refer lang.model.spec-postgres.tf/pg-tf-for :added "4.0"}
...
^{:refer lang.model.spec-postgres.tf/pg-tf-foreach :added "4.0"}
...
^{:refer lang.model.spec-postgres.tf/pg-tf-loop :added "4.0"}
(fact "creates loop"

  (pg-tf-loop '(loop (:++ i)))
  => '[:LOOP \\ (\| (do (:++ i))) \\ :END-LOOP \;])

^{:refer lang.model.spec-postgres.tf/pg-tf-throw :added "4.0"}
(fact "creates throw transform"

  (pg-tf-throw '(throw {}))
  => '[:raise-exception :using (quote [[:detail := (:text (% {}))] [:message := "nil"]])])

^{:refer lang.model.spec-postgres.tf/pg-tf-error :added "4.0"}
(fact "creates error transform"

  (pg-tf-error '(error {}))
  => '[:raise-exception :using (quote [[:detail := (:text (% {:status "error"}))] [:message := "nil"]])])

^{:refer lang.model.spec-postgres.tf/pg-tf-assert :added "4.0"}
