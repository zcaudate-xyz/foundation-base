(ns lang.model.spec-llvm-test
  (:use code.test)
  (:require [lang.model.spec-llvm :refer :all]))

^{:refer lang.model.spec-llvm/tf-define :added "4.1"}
...
^{:refer lang.model.spec-llvm/tf-declare :added "4.1"}
(fact "transforms declare forms"
  (tf-declare (list 'declare 'i32 (symbol "@puts") ['i8*]))
  => '(:- "declare" i32 "@puts(i8*)"))

^{:refer lang.model.spec-llvm/tf-label :added "4.1"}
(fact "transforms labels"
  (tf-label '(label %entry))
  => '(:- "%entry:"))

^{:refer lang.model.spec-llvm/tf-ret :added "4.1"}
...
^{:refer lang.model.spec-llvm/tf-assign :added "4.1"}
(fact "transforms assignments"
  (tf-assign '(:= %out (call i32 @next [])))
  => '(:- %out "=" (call i32 @next [])))

^{:refer lang.model.spec-llvm/tf-inst-bin :added "4.1"}
(fact "transforms binary instructions"
  ((tf-inst-bin "add") '(add i32 %a %b))
  => '(:- "add" i32 "%a," %b))

^{:refer lang.model.spec-llvm/tf-icmp :added "4.1"}
(fact "transforms icmp instructions"
  (tf-icmp '(icmp eq i32 %a %b))
  => '(:- "icmp" eq i32 "%a," %b))

^{:refer lang.model.spec-llvm/tf-br :added "4.1"}
...
^{:refer lang.model.spec-llvm/tf-store :added "4.1"}
(fact "transforms store instructions"
  (tf-store '(store i32 %value i32* %ptr))
  => '(:- "store" i32 "%value," i32* %ptr))

^{:refer lang.model.spec-llvm/tf-load :added "4.1"}
