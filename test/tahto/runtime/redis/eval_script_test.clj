(ns tahto.runtime.redis.eval-script-test
  (:require [clojure.string :as str]
             [kmi.redis :as redis]
             [lib.redis.bench :as bench]
             [lib.redis.script :as script]
             [tahto.runtime.redis.client :as r]
            [tahto.runtime.redis.eval-script :refer :all]
...
^{:refer tahto.runtime.redis.eval-script/raw-compile-form :added "4.0"}
(fact "converts a ptr into a form"

  (raw-compile-form redis/scan-sub)
  => '(return (kmi.redis/scan-sub (. KEYS [1]))))

^{:refer tahto.runtime.redis.eval-script/raw-compile :added "4.0"}
...
^{:refer tahto.runtime.redis.eval-script/raw-prep-in-fn :added "4.0"}
(fact "prepares the arguments for entry"

  (raw-prep-in-fn {:rt/redis {:nkeys 1}} [:key :arg])
  => [[:key] [:arg]])

^{:refer tahto.runtime.redis.eval-script/raw-prep-out-fn :added "4.0"}
(fact "prepares arguments out"

  (raw-prep-out-fn {:rt/redis {:encode {:out true}}} "{\"a\":1}")
  => {:a 1})

^{:refer tahto.runtime.redis.eval-script/rt-install-fn :added "4.0"}
...
^{:refer tahto.runtime.redis.eval-script/redis-invoke-sha :added "4.0"
