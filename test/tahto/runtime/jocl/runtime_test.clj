...
^{:refer tahto.runtime.jocl.runtime/kernel? :added "3.0"}
(fact "check that a code entry "

  (kernel? @sample)
  => true)

^{:refer tahto.runtime.jocl.runtime/init-exec-jocl :added "3.0"}
(fact "initialises the exec in the runtime"
  ...
  (init-exec-jocl +rt+
                  (cptr/pointer {:context :lang/c
                                 :lang :c
                                 :module 'tahto.runtime.jocl.runtime-test
                                 :section :code
                                 :id 'sample})
                  @sample)
  => exec/exec?
...
^{:refer tahto.runtime.jocl.runtime/init-ptr-jocl :added "3.0"}
(fact "initialises the pointer")

^{:refer tahto.runtime.jocl.runtime/invoke-ptr-jocl :added "3.0"}
(fact "invokes a jocl ptr (cached kernel)")

^{:refer tahto.runtime.jocl.runtime/stop-jocl :added "3.0"}
(fact "stops the runtime")

^{:refer tahto.runtime.jocl.runtime/jocl:create :added "3.0"}
(fact "creates a new runtime")

^{:refer tahto.runtime.jocl.runtime/jocl :added "3.0"}
