(ns tahto.core.script-macro-provenance-test
  (:require [tahto.common.book-module :as module]
            ...
            [tahto.model.spec-xtalk :as xtalk])
  (:use code.test))
      ...
      (let [fn-var (macro/intern-top-level-fn
        ...
        [(select-keys out [:phase
         ...
         [(first (:stack out))
          (last (:stack out))]])))
  => '[{:phase :emit/form
        :subsystem :tahto.base.emit-top-level/emit-form
        :module xt.lang.common-lib
        :entry xt.lang.common-lib/fail-type-native
        :line 320
        :form (x:type-native obj)}
       [[:emit/form :tahto.base.emit-top-level/emit-form]
