(ns tahto.model.spec-xtalk-typed-test
  (:require [clojure.string]
   [tahto.typed :as typed]
   [tahto.typed.xtalk-analysis :as analysis]
   [tahto.typed.xtalk-common :as types]
   [tahto.typed.xtalk-compat :as compat]
   [tahto.typed.xtalk-infer :as infer]
   [tahto.typed.xtalk-lower :as lower]
...
(fact "defspec.xt registers a spec declaration"
  ...
  (eval '(tahto.typed/defspec.xt LocalId :xt/str))
  (-> (typed/spec-def (typed/load-registry) 'tahto.model.spec-xtalk-typed-test/LocalId)
      :type
      types/type->data)
  => {:kind :primitive :name :xt/str})
...
(fact "defspec.xt resolves aliased type names during registration"
  (types/clear-registry!)
  (eval '(tahto.typed/defspec.xt AliasMaybeFixture
           [:xt/maybe types/User]))
  (-> (typed/spec-def (typed/load-registry) 'tahto.model.spec-xtalk-typed-test/AliasMaybeFixture)
      :type
      types/type->data)
  => '{:kind :maybe
       :item {:kind :named
              :name tahto.typed.xtalk-common/User}})
...
(fact "infers fn:> as a zero-arg constant function"
  (-> (infer/infer-type '(tahto.typed.xtalk-intrinsic/const-fn "ok")
                        {:ns 'sample.route
                         :aliases {}})
      :type
      types/type->data)
...
(fact "parses defspec.xt and merges function signatures"
  ...
  => '{:spec-count 3
       :input-types [{:kind :named :name tahto.model.spec-xtalk-typed-fixture/UserMap}
                     {:kind :primitive :name :xt/str}]
        :output {:kind :maybe
...
(fact "analyzes macro and value declarations separately"
  [(-> (parse/analyze-namespace 'xt.lang.spec-base)
       ...
       count
       pos?)
   (-> (parse/analyze-namespace 'xt.db.text.base-scope)
       :values
       count
       pos?)]
  => '[true true])
...
(fact "emits TypeScript declarations from xtalk specs"
  (let [out (ts/emit-namespace-declarations 'tahto.model.spec-xtalk-typed-fixture)]
...
(fact "checks valid xtalk functions"
  (let [ctx (typed/load-ns 'tahto.model.spec-xtalk-typed-fixture)]
    (-> (typed/function-report ctx 'tahto.model.spec-xtalk-typed-fixture/find-user)
...
(fact "reports return and call mismatches"
  (let [ctx (typed/load-ns 'tahto.model.spec-xtalk-typed-fixture)]
    [(-> (typed/function-report ctx 'tahto.model.spec-xtalk-typed-fixture/wrong-user-name)
         :errors
         first
         :tag)
     (-> (typed/function-report ctx 'tahto.model.spec-xtalk-typed-fixture/find-user-wrong-key)
         :errors
         first
         :tag)])
...
(fact "exposes typed analysis helpers"
  (let [ctx (typed/load-ns 'tahto.model.spec-xtalk-typed-fixture)]
    (typed/with-context-registry
      ctx
      #(vector (analysis/get-function-input-type 'tahto.model.spec-xtalk-typed-fixture/find-user 'id)
               (analysis/get-function-output-type 'tahto.model.spec-xtalk-typed-fixture/find-user))))
  => '[{:kind :primitive :name :xt/str}
       {:kind :maybe
        :item {:kind :named :name tahto.model.spec-xtalk-typed-fixture/User}}])
