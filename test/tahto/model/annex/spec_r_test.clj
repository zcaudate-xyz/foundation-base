...
^{:refer tahto.model.annex.spec-r/tf-defn :added "3.0"}
(fact "function declaration for R"
  (tf-defn '(defn hello [x y] (return (+ x y))))
  => '(def hello (fn [x y] (return (+ x y)))))

^{:refer tahto.model.annex.spec-r/tf-infix-if :added "4.0"}
(fact "transform for infix if"
  (tf-infix-if '(:? 1 2 3 4))
  => '((:- "`if`") 1 2 ((:- "`if`") 3 4)))

^{:refer tahto.model.annex.spec-r/tf-for-object :added "4.0"}
(fact "transform for `for:object`"
  (tf-for-object '(for:object [[k v] obj]))
  => '(for [k :in (names obj)] (:= v (. obj [k]))))

^{:refer tahto.model.annex.spec-r/tf-for-array :added "4.0"}
...
^{:refer tahto.model.annex.spec-r/tf-for-iter :added "4.0"}
(fact "transform for `for:iter`"
  (tf-for-iter '(for:iter [e it]))
  => '(for [e :in (% it)]))

^{:refer tahto.model.annex.spec-r/tf-for-index :added "4.0"}
(fact "transform for `for:index`"
  (tf-for-index '(for:index [i [0 10 3]]))
  => '(for [i :in (seq 0 10 3)]))


^{:refer tahto.model.annex.spec-r/r-map :added "4.1"}
(fact "R empty map literal"
  (r-map {} nil nil)
  => "structure(list(), names=character())")

^{:refer tahto.model.annex.spec-r/tf-formula :added "4.1"}
(fact "transform for formula"
  (tf-formula '(formula y x))
  => '(:- "y ~ x"))

^{:refer tahto.model.annex.spec-r/tf-library :added "4.1"}
(fact "transform for library"
  (tf-library '(library jsonlite))
  => '(:- "library(\"jsonlite\")"))

^{:refer tahto.model.annex.spec-r/tf-df :added "4.1"}
