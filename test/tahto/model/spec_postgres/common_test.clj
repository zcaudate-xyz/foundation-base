(ns tahto.model.spec-postgres.common-test
  (:require [tahto.model.spec-postgres :as g]
...
^{:refer tahto.model.spec-postgres.common/pg-type-alias :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-deftype-ref-name :added "4.1"}
...
^{:refer tahto.model.spec-postgres.common/pg-sym-meta :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-format :added "4.0"}
(fact "formats a form, extracting static components"

  (common/pg-format '(def ^{:- [:int]} hello 1))
  => vector?)

^{:refer tahto.model.spec-postgres.common/pg-policy-format :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-hydrate-module-static :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-hydrate :added "4.0"}
(fact "hydrate function for top level entries"

  (common/pg-hydrate '(defn foo [] 1) {} {:module {:static {:application "app" :all {:schema ["schema"]}}}})
  => vector?)

^{:refer tahto.model.spec-postgres.common/pg-current-module-link? :added "4.1"}
(fact "checks postgres module links")

^{:refer tahto.model.spec-postgres.common/pg-link-symbol :added "4.1"}
(fact "resolves postgres link symbols")

^{:refer tahto.model.spec-postgres.common/pg-resolve-entry :added "4.1"}
(fact "resolves postgres entries")

^{:refer tahto.model.spec-postgres.common/pg-string :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-uuid :added "4.1"}
(fact "constructs a pg uuid"

  (common/pg-uuid "123")
  => "'123'::uuid")

^{:refer tahto.model.spec-postgres.common/pg-map :added "4.0"}
(fact "creates a postgres json object"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "jsonb")]
    (common/pg-map {:a 1} nil nil))
  => "jsonb")

^{:refer tahto.model.spec-postgres.common/pg-set :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-linked :added "4.0"}
(fact "emits the linked symbol"
  ;; Skipping complex setup.
  )

^{:refer tahto.model.spec-postgres.common/block-do-block :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/block-do-suppress :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/block-loop-block :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/block-while-block :added "4.1"}
(fact "emits while block"
  (common/block-while-block '(= 1 1) '(do-something))
  => '[:while (= 1 1) :loop \\ (\| (do (do-something))) \\ :end-loop \;])

^{:refer tahto.model.spec-postgres.common/block-case-block :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-defenum :added "4.0"}
(fact "defenum block"
...
^{:refer tahto.model.spec-postgres.common/pg-defindex :added "4.0"}
(fact "defindex block"

  (common/pg-defindex '(defindex hello [table cols] body))
  => '[:create-index :if-not-exists hello :on table (quote ([cols])) body \;])

^{:refer tahto.model.spec-postgres.common/pg-defpolicy :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-publication-format :added "4.1"}
(fact "formats publication"
  (common/pg-publication-format '(defpublication pub [:all]))
  => vector?)

^{:refer tahto.model.spec-postgres.common/pg-defpublication :added "4.1"}
(fact "defpublication block"
  (common/pg-defpublication '(defpublication pub [:all]))
  => '(do [:drop-publication-if-exists pub]
          [:create-publication pub :for :all :tables]))

^{:refer tahto.model.spec-postgres.common/pg-subscription-format :added "4.1"}
(fact "formats subscription"
  (common/pg-subscription-format '(defsubscription sub [conn pub] {}))
  => vector?)

^{:refer tahto.model.spec-postgres.common/pg-defsubscription :added "4.1"}
...
^{:refer tahto.model.spec-postgres.common/pg-deftrigger :added "4.0"}
...
^{:refer tahto.model.spec-postgres.common/pg-defblock :added "4.0"}
