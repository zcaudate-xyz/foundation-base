(ns tahto.seedgen-test
  (:require [code.project :as project]
            [std.fs :as fs]
            [tahto.seedgen.form-bench :as form-bench]
            [tahto.seedgen.form-infile :as form-infile]
            [tahto.seedgen :refer :all])
  (:use code.test))
...
^{:refer tahto.seedgen/seedgen-root :added "4.1"}
...
^{:refer tahto.seedgen/seedgen-list :added "4.1"}
...
^{:refer tahto.seedgen/seedgen-readforms :added "4.1"}
...
^{:refer tahto.seedgen/seedgen-benchlist :added "4.1"}
...
^{:refer tahto.seedgen/seedgen-incomplete :added "4.1"}
...
^{:refer tahto.seedgen/seedgen-langremove :added "4.1"}
...
^{:refer tahto.seedgen/seedgen-langadd :added "4.1"}
...
^{:refer tahto.seedgen/seedgen-benchadd :added "4.1"}
  ...
  (let [root    (.toFile (java.nio.file.Files/createTempDirectory "seedgen-benchadd-summary"
    ...
    (try
      (spit path (str "(ns xt.sample.summary-test\n"
                      ...
                      "  (!.js (+ 1 2 3))\n"
                      "  => 6)\n"))
      (let [output (#'tahto.seedgen/seedgen-benchadd-summary
  ...
  (let [root      (.toFile (java.nio.file.Files/createTempDirectory "seedgen-benchadd-ref"
    ...
    (try
      (spit path (str "(ns xt.sample.ref-test\n"
                      ...
                      "(l/script- :js\n"
                      "  {:runtime :basic\n"
                      "   :require [[xt.net.conn-sql :as dbsql]\n"
                      "             ^{:seedgen/extra true}\n"
                      "             [js.net.conn-sqlite :as js-sqlite]]})\n\n"
...
^{:refer tahto.seedgen.form-infile/apply-item-transform-form :added "4.1"}
(fact "form transforms rewrite each original match once while preserving layout"
  (#'tahto.seedgen.form-infile/apply-item-transform-form
   "(fact \"single-pass\"\n  (!.js\n    +out+)\n  => +out+)"
   (symbol "+out+")
   '(l/as-lua +out+))
  => "(fact \"single-pass\"\n  (!.js\n    (l/as-lua +out+))\n  => (l/as-lua +out+))")
...
^{:refer tahto.seedgen/seedgen-benchremove :added "4.1"}
