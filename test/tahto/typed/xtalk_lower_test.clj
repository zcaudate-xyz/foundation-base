(ns tahto.typed.xtalk-lower-test
  (:use code.test)
  (:require [tahto.typed.xtalk-common :as types]
            [tahto.typed.xtalk-infer :as infer]
            [tahto.typed.xtalk-lower :refer :all]))

(def +ctx+ {:ns 'sample.route :aliases '{k xt.lang.common-lib}})

^{:refer tahto.typed.xtalk-lower/intrinsic-sym :added "4.1"}
(fact "builds intrinsic symbols"
  (intrinsic-sym "arrayify")
  => 'tahto.typed.xtalk-intrinsic/arrayify)

^{:refer tahto.typed.xtalk-lower/resolve-op :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/access-kind :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-dot :id lower-dot-annotated :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-dot :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-dot :id lower-dot-method-call :added "4.1"}
(fact "preserves list-shaped native method calls"
  (lower-dot '(. arr (filter pred)))
  => '(. arr (filter pred)))

^{:refer tahto.typed.xtalk-lower/lower-form :id lower-assignment-target :added "4.1"}
(fact "preserves structural access on assignment targets"
  (lower-form '(:= (. prev ["current"]) value) {})
  => '(:= (. prev ["current"]) value))

^{:refer tahto.typed.xtalk-lower/lower-fn-shorthand :added "4.1"}
(fact "lowers fn:> shorthands"
  [(lower-fn-shorthand '(fn:>))
   (lower-fn-shorthand '(fn:> [x] x))
   (lower-fn-shorthand '(fn:> "ok"))]
  => '[(tahto.typed.xtalk-intrinsic/const-fn nil)
       (fn [x] x)
       (tahto.typed.xtalk-intrinsic/const-fn "ok")])

^{:refer tahto.typed.xtalk-lower/lower-defaulted-target :added "4.1"}
(fact "applies defaulted targets"
  (lower-defaulted-target 'x:get-key '[obj "k" "fallback"])
  => '(x:get-key obj "k" "fallback"))

^{:refer tahto.typed.xtalk-lower/lower-offset-index :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-list :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-list :id preserve-runtime-helpers :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-form :id preserve-structural-forms :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-list :id preserve-value-conditionals :added "4.1"}
(fact "leaves value conditionals for target-language staging"
  (lower-list '(:? test 1 2) {:preserve-unknown true})
  => '(:? test 1 2))

^{:refer tahto.typed.xtalk-lower/lower-form :added "4.1"}
...
^{:refer tahto.typed.xtalk-lower/lower-form
  :id preserve-resolved-hard-link
  :added "4.1"}
(fact "preserves resolved hard-link targets while lowering their arguments"
  (lower-form '(xt.lang.common-data/obj-keys
                (. value ["data"]))
              {:preserve-unknown false})
  => '(xt.lang.common-data/obj-keys
       (x:get-key value "data")))
