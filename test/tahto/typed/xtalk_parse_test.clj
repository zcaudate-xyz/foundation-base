(ns tahto.typed.xtalk-parse-test
  (:use code.test)
  (:require [tahto.typed.xtalk-common :as types]
            [tahto.typed.xtalk-parse :refer :all]))
...
^{:refer tahto.typed.xtalk-parse/read-forms :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/ns-form? :added "4.1"}
(fact "detects ns forms"
  [(ns-form? '(ns sample.core))
   (ns-form? '(def x 1))]
  => [true false])

^{:refer tahto.typed.xtalk-parse/defspec? :added "4.1"}
(fact "detects defspec.xt forms"
  [(defspec? '(defspec.xt User :xt/str))
   (defspec? '(defn.xt user []))]
  => [true false])

^{:refer tahto.typed.xtalk-parse/defn? :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/defmacro? :added "4.1"}
(fact "detects defmacro.xt forms"
  [(defmacro? '(defmacro.xt f []))
   (defmacro? '(defn.xt f []))]
  => [true false])

^{:refer tahto.typed.xtalk-parse/defvalue? :added "4.1"}
(fact "detects def.xt forms"
  [(defvalue? '(def.xt Value 1))
   (defvalue? '(defn.xt f []))]
  => [true false])

^{:refer tahto.typed.xtalk-parse/parse-ns-name :added "4.1"}
(fact "extracts namespace names from form streams"
  (parse-ns-name '[(comment x) (ns sample.core) (def x 1)])
  => 'sample.core)

^{:refer tahto.typed.xtalk-parse/extract-aliases :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/parse-decl-preamble :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/parse-defspec :added "4.1"}
(fact "parses defspec forms"
  (:name (parse-defspec '(defspec.xt User :xt/str) 'sample.route {} "sample.clj"))
  => "User")

^{:refer tahto.typed.xtalk-parse/parse-callable-items :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/multi-callable-items? :added "4.1"}
(fact "detects multi-arity callable item lists"
  [(multi-callable-items? '(([a] 1) ([a b] 2)))
   (multi-callable-items? '([a] 1))]
  => [true false])

^{:refer tahto.typed.xtalk-parse/parse-defn :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/merge-spec-inputs :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/attach-function-spec :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/attach-value-spec :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/spec-map-by-kind :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/attach-specs :added "4.1"}
(fact "attaches specs across analysis outputs"
  (let [analysis (analyze-namespace 'tahto.model.spec-xtalk-typed-fixture)]
    [(some? (:spec (first (:functions analysis))))
     (= 3 (count (:specs analysis)))])
  => '[true true])

^{:refer tahto.typed.xtalk-parse/analyze-file-raw :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/analyze-file :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/register-types! :added "4.1"}
(fact "registers parsed declarations"
  (do
    (types/clear-registry!)
    (register-types! (analyze-namespace 'tahto.model.spec-xtalk-typed-fixture))
    (some? (types/get-function 'tahto.model.spec-xtalk-typed-fixture/find-user)))
  => true)

^{:refer tahto.typed.xtalk-parse/analyze-namespace-raw :added "4.1"}
(fact "looks up namespace source file and returns raw analysis"
  (let [result (analyze-namespace-raw 'tahto.model.spec-xtalk-typed-fixture)]
    [(map? result)
     (:ns result)])
...
(fact "falls back to a classpath source resource"
  (with-redefs [code.project/get-path (fn [& _] nil)]
    (:ns (analyze-namespace-raw 'xt.lang.common-data)))
  => 'xt.lang.common-data)

^{:refer tahto.typed.xtalk-parse/analyze-namespace :added "4.1"}
(fact "finds source files for namespaces"
  (count (:functions (analyze-namespace 'tahto.model.spec-xtalk-typed-fixture)))
  => 3)


^{:refer tahto.typed.xtalk-parse/existing-file-path :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/file-path-candidates :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/resolve-file-path :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/rest-arg-form? :added "4.1"}
...
^{:refer tahto.typed.xtalk-parse/rest-arg-type :added "4.1"}
