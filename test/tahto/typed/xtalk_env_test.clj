...
^{:refer tahto.typed.xtalk-env/maybe-register-function! :added "4.1"}
(fact "registers functions lazily from namespaces"
  (do
    (types/clear-registry!)
    (:name (maybe-register-function! 'tahto.model.spec-xtalk-typed-fixture/find-user)))
  => "find-user")

^{:refer tahto.typed.xtalk-env/lookup-symbol-type :added "4.1"}
(fact "looks up env or registered symbol types"
  (do
    ...
    [(lookup-symbol-type 'user-id {:env '{user-id {:kind :primitive :name :xt/str}} :ns 'sample.route :aliases {} :infer infer-type})
     ...
     (types/type->data (lookup-symbol-type 'tahto.model.spec-xtalk-typed-fixture/find-user +ctx+))])
...
^{:refer tahto.typed.xtalk-env/binding-decl :added "4.1"}
...
^{:refer tahto.typed.xtalk-env/dynamic-assignment-target? :added "4.1"}
(fact "recognizes dynamic assignment targets"
  [(dynamic-assignment-target? '(x:get-key acc tag))
   (dynamic-assignment-target? 'count)]
  => [true false])

^{:refer tahto.typed.xtalk-env/map-binding-updates :added "4.1"}
...
^{:refer tahto.typed.xtalk-env/binding-updates :added "4.1"}
