(ns tahto.base.emit-rewrite-test
  (:use code.test)
  (:require [tahto.base.emit :as emit]
            [tahto.base.emit-rewrite :refer :all]
            [tahto.typed.xtalk-analysis :as analysis]
            [tahto.typed.xtalk-infer :as infer]))

^{:refer tahto.base.emit-rewrite/stage-transforms :added "4.1"}
...
^{:refer tahto.base.emit-rewrite/canonical-stage :added "4.1"}
...
^{:refer tahto.base.emit-rewrite/canonical-stage :id canonical-stage-module-context :added "4.1"}
...
^{:refer tahto.base.emit-rewrite/canonical-stage :id canonical-stage-xtalk-entry :added "4.1"}
(fact "runs canonical lowering for XTalk entries"
  (canonical-stage '(. value [key])
                   {:mopts {:entry {:lang :xtalk}}})
  => '(. value [key]))

^{:refer tahto.base.emit-rewrite/rewrite-stage :added "4.1"}
