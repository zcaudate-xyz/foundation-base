(ns tahto.base.emit-fn-test
  (:require [tahto.base.emit-common :as common]
            [tahto.base.emit-fn :refer :all]
...
^{:refer tahto.base.emit-fn/emit-input-default :added "3.0"}
...
^{:refer tahto.base.emit-fn/emit-hint-type :added "4.0"}
(fact "emits the return type"
  (emit-hint-type {} (with-meta 'hello {:- [:char]}) nil +grammar+ {})
  => "char")

^{:refer tahto.base.emit-fn/emit-def-type :added "4.0"}
(fact "emits the def type"
  (emit-def-type (with-meta 'hello {:- [:char]}) nil +grammar+ {})
  => "char")

^{:refer tahto.base.emit-fn/emit-fn-type :added "3.0"}
...
^{:refer tahto.base.emit-fn/emit-fn-block :added "4.0"}
...
^{:refer tahto.base.emit-fn/emit-fn-preamble-args :added "4.0"}
...
^{:refer tahto.base.emit-fn/emit-fn-preamble :added "4.0"}
...
^{:refer tahto.base.emit-fn/emit-fn :added "3.0"}
...
^{:refer tahto.base.emit-fn/test-fn-loop :added "4.0"}
(fact "add blocks, fn, var and const to emit"
  (test-fn-loop '(+ 1 2) +grammar+ {})
  => "1 + 2")

^{:refer tahto.base.emit-fn/test-fn-emit :added "4.0"}
(fact  "add blocks, fn, var and const to emit"
