(ns tahto.base.grammar-test
  (:require [tahto.base.grammar :refer :all]
            [tahto.base.grammar-spec :as spec])
  (:use code.test))

^{:refer tahto.base.grammar/gen-ops :added "4.0"}
(fact "generates ops"

  (gen-ops 'tahto.base.grammar-spec "spec")
  => vector?)

^{:refer tahto.base.grammar/normalize-op-entry :added "4.1"}
(fact "normalizes shared macro and hard-link defaults"
  (normalize-op-entry
   {:op :x-add
    :emit :macro
    :macro #'tahto.base.grammar-xtalk/tf-add})
  => (contains {:value/template #'tahto.base.grammar-xtalk/tf-add
...
^{:refer tahto.base.grammar/collect-ops :added "4.0"}
(fact "collects alll ops together"

  (collect-ops +op-all+)
  => map?)

^{:refer tahto.base.grammar/ops-list :added "4.0"}
...
^{:refer tahto.base.grammar/ops-symbols :added "4.0"}
(fact "gets a list of symbols"

  (ops-symbols)
  => coll?)

^{:refer tahto.base.grammar/ops-summary :added "4.0"}
...
^{:refer tahto.base.grammar/ops-detail :added "4.0"}
(fact "get sthe detail of the ops"

  (ops-detail :macro-arrow)
  => map?)

^{:refer tahto.base.grammar/default-lookup :added "4.1"}
...
^{:refer tahto.base.grammar/build :added "4.1"}
(fact "functional core ops are selected explicitly"

  (build :include [:functional-core])
  => map?)

^{:refer tahto.base.grammar/build-min :added "4.0"}
(fact "minimum ops example for a language"

  (build-min)
  => map?)

^{:refer tahto.base.grammar/build-xtalk :added "4.0"}
(fact "xtalk ops"

  (build-xtalk)
  => map?)

^{:refer tahto.base.grammar/build:override :added "4.0"}
...
^{:refer tahto.base.grammar/build:extend :added "4.0"}
...
^{:refer tahto.base.grammar/to-reserved :added "3.0"}
...
^{:refer tahto.base.grammar/grammar-structure :added "3.0"}
...
^{:refer tahto.base.grammar/grammar-sections :added "3.0"}
...
^{:refer tahto.base.grammar/grammar-macros :added "3.0"}
(fact "process macros within the grammar"

  (grammar-macros (build-min))
  => #{:defn :defglobal :def :defrun})

^{:refer tahto.base.grammar/grammar? :added "3.0"}
