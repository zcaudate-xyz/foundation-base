(ns tahto.base.emit-common-grammar-test
  (:require [tahto.base.emit-common :as common :refer :all]
            [tahto.base.emit-data :as data :refer :all]
...
^{:refer tahto.base.emit-common/emit-comment :adopt true :added "4.0"}
(fact "emits a comment"

  (emit-comment nil '(:# "This is a comment" A B 1) +grammar+ {})
  => "// This is a comment A B 1")

^{:refer tahto.base.emit-common/emit-macro :adopt true :added "4.0"}
(fact "emits form"
...
^{:refer tahto.base.emit-common/emit-array :adopt true :added "4.0"}
(fact  "returns an array of emitted strings"

  (emit-array [1 2 3] {} {})
  => '("1" "2" "3"))

^{:refer tahto.base.emit-common/emit-wrappable? :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-wrapping :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/wrapped-str :adopt true :added "3.0"}
(fact "wrapped string using `:start` and `:end` keys of grammar"

  (wrapped-str "hello" [:data :map] +grammar+)
  => "{hello}")

^{:refer tahto.base.emit-common/emit-pre :adopt true :added "3.0"}
(fact "emits string before the arg"

  (emit-pre "!" '[x] +grammar+ {})
  => "!x")

^{:refer tahto.base.emit-common/emit-post :adopt true :added "3.0"}
(fact "emits string after the arg"

  (emit-post "--" '[x] +grammar+ {})
  => "x--")

^{:refer tahto.base.emit-common/emit-infix :adopt true :added "3.0"}
(fact "emits infix ops"

  (emit-infix "|" '[x y z] +grammar+ {})
  => "x | y | z")

^{:refer tahto.base.emit-common/emit-infix-default :adopt true :added "3.0"}
(fact "emits infix with a default value"

  (emit-infix-default "/" '[x] 1 +grammar+ {})
  => "1 / x")

^{:refer tahto.base.emit-common/emit-infix-pre :adopt true :added "3.0"}
(fact "emits infix with a default value"

  (emit-infix-pre "-" '[x] +grammar+ {})
  => "-x")

^{:refer tahto.base.emit-common/emit-infix-if-single :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-infix-if :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-between :adopt true :added "3.0"}
(fact "emits the raw symbol between two elems"

  (emit-between ":" [1 2] +grammar+ {})
  => "1:2")

^{:refer tahto.base.emit-common/emit-bi :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-assign :adopt true :added "3.0"}
(fact "emits a setter expression"

  (emit-assign ":eq" '[x 1] +grammar+ {})
  => "x :eq 1")

^{:refer tahto.base.emit-common/emit-return :adopt true :added "3.0"}
(fact "creates a return type statement"

  (emit-return "break" [1] +grammar+ {})
  => "break 1")

^{:refer tahto.base.emit-common/emit-map-key :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-map-entry :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-coll-layout :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-coll :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-data-standard :adopt true :added "4.0"}
(fact "emits either a custom string or default coll")

^{:refer tahto.base.emit-common/emit-data :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-symbol-classify :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-symbol-standard :adopt true :added "3.0"}
(fact "emits a standard symbol"

  (emit-symbol-standard 'print! +grammar+ {:layout :full})
  => "printf")

^{:refer tahto.base.emit-common/emit-symbol :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-token :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/invoke-kw-parse :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-kw-pair :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-args :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-layout :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-raw :adopt true :added "3.0"}
(fact "invoke call for reserved ops"

  (emit-invoke-raw "-" '[abc] +grammar+ {})
  => "-(abc)")

^{:refer tahto.base.emit-common/emit-invoke-static :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-typecast :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-new :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-raw :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-index-entry :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-index :adopt true :added "3.0"}
(fact "creates an indexed expression"

  (emit-index nil '[x [hello] (world foo bar) baz] +grammar+ {})
  => "x[hello].world(foo,bar).baz")

^{:refer tahto.base.emit-common/emit-quote :adopt true :added "4.0"}
(fact "emits quoted"
...
^{:refer tahto.base.emit-common/emit-table-group :adopt true :added "4.0"}
(fact "gets all the table groups"

  (emit-table-group '[:a 1 :b 3 (:.. x) (:.. y)])
  => '[[:a 1] [:b 3] (:.. x) (:.. y)])

^{:refer tahto.base.emit-common/emit-table :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-op :adopt true :added "3.0"}
...
^{:refer tahto.base.emit/form-key :adopt true :added "3.0"}
...
^{:refer tahto.base.emit-data/test-data-loop :adopt true :added "4.0"}
...
^{:refer tahto.base.emit-data/test-data-emit :adopt true :added "4.0"}
