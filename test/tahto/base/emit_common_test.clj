(ns tahto.base.emit-common-test
  (:require [tahto.base.emit-common :as common :refer :all]
            [tahto.base.emit-helper :as helper]
            [std.string.prose :as prose])
  (:use code.test))

^{:refer tahto.base.emit-common/with:explode :added "4.0"}
(fact "form to control `explode` option"
  (with:explode *explode*) => true)

^{:refer tahto.base.emit-common/with-trace :added "4.0"}
(fact "form to control `trace` option"
  (with-trace *trace*) => true)

^{:refer tahto.base.emit-common/with-compressed :added "3.0"}
(fact "formats without newlines and indents"
  (with-compressed *compressed*) => true)

^{:refer tahto.base.emit-common/with-indent :added "3.0"}
...
^{:refer tahto.base.emit-common/newline-indent :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-reserved-value :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-free-raw :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-comment :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-indent :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-macro :added "4.0"}
(fact "emits form"
...
^{:refer tahto.base.emit-common/emit-array :added "4.0"}
(fact  "returns an array of emitted strings"

  (emit-array [1 2 3] {} {})
  => '("1" "2" "3"))

^{:refer tahto.base.emit-common/emit-wrappable? :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-squash :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-wrapping :added "4.0"}
...
^{:refer tahto.base.emit-common/wrapped-str :added "3.0"}
(fact "wrapped string using `:start` and `:end` keys of grammar"

  (wrapped-str "hello" [:data :map] helper/+default+)
  => "{hello}")

^{:refer tahto.base.emit-common/emit-unit :added "4.0"}
(fact "emits a unit"
...
^{:refer tahto.base.emit-common/emit-internal :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-internal-str :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-pre :added "3.0"}
(fact "emits string before the arg"

  (emit-pre "!" '[x] helper/+default+ {})
  => "!x")

^{:refer tahto.base.emit-common/emit-post :added "3.0"}
(fact "emits string after the arg"

  (emit-post "--" '[x] helper/+default+ {})
  => "x--")

^{:refer tahto.base.emit-common/emit-prefix :added "4.0"}
(fact "emits operator before the arg"

  (emit-prefix "hello" '[x] helper/+default+ {})
  => "hello x")

^{:refer tahto.base.emit-common/emit-postfix :added "4.0"}
(fact  "emits operator before the arg"

  (emit-postfix "hello" '[x] helper/+default+ {})
  => "x hello")

^{:refer tahto.base.emit-common/emit-infix :added "3.0"}
(fact "emits infix ops"

  (emit-infix "|" '[x y z] helper/+default+ {})
  => "x | y | z")

^{:refer tahto.base.emit-common/emit-infix-default :added "3.0"}
(fact "emits infix with a default value"

  (emit-infix-default "/" '[x] 1 helper/+default+ {})
  => "1 / x")

^{:refer tahto.base.emit-common/emit-infix-pre :added "3.0"}
(fact "emits infix with a default value"

  (emit-infix-pre "-" '[x] helper/+default+ {})
  => "-x")

^{:refer tahto.base.emit-common/emit-infix-if-single :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-infix-if :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-between :added "3.0"}
(fact "emits the raw symbol between two elems"

  (emit-between ":" [1 2] helper/+default+ {})
  => "1:2")

^{:refer tahto.base.emit-common/emit-bi :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-assign :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-return-do :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-return-base :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-return :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-with-global :added "4.0"}
(fact "customisable emit function for global vars"

  (emit-with-global nil '(!:G HELLO) {} {})
  => "HELLO")

^{:refer tahto.base.emit-common/emit-symbol-classify :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-symbol-standard :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-symbol :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-token :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-with-decorate :added "4.0"}
(fact "customisable emit function for global vars"

  (emit-with-decorate nil '(!:decorate {:id 1} HELLO) {} {})
  => "HELLO")

^{:refer tahto.base.emit-common/emit-with-uuid :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-with-rand :added "4.0"}
(fact "injects uuid for testing"
...
^{:refer tahto.base.emit-common/invoke-kw-parse :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-kw-pair :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-args :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-layout :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-raw :added "3.0"}
(fact "invoke call for reserved ops"

  (emit-invoke-raw "-" '[abc] helper/+default+ {})
  => "-(abc)")

^{:refer tahto.base.emit-common/emit-invoke-static :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke-typecast :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-invoke :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-new :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-class-static-invoke :added "4.0"}
(fact "creates "
...
^{:refer tahto.base.emit-common/emit-index-entry :added "3.0"}
...
^{:refer tahto.base.emit-common/form-key :added "3.0"}
...
^{:refer tahto.base.emit-common/emit-common-loop :added "4.0"}
...
^{:refer tahto.base.emit-common/emit-common :added "4.0"}
