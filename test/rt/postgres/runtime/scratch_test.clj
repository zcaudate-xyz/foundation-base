(ns rt.postgres.test.scratch-v1-test
  (:require [rt.postgres]
...
^{:refer rt.postgres.test.scratch-v1/as-array :added "4.0"}
(fact "returns a jsonb array"

  (l/emit-as :postgres `[(scratch/as-array "{}")])
  => string?)

^{:refer rt.postgres.test.scratch-v1/TaskCache :added "4.0"}
(fact "constructs a task cache")

^{:refer rt.postgres.test.scratch-v1/Task :added "4.0"}
(fact "constructs a task")

^{:refer rt.postgres.test.scratch-v1/Entry :added "4.0"}
(fact "construcs an entry")

^{:refer rt.postgres.test.scratch-v1/as-upper :added "4.0"}
(fact "converts to upper case"

  (l/emit-as :postgres `[(scratch/as-upper "abc")])
  => "\"scratch\".as_upper('abc')")

^{:refer rt.postgres.test.scratch-v1/ping :added "4.0"}
(fact "tests that the db is working"

  (l/emit-as :postgres `[(scratch/ping)])
  => "\"scratch\".ping()")

^{:refer rt.postgres.test.scratch-v1/ping-ok :added "4.0"}
(fact "tests that the db is working with json"

  (l/emit-as :postgres `[(scratch/ping-ok)])
  => "\"scratch\".ping_ok()")

^{:refer rt.postgres.test.scratch-v1/echo :added "4.0"}
