(ns rt.postgres.base.grammar.form-let-test
  (:require [clojure.string]
            [rt.postgres.base.grammar.common :as common]
            [rt.postgres.base.grammar.form-let :refer :all]
            [std.lang :as l]
...
^{:refer rt.postgres.base.grammar.form-let/pg-tf-let-check-body :added "4.0"}
...
^{:refer rt.postgres.base.grammar.form-let/pg-do-block :added "4.0"}
(fact "emits a block with let usage"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "block")]
    (pg-do-block '(do:block (let [a 1] (return a))) nil nil))
  => "block")

^{:refer rt.postgres.base.grammar.form-let/pg-do-suppress :added "4.0"}
...
^{:refer rt.postgres.base.grammar.form-let/pg-loop-block :added "4.0"}
(fact "creates a loop block"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "block")]
    (pg-loop-block '(loop [] (let [a 1] (return a))) nil nil))
  => "block")

^{:refer rt.postgres.base.grammar.form-let/pg-while-block :added "4.0"}
(fact "creates a while block"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "block")]
    (pg-while-block '(while (true) (let [a 1] (return a))) nil nil))
  => "block")

^{:refer rt.postgres.base.grammar.form-let/pg-case-block :added "4.0"}
