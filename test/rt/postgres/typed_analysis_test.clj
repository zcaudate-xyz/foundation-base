(ns rt.postgres.typed-analysis-test
...
^{:refer rt.postgres.typed-analysis/analyze-file :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/analyze-namespace :added "4.1"}
(fact "analyze-namespace analyzes a namespace and returns type definitions"
  (let [result (typed-analysis/analyze-namespace 'rt.postgres.grammar.typed-common)]
    (contains? result :tables) => true
    (contains? result :enums) => true
    (contains? result :functions) => true))

^{:refer rt.postgres.typed-analysis/analyze-and-register! :added "4.1"}
(fact "analyze-and-register! analyzes and registers types from a namespace"
  (typed/clear-registry!)
  (let [result (typed-analysis/analyze-and-register! 'rt.postgres.grammar.typed-common)]
    ;; Result is the analysis map with tables, enums, functions
    (contains? result :tables) => true
    (contains? result :enums) => true
    (contains? result :functions) => true))

^{:refer rt.postgres.typed-analysis/make-function-report :added "4.1"}
(fact "make-function-report returns a function-level infer report"
  (let [report (typed-analysis/make-function-report 'rt.postgres.script.test.scratch-v2
                                           'insert-entry)]
    (get-in report [:function :name]) => "insert-entry"
    (get-in report [:analysis :mutating]) => true
    (get-in report [:analysis :source-tables]) => ["Entry"]))

^{:refer rt.postgres.typed-analysis/get-function-report :added "4.1"}
(fact "get-function-report lazily retrieves and caches infer reports for registered functions"
  ...
  (let [analysis (-> 'rt.postgres.script.test.scratch-v2
    ...
    (let [report (typed-analysis/get-function-report 'rt.postgres.script.test.scratch-v2/insert-entry)]
      (get-in report [:function :name]) => "insert-entry"
      (get-in report [:analysis :mutating]) => true)))

^{:refer rt.postgres.typed-analysis/Type :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/inferred->shape :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/format-shape :added "4.1"}
(fact "format-shape dispatches by output format"
  (let [shape (types/make-jsonb-shape {:id {:type :uuid}} "Entry")]
    (typed-analysis/format-shape shape :shape) => shape
    (typed-analysis/format-shape shape :openapi) => {:type "object"
                                            :properties {"id" {:type "string"
                                                                :format "uuid"}}
                                            :required ["id"]}))

^{:refer rt.postgres.typed-analysis/get-app-function-report :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/get-function-report-json :added "4.1"}
(fact "get-function-report-json serializes reports for registered functions"
  (typed/clear-registry!)
  (-> 'rt.postgres.script.test.scratch-v2
      parse/analyze-namespace
      parse/register-types!)
  (let [output (typed-analysis/get-function-report-json 'rt.postgres.script.test.scratch-v2/insert-entry
                                               true)]
    (string? output) => true
    (clojure.string/includes? output "\"insert-entry\"") => true))

^{:refer rt.postgres.typed-analysis/get-app-function-report-json :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/get-function-def :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/fn-ref->fn-sym :added "4.1"}
(fact "fn-ref->fn-sym delegates to the resolve layer"
  (typed-analysis/fn-ref->fn-sym #'clojure.string/blank?) => 'clojure.string/blank?)

^{:refer rt.postgres.typed-analysis/resolve-function-def :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/get-app-function-def :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/with-app-typed-registry :added "4.1"}
  ...
  (let [table (types/make-table-def "demo" "Entry" [] :id)]
    (with-redefs [rt.postgres.grammar.common-application/app-typed
                  (fn [_] {:tables {'demo/Entry table}
                           :enums {}
                           :functions {}})]
      (let [seen (typed-analysis/with-app-typed-registry
                  "demo"
                  (fn [_]
...
^{:refer rt.postgres.typed-analysis/get-function-input-shape :added "4.1"}
(fact "get-function-input-shape projects update columns and keeps helper-chain narrowing intact"
  ...
  (let [user-columns [(types/make-column-def :id
    ...
    (let [update-shape (typed-analysis/get-function-input-shape 'test.ns/update-user-raw 'm)
          set-shape (typed-analysis/get-function-input-shape 'test.ns/set-user 'm)
          public-shape (typed-analysis/get-function-input-shape 'test.ns/user-set-public 'm)
          public-type (typed-analysis/Type public-def)]
...
^{:refer rt.postgres.typed-analysis/get-app-function-input-shape :added "4.1"}
(fact "get-app-function-input-shape infers jsonb input shape from an app typed payload"
  ...
  (let [task-columns [(types/make-column-def :id
    ...
    (with-redefs [rt.postgres.grammar.common-application/app-typed (fn [_] typed-payload)]
      (let [shape (typed-analysis/get-app-function-input-shape "demo" 'test.ns/insert-task-raw 'm)]
        (types/jsonb-shape? shape) => true
        (-> shape :fields keys set) => #{:id :status}))))

^{:refer rt.postgres.typed-analysis/get-function-input-schema :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/get-app-function-input-schema :added "4.1"}
(fact "get-app-function-input-schema formats inferred app input shapes"
  (let [task-columns [(types/make-column-def :id
        ...
        typed-payload {:tables {'test.ns/Task task-table}
                       ...
                       :functions {'test.ns/insert-task-raw fn-def}}]
    (with-redefs [rt.postgres.grammar.common-application/app-typed (fn [_] typed-payload)]
      (get-in (typed-analysis/get-app-function-input-schema "demo"
      ...
      (string? (typed-analysis/get-app-function-input-schema "demo"
                                                    'test.ns/insert-task-raw
                                                    'm
                                                    :typescript))
      => true)))

^{:refer rt.postgres.typed-analysis/get-function-output-shape :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/get-app-function-output-shape :added "4.1"}
(fact "get-app-function-output-shape infers output shapes from an app typed payload"
  (let [form '(defn.pg
    ...
    (with-redefs [rt.postgres.grammar.common-application/app-typed (fn [_] typed-payload)]
      (let [shape (typed-analysis/get-app-function-output-shape "demo" 'test.ns/prepare-topic)]
        (types/jsonb-shape? shape) => true
...
^{:refer rt.postgres.typed-analysis/get-function-output-schema :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/get-app-function-output-schema :added "4.1"}
(fact "get-app-function-output-schema formats inferred app output shapes"
  (let [form '(defn.pg
        ...
        typed-payload {:tables {}
                       ...
                       :functions {'test.ns/prepare-topic fn-def}}]
    (with-redefs [rt.postgres.grammar.common-application/app-typed (fn [_] typed-payload)]
      (get-in (typed-analysis/get-app-function-output-schema "demo"
      ...
      (string? (typed-analysis/get-app-function-output-schema "demo"
                                                     'test.ns/prepare-topic
                                                     :typescript))
      => true)))

^{:refer rt.postgres.typed-analysis/report-json :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/make-function-json :added "4.1"}
(fact "make-function-json returns serialized analysis for a function"
  (let [output (typed-analysis/make-function-json 'rt.postgres.script.test.scratch-v2
                                         'insert-entry
                                         true)]
    (string? output) => true
...
^{:refer rt.postgres.typed-analysis/make-openapi :added "4.1"}
(fact "make-openapi generates OpenAPI spec for a namespace"
  (let [spec (typed-analysis/make-openapi 'rt.postgres.script.test.scratch-v2 (constantly true))]
    (contains? spec :openapi) => true
    (contains? spec :info) => true
    (contains? spec :paths) => true
    (contains? spec :components) => true))

^{:refer rt.postgres.typed-analysis/make-json-schema :added "4.1"}
(fact "make-json-schema generates JSON Schema definitions"
  (typed/clear-registry!)
  (typed-analysis/analyze-and-register! 'rt.postgres.script.test.scratch-v2)
  (let [schema (typed-analysis/make-json-schema)]
    (map? schema) => true))

^{:refer rt.postgres.typed-analysis/make-typescript :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/app-name-from-static :added "4.1"}
(fact "app-name-from-static normalizes app metadata"
  (typed-analysis/app-name-from-static ["demo"]) => "demo"
  (typed-analysis/app-name-from-static "demo") => "demo")

^{:refer rt.postgres.typed-analysis/fn-ref->app-name :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/arg-type :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/arg-type-name :added "4.1"}
(fact "arg-type-name returns the string name of the argument type"
  (typed-analysis/arg-type-name {:type :uuid}) => "uuid")

^{:refer rt.postgres.typed-analysis/track-arg-role? :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/enrich-function-arg-roles :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/jsonb-arg? :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/format-primitive :added "4.1"}
...
^{:refer rt.postgres.typed-analysis/build-input-schemas :added "4.1"}
(fact "build-input-schemas formats inputs and inferred jsonb shapes"
  (let [fn-def (types/make-fn-def
                ...
                nil)]
    (contains? (typed-analysis/build-input-schemas fn-def :shape) :m) => true
    (get-in (typed-analysis/build-input-schemas fn-def :shape) [:m :shape :fields :id :type])
    => :jsonb
    (get-in (typed-analysis/build-input-schemas fn-def :shape) [:name :schema]) => {:type :text}))

^{:refer rt.postgres.typed-analysis/build-output-schema :added "4.1"}
(fact "build-output-schema wraps the inferred output shape and schema"
  (let [analysis (-> 'rt.postgres.script.test.scratch-v2
                     parse/analyze-namespace
                     parse/register-types!)
        fn-def (some #(when (= "insert-entry" (:name %)) %) (:functions analysis))
        out (typed-analysis/build-output-schema fn-def :shape)]
  ...
  (let [table (types/make-table-def "test" "Foo"
    ...
    (with-redefs [rt.postgres.grammar.common-application/app
                  ...
                  (fn [_] {:tables {'test/Foo table}
                           :enums {}
                           :functions {}})]
      (contains? (get-in (typed-analysis/Type 'test.ns/insert-foo)
                         [:output :shape :fields])
                 :extra)
      => true)))
