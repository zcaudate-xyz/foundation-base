...
(fact "executes a statement with role (single)"
  ...
  => '(rt.postgres.supabase/process-return (!.pg (try [:set-local-role rt.postgres.supabase-test/anon] (let [(:integer out) (clojure.core/+ 1 2 3)] (return out)) (catch others (return {:code SQLSTATE, :message SQLERRM}))))))
...
(fact "executes a statement with role"
  ...
  => '(rt.postgres.supabase/with-role-single [rt.postgres.supabase-test/anon :integer] (clojure.core/+ 1 2 3)))
...
(fact "executes a statement with auth (single)"
  ...
  => '(rt.postgres.supabase/process-return (!.pg (try [:set-local-role authenticated] [:perform (set-config "request.jwt.claim.sub" (:text "user") true)] (let [(:integer out) (clojure.core/+ 1 2 3)] (return out)) (catch others (return {:code SQLSTATE, :message SQLERRM}))))))
...
(fact "executes a statement with auth"
  ...
  (macroexpand-1
   `(s/with-auth ["00000000-0000-0000-0000-000000000000"
                  :integer]
      (+ 1 2 3)))
  => '(rt.postgres.supabase/with-auth-single ["00000000-0000-0000-0000-000000000000" :integer] (clojure.core/+ 1 2 3)))
...
(fact "executes a statement with super (single)"
  ...
  => '(rt.postgres.supabase/process-return (!.pg (try [:set-local-role authenticated] [:perform (set-config "request.jwt.claim.sub" "user" true)] [:perform (set-config "request.jwt.claims" (:text {:sub "user", :user_metadata {:super true}}) true)] (let [(:integer out) (clojure.core/+ 1 2 3)] (return out)) (catch others (return {:code SQLSTATE, :message SQLERRM}))))))
...
(fact "executes a statement with super"
  ...
  => '(rt.postgres.supabase/with-super-single ["user" :integer] (clojure.core/+ 1 2 3)))
