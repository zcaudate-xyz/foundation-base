...
^{:refer rt.postgres.grammar.form-defpartition/pg-partition-hydrate :added "4.0"}
(fact "hydrates the partition form"
  (form-defpartition/pg-partition-hydrate
   '(defpartition part [parent] [{:use :col :in ["a"]}])
   nil
   {:module {:static {:application "app" :all {:schema ["schema"]}}}})
  => vector?)
