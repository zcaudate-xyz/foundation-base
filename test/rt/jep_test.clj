...
^{:refer rt.jep/make-interpreter :added "3.0"
  :setup [(mapv (fn [itp]
                  (std.lib.foundation/suppress (jep/close-interpreter itp)))
                @jep/*interpreters*)]}
...
^{:refer rt.jep/eval-command-jep :added "3.0"
  :setup    [(def +jep+ (std.lib.component/start (jep/rt-jep:create {})))]
  :teardown [(std.lib.component/stop +jep+)]}
...
(fact "stops the jep runtime"
  (let [jep (std.lib.component/start (rt-jep:create {}))]
    (stop-jep jep)
    ;; check if stopped?
    ))
