(ns xt.old.sys.cache-common-test
  (:require [rt.nginx.script :as script]
            [xt.lang.common-notify :as notify]
            [std.lang :as l])
  (:use code.test))

(l/script- :js
  {:runtime :basic
   :require [[xt.lang.spec-base :as xt]
             [xt.lang.common-lib :as k]
             [xt.lang.common-repl :as repl]
             [xt.old.sys.cache-common :as cache]]})

(defn create-resty-params
  "creates default resty params"
  {:added "4.0"}
  ([]
   (script/write [[:client-body-buffer-size "1m"]
                  [:variables-hash-max-size 2048]
                  [:variables-hash-bucket-size 128]
                  [:lua-shared-dict [:GLOBAL "20k"]]
                  [:lua-shared-dict [:WS_DEBUG "20k"]]
                  [:lua-shared-dict [:ES_DEBUG "20k"]]])))

(l/script- :lua
  {:runtime :basic
   :config  {:exec ["resty" "--http-conf" "client_body_buffer_size 1m;\nvariables_hash_max_size 2048;\nvariables_hash_bucket_size 128;\nlua_shared_dict GLOBAL 20k;\nlua_shared_dict WS_DEBUG 20k;\nlua_shared_dict ES_DEBUG 20k;" "-e"]}
   :require [[xt.lang.spec-base :as xt]
             [xt.old.sys.cache-common :as cache]]})

(fact:global
  {:setup    [(l/rt:restart)
              (notify/wait-on [:js 5000]
                (:= (!:G window)  {})
                (:= (!:G LocalStorage)  (. (require "node-localstorage")
                                           LocalStorage))
                (:= window.localStorage (new LocalStorage "./test-scratch/localstorage"))
                (repl/notify true))]
  :teardown [(l/rt:stop)]})

^{:refer xt.old.sys.cache-common/cache :added "4.0"}
(fact "gets a cache"

  (!.js
   (var cache (cache/cache :GLOBAL))
   (cache/flush     cache)
   (cache/list-keys cache))
  => []

  (!.lua
   (cache/flush     (cache/cache :GLOBAL))
   (cache/list-keys (cache/cache :GLOBAL)))
  => {})

^{:refer xt.old.sys.cache-common/list-keys :added "4.0"}
(fact "lists keys in the cache"

  (!.js
   (var cache (cache/cache :GLOBAL))
   (cache/set cache "A" 1)
   (cache/set cache "B" 2)
   (cache/list-keys cache))
  => ["A" "B"]

  (!.lua
   (var cache (cache/cache :GLOBAL))
   (cache/set cache "A" 1)
   (cache/set cache "B" 2)
   (cache/list-keys cache))
  => ["A" "B"])

^{:refer xt.old.sys.cache-common/flush :added "4.0"}
(fact "clears all keys in the cache"

  (!.js
   (var cache (cache/cache :GLOBAL))
   (xt/for:array [k ["A" "B" "C" "D" "E"]]
     (cache/set cache k k))
   (cache/flush cache)
   (cache/get-all cache))
  => {}

  (!.lua
   (var cache (cache/cache :GLOBAL))
   (xt/for:array [k ["A" "B" "C" "D" "E"]]
     (cache/set cache k k))
   (cache/flush cache)
   (cache/get-all cache))
  => {})

^{:refer xt.old.sys.cache-common/get :added "4.0"}
(fact "gets a cache entry")

^{:refer xt.old.sys.cache-common/set :added "4.0"}
(fact "sets a cache entry")

^{:refer xt.old.sys.cache-common/del :added "4.0"}
(fact "removes a cache entry")

^{:refer xt.old.sys.cache-common/incr :added "4.0"}
(fact "increments the cache key"

  (!.js
   (var cache (cache/cache :GLOBAL))
   (cache/set cache "A" 1)
   (cache/incr cache "A" 10))
  => 11

  (!.lua
   (var cache (cache/cache :GLOBAL))
   (cache/set cache "A" 1)
   (cache/incr cache "A" 10))
  => 11)

^{:refer xt.old.sys.cache-common/get-all :added "4.0"}
(fact "gets the cache map"

  (!.js
   (var cache (cache/cache :GLOBAL))
   (cache/flush cache)
   (xt/for:array [k ["A" "B" "C" "D" "E"]]
     (cache/set cache k k))
   (cache/get-all cache))
  => {"E" "E", "C" "C", "B" "B", "A" "A", "D" "D"}

  (!.lua
   (var cache (cache/cache :GLOBAL))
   (cache/flush cache)
   (xt/for:array [k ["A" "B" "C" "D" "E"]]
     (cache/set cache k k))
   (cache/get-all cache))
  => {"E" "E", "C" "C", "B" "B", "A" "A", "D" "D"})

^{:refer xt.old.sys.cache-common/meta-key :added "4.0"}
(fact "constructs a meta key"

  (!.js
   (cache/meta-key "hello"))
  => "__meta__:hello"

  (!.lua
   (cache/meta-key "hello"))
  => "__meta__:hello")

^{:refer xt.old.sys.cache-common/meta-get :added "4.0"}
(fact  "gets the meta map"

  (!.js
   (var cache (cache/cache :GLOBAL))
   (cache/flush cache)
   [(cache/meta-get "task")
    (cache/meta-update "task"
                       (fn:> [m]
                             (xt/x:set-key m "A" 1)
                             (return m)))
    (cache/meta-assoc "task" "B" 2)
    (cache/meta-dissoc "task" "A")])
  => [{}
      {"A" 1}
      {"B" 2, "A" 1}
      {"B" 2}]

  (!.lua
   (var cache (cache/cache :GLOBAL))
   (cache/flush cache)
   [(cache/meta-get "task")
    (cache/meta-update "task"
                       (fn:> [m]
                             (xt/x:set-key m "A" 1)
                             (return m)))
    (cache/meta-assoc "task" "B" 2)
    (cache/meta-dissoc "task" "A")])
  => [{}
      {"A" 1}
      {"B" 2, "A" 1}
      {"B" 2}])

^{:refer xt.old.sys.cache-common/meta-update :added "4.0"}
(fact "updates the meta map")

^{:refer xt.old.sys.cache-common/meta-assoc :added "4.0"}
(fact "adds a key to the meta")

^{:refer xt.old.sys.cache-common/meta-dissoc :added "4.0"}
(fact "dissocs a key from the meta")
