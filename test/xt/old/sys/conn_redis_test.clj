...
^{:refer xt.old.sys.conn-redis/connect :added "4.0"}
...
^{:refer xt.old.sys.conn-redis/exec :added "4.0"}
...
^{:refer xt.old.sys.conn-redis/create-subscription :added "4.0"
...
^{:refer xt.old.sys.conn-redis/create-psubscription :added "4.0"
  :setup [(bench/stop-redis-array [17001])
          (bench/start-redis-array [17001])
          (l/rt:restart)]}
...
^{:refer xt.old.sys.conn-redis/eval-body :added "4.0"}
...
^{:refer xt.old.sys.conn-redis/eval-script :added "4.0"}
(fact "evaluates sha, then body if errored"
  ...
  (!.lua
   ...
   [(redis/exec conn "flushdb" [])
    (redis/eval-script conn {:sha (@! (security/sha1 "return 1"))
                             :body "return 1"}
                       [])])
  => ["OK" 1]
  ...
  (do
    ...
    [(notify/wait-on :js
     ...
     (notify/wait-on :js
       (redis/eval-script conn {:sha (@! (security/sha1 "return 2"))
                                :body "return 2"}
                          []
                          (repl/<!)))])
  => ["OK" 2])