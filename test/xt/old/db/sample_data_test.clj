...
(l/script :postgres
  {:require [[rt.postgres :as pg]]
   :static {:application ["xt.old.db.sample"]
...
(defsel.pg ^{:- [-/Currency]
             ...
             :args [:text i-type]
             :api/view true}
  currency-by-type {:type (++ i-type xt.old.db.sample-data-test/EnumCurrencyType)})

(defsel.pg ^{:- [-/Currency]
             :scope #{:public}
             :args [:citext i-iso]
             :api/view true}
  currency-by-country {:id [:in (rt.postgres/t:select xt.old.db.sample-data-test/RegionCountry
