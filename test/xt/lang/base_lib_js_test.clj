...
(l/script- :js {:runtime :basic, :require [[xt.lang.base-lib :as k] [xt.lang.base-macro :as km]]})
...
(fact "pads string with n chars on right" (!.js (k/pad-right "000" 5 "-")) => "000--")
...
(fact "emits index from new array" (!.js (k/arr-omit ["a" "b" "c" "d"] 2)) => ["a" "b" "d"])
...
(fact "maps a function across an array" (!.js (k/arr-map [1 2 3 4 5] km/inc)) => [2 3 4 5 6])
...
(fact "applies a filter across an array" (!.js (k/arr-filter [1 2 3 4 5] km/odd?)) => [1 3 5])
...
(fact "performs right reduce" (!.js (k/arr-foldr [1 2 3 4 5] k/step-push [])) => [5 4 3 2 1])
...
(fact "joins array with string" (!.js (k/arr-join ["1" "2" "3" "4"] " ")) => "1 2 3 4")
...
(fact "gets a random element from array" (!.js (k/arr-random [1 2 3 4])) => #{1 4 3 2})

^{:refer xt.lang.base-lib/arr-normalise, :added "4.0"}
(fact "normalises array elements to 1" (!.js (k/arr-normalise [1 2 3 4])) => [0.1 0.2 0.3 0.4])
...
(fact "clones an object" (!.js (k/obj-clone {:a 1, :b 2, :c 3})) => {"a" 1, "b" 2, "c" 3})
...
(fact "deletes multiple keys" (!.js (k/obj-del {:a 1, :b 2, :c 3} ["a" "b"])) => {"c" 3})
...
(fact "select keys in object" (!.js (k/obj-pick {:a 1, :b 2, :c 3} ["a" "b"])) => {"a" 1, "b" 2})

^{:refer xt.lang.base-lib/obj-omit, :added "4.0"}
(fact "new object with missing keys" (!.js (k/obj-omit {:a 1, :b 2, :c 3} ["a" "b"])) => {"c" 3})
...
(fact "creates a get-spec of a datastructure" (!.js (k/get-spec (@! +in+))) => +out+)
...
(fact "adds an entry to the log" (!.js (k/trace-log-add "hello" "hello" {})) => number?)
