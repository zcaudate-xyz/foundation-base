...
(l/script :xtalk {:require [[xt.lang.common-lib :as k]]})
...
(def +tree+ (pg/bind-schema (:schema +app+)))

(def.xt Schema (@! +tree+))

(def.xt SchemaCurrency (@! {"Currency" (get-in +tree+ ["Currency"])}))

(def.xt SchemaLookup (@! (pg/bind-app +app+)))
