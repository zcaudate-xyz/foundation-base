(ns
 xtbench.python.db.base-util-test
...
(fact
 "collect routes"
 ^{:hidden true}
 (!.py (ut/collect-routes (@! +routes+) "db"))
 =>
 +result+)
...
(fact
 "keeps given limit"
 ^{:hidden true}
 (!.py (ut/keepf-limit [1 2 3 4 5] xt/x:odd? k/identity 3))
 =>
 [1 3 5])
...
(fact
 "helper for lu-map"
 ^{:hidden true}
 (!.py (ut/lu-nested [{:id "1"}] (fn [e] (return (. e ["id"])))))
 =>
 {"1" {"id" "1"}})
