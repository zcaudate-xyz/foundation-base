(ns
 xtbench.python.runtime.common-hash-test
...
(fact
 "hashes an iterator"
 ^{:hidden true}
 (!.py (hash/hash-iter (it/range [0 100]) hash/hash-native))
 =>
 3373073)
...
(fact
 "hashes an integer"
 ^{:hidden true}
 (!.py [(hash/hash-integer 1) (hash/hash-integer 16777217)])
 =>
 [1 1])
...
(fact
 "hashes a boolean"
 ^{:hidden true}
 (!.py [(hash/hash-boolean true) (hash/hash-boolean false)])
 =>
 [1 -1])
...
(fact
 ...
 ^{:hidden true}
 (!.py [(hash/native-class "abc") (hash/native-class {"::" "demo"})])
 =>
 ["string" "demo"])
