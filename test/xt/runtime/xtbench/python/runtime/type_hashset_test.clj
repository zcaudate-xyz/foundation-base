(ns
 xtbench.python.runtime.type-hashset-test
...
(fact
 "creates a hashset and removes duplicates"
 ^{:hidden true}
 (!.py
...
(fact
 "finds existing values and preserves nil members"
 ^{:hidden true}
 (!.py
  (var out (-> hs/EMPTY_HASHSET (hs/hashset-push nil)))
  [(hs/hashset-has? out nil) (== nil (hs/hashset-find out "missing"))])
 =>
 [true true])
...
(fact
 ...
 (!.py
  ...
  [(hs/hashset-has? s0 "b") (hs/hashset-has? s1 "b") (. s1 _size)])
 =>
 [false true 2])
...
(fact
 "compares and hashes sets independent of insertion order"
 ^{:hidden true}
 (!.py
  (var s1 (hs/hashset "a" "b"))
  (var s2 (hs/hashset "b" "a"))
  [(hs/hashset-eq s1 s2) (== (. s1 (hash)) (. s2 (hash)))])
 =>
 [true true])
...
(fact
 ...
 ^{:hidden true}
 (!.py (hs/hashset-to-array (hs/hashset "a" "b")))
 =>
 ["b" "a"])

^{:refer xt.runtime.type-hashset/hashset-to-iter, :added "4.1"}
(fact
 "iterates over the hashset values"
 ^{:hidden true}
 (!.py (it/arr< (hs/hashset-to-iter (hs/hashset "a" "b"))))
 =>
 ["b" "a"])

^{:refer xt.runtime.type-hashset/hashset-new, :added "4.1"}
(fact
 "creates a bare hashset from an underlying hashmap"
 ^{:hidden true}
 (!.py
  (var out (hs/hashset-new hm/EMPTY_HASHMAP nil))
  [(. out ["::"]) (. out _size) (. (. out _map) ["::"])])
 =>
 ["hashset" 0 "hashmap"])

^{:refer xt.runtime.type-hashset/hashset-empty, :added "4.1"}
(fact
 "creates an empty hashset from the current hashset"
 ^{:hidden true}
 (!.py
  (var out (hs/hashset-empty (hs/hashset "a")))
  [(. out _size) (hs/hashset-show out)])
 =>
 [0 "#{}"])

^{:refer xt.runtime.type-hashset/hashset-is-editable, :added "4.1"}
(fact
 "detects mutable vs persistent hashsets"
 ^{:hidden true}
 (!.py
...
(fact
 "converts a mutable hashset back into a persistent hashset"
 ^{:hidden true}
 (!.py
  (var
   out
   (->
    ...
    (hs/hashset-push! "b")
    (hs/hashset-to-persistent!)))
  [(hs/hashset-is-editable out) (hs/hashset-has? out "b")])
 =>
 [false true])
...
^{:refer xt.runtime.type-hashset/hashset-push!, :added "4.1"}
(fact
 "mutably adds values without growing for duplicates"
 ^{:hidden true}
 (!.py
  ...
  (hs/hashset-push! out "b")
  [(hs/hashset-has? out "a") (hs/hashset-has? out "b") (. out _size)])
 =>
 [true true 2])

^{:refer xt.runtime.type-hashset/hashset-dissoc, :added "4.1"}
(fact
 "keeps persistent removals immutable"
 ^{:hidden true}
 (!.py
  (var s0 (hs/hashset "a" "b"))
  (var s1 (hs/hashset-dissoc s0 "a"))
  [(hs/hashset-has? s0 "a") (hs/hashset-has? s1 "a") (. s1 _size)])
 =>
 [true false 1])

^{:refer xt.runtime.type-hashset/hashset-dissoc!, :added "4.1"}
(fact
 "mutably removes values and ignores missing values"
 ^{:hidden true}
 (!.py
  (var out (-> (hs/hashset "a" "b") (hs/hashset-to-mutable!)))
...
(fact
 ...
 (!.py
  (var s1 (hs/hashset "a" "b"))
  (var s2 (hs/hashset "b" "a"))
  [(== (hs/hashset-hash s1) (. s1 (hash)))
   (== (hs/hashset-hash s1) (hs/hashset-hash s2))])
 =>
 [true true])

^{:refer xt.runtime.type-hashset/hashset-show, :added "4.1"}
(fact
 "shows the hashset as an EDN-like set string"
 ^{:hidden true}
 (!.py (hs/hashset-show (hs/hashset "a" "b")))
 =>
 #"\#\{(?:\"a\", \"b\"|\"b\", \"a\")\}")

^{:refer xt.runtime.type-hashset/hashset-create, :added "4.1"}
(fact
 "creates a hashset with the default prototype"
 ^{:hidden true}
 (!.py
  (var out (hs/hashset-create hm/EMPTY_HASHMAP))
  [(. out ["::"]) (. out _size) (hs/hashset-show out)])
 =>
 ["hashset" 0 "#{}"])
...
(fact
 ...
 (!.py
  (var out (hs/hashset-empty-mutable))
  [(hs/hashset-is-editable out) (. out _size) (hs/hashset-show out)])
 =>
 [true 0 "#{}"])
