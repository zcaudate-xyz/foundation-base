(ns
 xtbench.lua.runtime.parser-test
...
(fact
 ...
 ^{:hidden true}
 (!.lua (var reader (rdr/create "bc]")) (pc/read-token reader "a"))
 =>
 "abc")
...
(fact
 ...
 ^{:hidden true}
 (!.lua (pc/interpret-token (rdr/create "") "12a"))
 =>
 (throws))
...
(fact
 ...
 ^{:hidden true}
 (!.lua (var reader (rdr/create "1 2]")) (p/read-delimited reader "]"))
 =>
 [1 2])
...
(fact
 "reads list forms"
 ^{:hidden true}
 (!.lua (list/list-to-array (p/read-list (rdr/create "1 2 3)"))))
 =>
 [1 2 3])
...
(fact
 "reads vector forms"
 ^{:hidden true}
 (!.lua (. (p/read-vector (rdr/create "1 2 3]")) (to-array)))
 =>
 [1 2 3])
...
(fact
 ...
 ^{:hidden true}
 (!.lua (p/read-map (rdr/create ":a 1 :b}")))
 =>
 (throws))
...
(fact
 ...
 ^{:hidden true}
 (!.lua (p/read-dispatch (rdr/create "?x")))
 =>
 (throws))
...
(fact
 ...
 =>
 (throws)
 (!.lua (p/read (rdr/create "^:tag")))
 =>
 (throws)
 (!.lua (p/read (rdr/create "#'")))
 =>
 (throws)
 (!.lua (p/read (rdr/create "#_skip")))
 =>
 (throws))
