(ns
 xtbench.lua.runtime.type-vector-node-test
...
(fact
 ...
 ^{:hidden true}
 (!.lua [(node/impl-mask -1) (node/impl-mask 5) (node/impl-mask 32)])
 =>
 [31 5 0])
...
(fact
 "creates a new node"
 ^{:hidden true}
 (!.lua (node/node-create 1 [1 2 3 4]))
 =>
...
(fact
 "clones the node"
 ^{:hidden true}
 (!.lua (node/node-clone (node/node-create 1 [1 2 3 4])))
 =>
 {"edit_id" 1, "children" [1 2 3 4], "::" "vector.node"})
...
(fact
 ...
 ^{:hidden true}
 (!.lua (node/node-editable-root (node/node-create 1 [1 2 3 4])))
 =>
 (contains-in
  {"edit_id" number?, "children" [1 2 3 4], "::" "vector.node"}))
...
(fact
 ...
 (!.lua
  ...
  (== node (node/node-editable node 1)))
 =>
 true)
...
(fact
 ...
 ^{:hidden true}
 (!.lua (node/ensure-editable (node/node-create nil [1 2 3])))
 =>
 (throws))
...
(fact
 ...
 ^{:hidden true}
 (!.lua (node/ensure-persistent (node/node-create 1 [])))
 =>
...
(fact
 "new path"
 ^{:hidden true}
 (!.lua (node/node-new-path nil 5 (node/node-create nil [1 2 3])))
 =>
 {"children" [{"children" [1 2 3], "::" "vector.node"}],
  "::" "vector.node"})
 ...
 (!.lua
  (var
   ...
   (node/node-pop-tail
    ...
    33
    5
    (node/node-create nil [(node/node-create nil [1 2 3])])
    false))
  [(. out ["::"]) (xt/x:len (. out children))])
 =>
 ["vector.node" 0])

^{:refer xt.runtime.type-vector-node/node-assoc, :added "4.0"}
(fact
 "associates a given node without mutating the original"
 ^{:hidden true}
 (!.lua
