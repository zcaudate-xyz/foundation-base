(ns
 xtbench.lua.runtime.interface-common-test
...
(l/script-
 ...
 {:runtime :basic,
  :require
  [[xt.runtime.interface-common :as v]
...
(fact
 ...
 ^{:hidden true}
 (!.lua (== v/NIL (v/impl-normalise nil)))
 =>
 true)
...
(fact
 ...
 (!.lua
  (var calls 0)
  (var obj {})
  [(v/hash-with-cache obj (fn:> [x] (:= calls (+ calls 1)) 42))
   (v/hash-with-cache obj (fn:> [x] (:= calls (+ calls 1)) 99))
   calls
   (. obj _hash)])
 =>
 [42 42 1 42])
...
(fact
 ...
 (!.lua
  ...
  (var
   cached
   (v/wrap-with-cache
    (fn:> [obj] (:= cached-calls (+ cached-calls 1)) 41)
    nil))
 ...
 (!.lua
  (var
   ...
   (->
    ...
    (v/push-mutable 4)
    (v/to-persistent)))
  [(v/is-persistent? out) (v/show out)])
 =>
 [true "[1, 2, 3, 4]"])
...
(fact
 ...
 ^{:hidden true}
 (!.lua
  [(-> (list/list) (v/push 1) (v/push 2) (v/push 3) (v/show))
   (-> (vec/vector) (v/push 1) (v/push 2) (v/push 3) (v/show))])
 =>
 ["(3, 2, 1)" "[1, 2, 3]"])
...
(fact
 ...
 ^{:hidden true}
 (!.lua
  [(-> (list/list 1 2 3 4) (v/pop) (v/pop) (v/show))
   (-> (vec/vector 1 2 3 4) (v/pop) (v/pop) (v/show))])
 =>
 ["(3, 4)" "[1, 2]"])
...
(fact
 "nth coll"
 ^{:hidden true}
 (!.lua [(v/nth (vec/vector 1 2 3) 0) (v/nth (vec/vector 1 2 3) 2)])
 =>
 [1 3])
...
(fact
 ...
 (!.lua
  ...
  [(hm/hashmap-lookup-key out "a" "missing") (. out _size)])
 =>
 ["missing" 1])
...
(fact
 ...
 (!.lua
  ...
  [(hm/hashmap-lookup-key out "b" "missing") (. out _size)])
 =>
 [2 2])
...
(fact
 "to iter"
 ^{:hidden true}
 (!.lua (it/arr< (v/to-iter (vec/vector 1 2 3))))
 =>
 [1 2 3])
...
(fact
 "to array"
 ^{:hidden true}
 (!.lua (v/to-array (vec/vector 1 2 3)))
 =>
 [1 2 3])
