(ns
 xtbench.lua.runtime.type-vector-test
...
(fact
 ...
 ^{:hidden true}
 (!.lua (it/arr< (v/vector-to-iter (v/vector 1 2 3 4))))
 =>
 [1 2 3 4])
...
(fact
 ...
 ^{:hidden true}
 (!.lua (v/vector-to-array (v/vector 1 2 3 4)))
 =>
 [1 2 3 4])
...
(fact
 ...
 (!.lua
  (var
   out
   (v/vector-new (node/node-create nil []) 2 node/BITS [1 2] nil))
  [(. out ["::"])
   (. out _size)
   (. out _shift)
...
(fact
 ...
 ^{:hidden true}
 (!.lua (v/vector-to-array (v/vector-empty (v/vector [1 2 3 4]))))
 =>
 {})
...
(fact
 ...
 ^{:hidden true}
 (!.lua (v/vector-to-array (v/vector-push-last v/EMPTY_VECTOR 1)))
 =>
 [1])
...
(fact
 ...
 (!.lua
  ...
  (k/arr-foldl (k/arr-range [0 10]) v/vector-push-last! V0)
  V0)
 =>
...
(fact
 ...
 (!.lua
  ...
  [(v/vector-is-editable out) (v/vector-to-array out)])
 =>
 [true [1 2 3]])
...
(fact
 "creates a vector"
 ^{:hidden true}
 (!.lua (v/vector-create (node/node-create nil []) 0 node/BITS []))
 =>
 {"_tail" {},
  "::" "vector",
  "_size" 0,
