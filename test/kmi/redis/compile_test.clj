...
(fact "creates a form for arrays"
  ...
  (to:array ['<RETURN>])
  => '[(unpack (xt.lang.common-lib/to-flat <RETURN>))]
...
(fact "export all sub keys"
  ...
  (compile type/<SPEC>
           '[:GET [] [:active]])
  => '(xt.lang.common-lib/arr-map (kmi.redis/scan-level "active") kmi.redis/key-export))
...
(fact "export input keys"
  ...
  (compile type/<SPEC>
           '[:GET [] [:active] ["ask" "bid"]])
  => '(xt.lang.common-lib/arr-map ["ask" "bid"] (fn [_k] (return (kmi.redis/key-export (cat "active" ":" _k))))))
...
(fact "delete for :key inputs"
  ...
  => '(redis.call "DEL"
                  (unpack (xt.lang.common-lib/arr-map
                           ["a" "b"]
                           (fn [_k]
...
(fact "command for set vals"
  ...
  => '(redis.call "HMSET" (cat "test" ":" "unfilled" ":" "ask")
                  (unpack (xt.lang.common-lib/to-flat {10 100}))))
