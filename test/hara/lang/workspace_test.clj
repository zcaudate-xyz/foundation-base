...
^{:refer tahto.core.workspace/rt-resolve :added "4.0"}
...
^{:refer tahto.core.workspace/sym-entry :added "4.0"}
(fact "gets the entry using a symbol"

  (w/sym-entry :xtalk `cl/noop)
  => map?)

^{:refer tahto.core.workspace/sym-pointer :added "4.0"}
...
^{:refer tahto.core.workspace/module-entries :added "4.0"}
(fact "gets all module entries"

  (w/module-entries :xtalk 'xt.lang.common-lib identity)
  => coll?)

^{:refer tahto.core.workspace/emit-ptr :added "4.0"}
(fact "emits the poiner as a string"

  (w/emit-ptr cl/noop)
  => string?)

^{:refer tahto.core.workspace/ptr-display-str :added "4.0"}
...
^{:refer tahto.core.workspace/ptr-clip :added "4.0"}
...
^{:refer tahto.core.workspace/ptr-print :added "4.0"}
(fact "copies pointer text to clipboard"
...
^{:refer tahto.core.workspace/ptr-setup :added "4.0"}
(fact "calls setup on a pointer"
  (w/ptr-setup cl/noop) => any?)

^{:refer tahto.core.workspace/ptr-teardown :added "4.0"}
(fact "calls teardown on a pointer"
  (w/ptr-teardown cl/noop) => any?)

^{:refer tahto.core.workspace/ptr-setup-deps :added "4.0"}
(fact "calls setup on a pointer and all dependencies"
  (w/ptr-setup-deps cl/noop) => any?)

^{:refer tahto.core.workspace/ptr-teardown-deps :added "4.0"}
(fact "calls teardown on pointer all dependencies"
  (w/ptr-teardown-deps cl/noop) => any?)

^{:refer tahto.core.workspace/emit-module :added "4.0"}
(fact "emits the entire module"

  (w/emit-module (l/rt 'tahto.core.workspace-test :xtalk))
  => string?)

^{:refer tahto.core.workspace/print-module :added "4.0"}
(fact "emits and prints out the module"

  (env/with-out-str
    (w/print-module (l/rt 'tahto.core.workspace-test :xtalk)))
  => string?)

^{:refer tahto.core.workspace/rt:module :added "4.0"}
(fact "gets the book module for a runtime"

  (w/rt:module (l/rt 'xt.lang.common-lib :xtalk))
  => map?)

^{:refer tahto.core.workspace/rt:module-meta :added "4.0"}
(fact "gets the book module for a runtime"
  (w/rt:module-meta :xtalk) => map?)

^{:refer tahto.core.workspace/rt:module-purge :added "4.0"}
(fact "purges the current workspace"
  (w/rt:module-purge (l/rt 'xt.lang.common-lib :xtalk)) => any?)

^{:refer tahto.core.workspace/rt:inner :added "4.0"}
(fact "gets the inner client for a shared runtime"
  (w/rt:inner :xtalk) => any?)

^{:refer tahto.core.workspace/rt:restart :added "4.0"}
(fact "restarts the shared runtime"
  (w/rt:restart :xtalk) => any?)

^{:refer tahto.core.workspace/intern-macros :added "4.0"}
