(ns tahto.core.type-shared-test
  (:require [tahto.core :as l]
            [tahto.core.type-shared :as shared])
  (:use code.test))

^{:refer tahto.core.type-shared/get-groups :added "4.0"}
(fact "gets all shared groups"

  (shared/get-groups)
tahto/lang/type_shared_test.clj:10:  ;; (:tahto/rt.postgres :tahto/rt.redis :tahto/rt.nginx :tahto/rt.cpython.shared :tahto/rt.luajit.shared)
  => vector?)

^{:refer tahto.core.type-shared/get-group-count :added "4.0"}
(fact "gets the group count for a type and id"

tahto/lang/type_shared_test.clj:16:  (shared/get-group-count :tahto/rt.redis)
  ;; {:default 21, :test 2}
  => map?)

^{:refer tahto.core.type-shared/update-group-count :added "4.0"
...
^{:refer tahto.core.type-shared/rt-is-shared? :added "4.0"}
(fact "checks if a runtime is shared"
  (shared/rt-is-shared? (shared/rt-shared:create {}))
  => true)

^{:refer tahto.core.type-shared/rt-get-inner :added "4.0"}
...
tahto/lang/type_shared_test.clj:98:  (require '[tahto.runtime.basic.impl.script :as script]))
