(ns hara.lang.model-annex.spec-fortran-test
  (:use code.test)
  (:require [hara.lang.base.impl :as impl]
            [hara.lang.model-annex.spec-fortran :refer :all]))

^{:refer hara.lang.model-annex.spec-fortran/fortran-type :added "4.1"}
...
^{:refer hara.lang.model-annex.spec-fortran/fortran-args :added "4.1"}
(fact "emits Fortran argument list"
  (impl/emit-script '(defn ^int add [[^int x] [^int y]] (+ x y))
                    {:lang :fortran})
  => string?)

^{:refer hara.lang.model-annex.spec-fortran/fortran-decl :added "4.1"}
...
^{:refer hara.lang.model-annex.spec-fortran/fortran-defn :added "4.1"}
...
^{:refer hara.lang.model-annex.spec-fortran/fortran-defprogram :added "4.1"}
...
^{:refer hara.lang.model-annex.spec-fortran/fortran-def :added "4.1"}
(fact "transforms def to Fortran typed declaration"
  (let [form '(def ^int x 5)]
    (fortran-def form))
  => list?)

^{:refer hara.lang.model-annex.spec-fortran/fortran-print :added "4.1"}
...
^{:refer hara.lang.model-annex.spec-fortran/fortran-module :added "4.1"}
...
^{:refer hara.lang.model-annex.spec-fortran/fortran-emit-if :added "4.1"}
(fact "emits IF THEN ELSE block"
  (impl/emit-as :fortran ['(if (> x 0) x 0)])
  => (fn [s] (clojure.string/includes? s "IF (")))

^{:refer hara.lang.model-annex.spec-fortran/fortran-emit-for :added "4.1"}
(fact "emits DO loop"
