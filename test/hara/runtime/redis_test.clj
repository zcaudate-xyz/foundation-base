(ns hara.runtime.redis-test
  (:require [kmi.queue.list :as list]
            [lib.redis.bench :as bench]
            [net.resp.connection :as conn]
            [hara.runtime.redis :refer :all]
            [hara.runtime.redis.eval-script :as script]
...
^{:refer hara.runtime.redis/generate-script :added "4.0"}
(fact "generates a script given a pointer"

  (generate-script kmi.queue.list/mq-list-group-init)
  => string?)

^{:refer hara.runtime.redis/test:req :added "4.0"}
(fact "does a request on a single test connection")

^{:refer hara.runtime.redis/test:invoke :added "4.0"}
