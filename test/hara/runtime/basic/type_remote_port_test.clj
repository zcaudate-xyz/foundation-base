(ns hara.runtime.basic.type-remote-port-test
  (:require [hara.runtime.basic.type-common :as common]
            [hara.runtime.basic.type-remote-port :refer :all]
            [std.concurrent :as cc]
            [std.lib.component :as component]
            [std.lib.env :as env])
  (:use code.test))

^{:refer hara.runtime.basic.type-remote-port/start-remote-port :added "4.0"}
...
^{:refer hara.runtime.basic.type-remote-port/stop-remote-port :added "4.0"}
(fact "stops the connection to the remote port"
  (with-redefs [component/stop (fn [_] nil)]
    (stop-remote-port {:relay :relay})
    => (complement :relay)))

^{:refer hara.runtime.basic.type-remote-port/raw-eval-remote-port-relay :added "4.0"}
...
^{:refer hara.runtime.basic.type-remote-port/raw-eval-remote-port :added "4.0"}
(fact "evaluates over the remote port"
  (with-redefs [raw-eval-remote-port-relay (fn [& _] :ok)]
    (raw-eval-remote-port {} "1")
    => :ok))

^{:refer hara.runtime.basic.type-remote-port/invoke-ptr-remote-port :added "4.0"}
(fact "invokes over the remote port"
  ;; delegates to default-invoke-script
  )

^{:refer hara.runtime.basic.type-remote-port/rt-remote-port-string :added "4.0"}
(fact "gets the remote port string"
  (rt-remote-port-string {:lang :lua :port 1234})
  => string?)

^{:refer hara.runtime.basic.type-remote-port/rt-remote-port:create :added "4.0"}
...
^{:refer hara.runtime.basic.type-remote-port/rt-remote-port :added "4.0"}
