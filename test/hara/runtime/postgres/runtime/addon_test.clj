(ns hara.runtime.postgres.runtime.addon-test
...
^{:refer hara.runtime.postgres.runtime.addon/exec :added "4.0"}
(fact "executes an sql statement"

  (addon/exec [:select 1])
  => 1)

^{:refer hara.runtime.postgres.runtime.addon/ARRAY :added "4.0"}
(fact "creates an array"

  (addon/ARRAY 1 2 3 4)
  => '(1 2 3 4))

^{:refer hara.runtime.postgres.runtime.addon/id :added "4.0"}
(fact "gets the id of an object"

  (str (addon/id {:id "40ff7565-2f3d-4ac0-acb5-3527370eb646"}))
  => "40ff7565-2f3d-4ac0-acb5-3527370eb646")

^{:refer hara.runtime.postgres.runtime.addon/name :added "4.0"}
(fact "gets the name of a table"

  (pg/name scratch/Task)
  => "Task")

^{:refer hara.runtime.postgres.runtime.addon/full :added "4.0"}
(fact "gets the full jsonb for table or function"

  (addon/full scratch/Task)
  => ["scratch" "Task"])

^{:refer hara.runtime.postgres.runtime.addon/full-str :added "4.0"}
(fact "gets the full json str form table or function"

  (addon/full-str scratch/Task)
  => "[\"scratch\",\"Task\"]")

^{:refer hara.runtime.postgres.runtime.addon/coord :added "4.0"}
(fact "gets the coordinate of a row"

  (addon/coord scratch/Task "id")
  => ["scratch" "Task" "id"])

^{:refer hara.runtime.postgres.runtime.addon/rand-hex :added "4.0"}
...
^{:refer hara.runtime.postgres.runtime.addon/sha1 :added "4.0"}
...
^{:refer hara.runtime.postgres.runtime.addon/client-list :added "4.0"}
(fact "gets the client list for pg"

  (count (addon/client-list))
  => pos?)

^{:refer hara.runtime.postgres.runtime.addon/time-ms :added "4.0"}
(fact "returns the time in ms"

  (l/emit-as :postgres '[(hara.runtime.postgres/time-ms)])
  => string?)

^{:refer hara.runtime.postgres.runtime.addon/time-us :added "4.0"}
(fact "returns the time in us"

  (l/emit-as :postgres '[(hara.runtime.postgres/time-us)])
  => string?)

^{:refer hara.runtime.postgres.runtime.addon/throw :added "4.0"
  :setup [(l/rt:stop)]}
...
^{:refer hara.runtime.postgres.runtime.addon/error :added "4.0"}
(fact "raises a json error with value"

  (addon/error [1 2 3 4])
  => "RAISE EXCEPTION USING\n  DETAIL = (jsonb_build_object('status','error','value',jsonb_build_array(1,2,3,4)))::TEXT,\n  MESSAGE = 'nil'\n")

^{:refer hara.runtime.postgres.runtime.addon/assert :added "4.0"}
(fact "asserts given a block"

  (addon/assert '(exists 1) [:wrong {}])
  => string?)

^{:refer hara.runtime.postgres.runtime.addon/case :added "4.0"}
...
^{:refer hara.runtime.postgres.runtime.addon/get-stack-diagnostics :added "4.0"}
(fact "gets the stack diagnostics"

  (l/emit-as :postgres '[(hara.runtime.postgres/get-stack-diagnostics)])
  => string?)

^{:refer hara.runtime.postgres.runtime.addon/field-id :added "4.0"}
...
^{:refer hara.runtime.postgres.runtime.addon/map:rel :added "4.0"}
(fact "basic map across relation"

  (l/emit-as :postgres '[(hara.runtime.postgres/map:rel f rel)])
  => "jsonb_agg(f(o_ret)) FROM rel AS o_ret")

^{:refer hara.runtime.postgres.runtime.addon/map:js-text :added "4.0"}
...
^{:refer hara.runtime.postgres.runtime.addon/ret :added "4.0"}
(fact "returns a value alias for select"

  (pg/ret 1)
  => 1)

^{:refer hara.runtime.postgres.runtime.addon/b:update :added "4.0"}
(fact "update macro"

  (l/emit-as :postgres '[(hara.runtime.postgres/b:update t {:a 1})])
  => "UPDATE t \"a\" = 1")

^{:refer hara.runtime.postgres.runtime.addon/b:insert :added "4.0"}
(fact "insert macro"

  (l/emit-as :postgres '[(hara.runtime.postgres/b:insert t {:a 1})])
  => "INSERT t \"a\" = 1")

^{:refer hara.runtime.postgres.runtime.addon/b:delete :added "4.0"}
(fact "delete macro"

  (l/emit-as :postgres '[(hara.runtime.postgres/b:delete t)])
  => "DELETE t")

^{:refer hara.runtime.postgres.runtime.addon/perform :added "4.0"}
(fact "perform macro"

  (l/emit-as :postgres '[(hara.runtime.postgres/perform 1)])
  => "PERFORM 1")

^{:refer hara.runtime.postgres.runtime.addon/random-enum :added "4.0"}
(fact "gets random enum"

  (keyword (addon/random-enum scratch/EnumStatus))
  => #{:pending :error :success})

^{:refer hara.runtime.postgres.runtime.addon/do:plpgsql :added "4.0"}
