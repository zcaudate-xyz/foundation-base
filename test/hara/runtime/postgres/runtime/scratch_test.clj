(ns hara.runtime.postgres.test.scratch-v1-test
...
^{:refer hara.runtime.postgres.test.scratch-v1/as-array :added "4.0"}
(fact "returns a jsonb array"

  (l/emit-as :postgres `[(scratch/as-array "{}")])
  => string?)

^{:refer hara.runtime.postgres.test.scratch-v1/TaskCache :added "4.0"}
(fact "constructs a task cache")

^{:refer hara.runtime.postgres.test.scratch-v1/Task :added "4.0"}
(fact "constructs a task")

^{:refer hara.runtime.postgres.test.scratch-v1/Entry :added "4.0"}
(fact "construcs an entry")

^{:refer hara.runtime.postgres.test.scratch-v1/as-upper :added "4.0"}
(fact "converts to upper case"

  (l/emit-as :postgres `[(scratch/as-upper "abc")])
  => "\"scratch\".as_upper('abc')")

^{:refer hara.runtime.postgres.test.scratch-v1/ping :added "4.0"}
(fact "tests that the db is working"

  (l/emit-as :postgres `[(scratch/ping)])
  => "\"scratch\".ping()")

^{:refer hara.runtime.postgres.test.scratch-v1/ping-ok :added "4.0"}
(fact "tests that the db is working with json"

  (l/emit-as :postgres `[(scratch/ping-ok)])
  => "\"scratch\".ping_ok()")

^{:refer hara.runtime.postgres.test.scratch-v1/echo :added "4.0"}
...
^{:refer hara.runtime.postgres.test.scratch-v1/addf :added "4.0"}
(fact "adds two values"

  (l/emit-as :postgres `[(scratch/addf 1 2)])
  => "\"scratch\".addf(1,2)")

^{:refer hara.runtime.postgres.test.scratch-v1/subf :added "4.0"}
(fact "subtracts two values"

  (l/emit-as :postgres `[(scratch/subf 1 2)])
  => "\"scratch\".subf(1,2)")

^{:refer hara.runtime.postgres.test.scratch-v1/mulf :added "4.0"}
(fact "multiplies two values"

  (l/emit-as :postgres `[(scratch/mulf 1 2)])
  => "\"scratch\".mulf(1,2)")

^{:refer hara.runtime.postgres.test.scratch-v1/divf :added "4.0"}
(fact "divide two values"

  (l/emit-as :postgres `[(scratch/divf 1 2)])
  => "\"scratch\".divf(1,2)")

^{:refer hara.runtime.postgres.test.scratch-v1/insert-task :added "4.0"}
(fact "inserts a task"

  (l/emit-as :postgres `[(scratch/insert-task "h1" "success" {})])
  => "\"scratch\".insert_task('h1','success',jsonb_build_object())")

^{:refer hara.runtime.postgres.test.scratch-v1/insert-entry :added "4.0"}
