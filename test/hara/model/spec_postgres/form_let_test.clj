(ns hara.model.spec-postgres.form-let-test
  (:require [clojure.string]
            [hara.model.spec-postgres.common :as common]
            [hara.model.spec-postgres.form-let :refer :all]
            [hara.lang :as l]
            [hara.common.emit-common :as emit-common])
  (:use code.test))

^{:refer hara.model.spec-postgres.form-let/pg-tf-let-block :added "4.0"}
...
^{:refer hara.model.spec-postgres.form-let/pg-tf-let-assign :added "4.0"}
...
^{:refer hara.model.spec-postgres.form-let/pg-tf-let-check-body :added "4.0"}
...
^{:refer hara.model.spec-postgres.form-let/pg-tf-let :added "4.0"}
...
^{:refer hara.model.spec-postgres.form-let/pg-do-block :added "4.0"}
(fact "emits a block with let usage"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "block")]
    (pg-do-block '(do:block (let [a 1] (return a))) nil nil))
  => "block")

^{:refer hara.model.spec-postgres.form-let/pg-do-suppress :added "4.0"}
...
^{:refer hara.model.spec-postgres.form-let/pg-loop-block :added "4.0"}
(fact "creates a loop block"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "block")]
    (pg-loop-block '(loop [] (let [a 1] (return a))) nil nil))
  => "block")

^{:refer hara.model.spec-postgres.form-let/pg-while-block :added "4.0"}
(fact "creates a while block"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "block")]
    (pg-while-block '(while (true) (let [a 1] (return a))) nil nil))
  => "block")

^{:refer hara.model.spec-postgres.form-let/pg-case-block :added "4.0"}
