...
(fact "all xtalk grammar map entries expose op-spec contracts"
  (vec
   (for [[sym v] (sort-by key (ns-publics 'hara.common.grammar-xtalk))
...
^{:refer hara.common.grammar-xtalk/tf-add :added "4.1"}
(fact "wrapper for add transform"
  (tf-add '(x:add a b))
  => '(+ a b))

^{:refer hara.common.grammar-xtalk/tf-sub :added "4.1"}
(fact "wrapper for sub transform"
  (tf-sub '(x:sub a b))
  => '(- a b))

^{:refer hara.common.grammar-xtalk/tf-mul :added "4.1"}
(fact "wrapper for mul transform"
  (tf-mul '(x:mul a b))
  => '(* a b))

^{:refer hara.common.grammar-xtalk/tf-div :added "4.1"}
(fact "wrapper for div transform"
  (tf-div '(x:div a b))
  => '(/ a b))

^{:refer hara.common.grammar-xtalk/tf-neg :added "4.1"}
(fact "wrapper for neg transform"
  (tf-neg '(x:neg a))
  => '(- a))

^{:refer hara.common.grammar-xtalk/tf-inc :added "4.1"}
(fact "wrapper for inc transform"
  (tf-inc '(x:inc a))
  => '(+ a 1))

^{:refer hara.common.grammar-xtalk/tf-dec :added "4.1"}
(fact "wrapper for dec transform"
  (tf-dec '(x:dec a))
  => '(- a 1))

^{:refer hara.common.grammar-xtalk/tf-eq :added "4.1"}
(fact "wrapper for eq transform"
  (tf-eq '(x:eq a b))
  => '(== a b))

^{:refer hara.common.grammar-xtalk/tf-neq :added "4.1"}
(fact "wrapper for neq transform"
  (tf-neq '(x:neq a b))
  => '(not= a b))

^{:refer hara.common.grammar-xtalk/tf-lt :added "4.1"}
(fact "wrapper for lt transform"
  (tf-lt '(x:lt a b))
  => '(< a b))

^{:refer hara.common.grammar-xtalk/tf-lte :added "4.1"}
(fact "wrapper for lte transform"
  (tf-lte '(x:lte a b))
  => '(<= a b))

^{:refer hara.common.grammar-xtalk/tf-gt :added "4.1"}
(fact "wrapper for gt transform"
  (tf-gt '(x:gt a b))
  => '(> a b))

^{:refer hara.common.grammar-xtalk/tf-gte :added "4.1"}
(fact "wrapper for gte transform"
  (tf-gte '(x:gte a b))
  => '(>= a b))

^{:refer hara.common.grammar-xtalk/tf-zero? :added "4.1"}
(fact "wrapper for zero? transform"
  (tf-zero? '(x:zero? a))
  => '(== a 0))

^{:refer hara.common.grammar-xtalk/tf-pos? :added "4.1"}
...
^{:refer hara.common.grammar-xtalk/tf-even? :added "4.1"}
(fact "wrapper for even? transform"
  (tf-even? '(x:even? a))
  => '(== 0 (mod a 2)))

^{:refer hara.common.grammar-xtalk/tf-odd? :added "4.1"}
(fact "wrapper for odd? transform"
  (tf-odd? '(x:odd? a))
  => '(not (== 0 (mod a 2))))

^{:refer hara.common.grammar-xtalk/tf-eq-nil? :added "4.0"}
(fact "equals nil transform"
  (tf-eq-nil? '(x:nil? a))
  => '(== nil a))

^{:refer hara.common.grammar-xtalk/tf-not-nil? :added "4.0"}
(fact "not nil transform"
  (tf-not-nil? '(x:not-nil? a))
  => '(not= nil a))

^{:refer hara.common.grammar-xtalk/tf-has-key? :added "4.0"}
...
^{:refer hara.common.grammar-xtalk/tf-get-key :added "4.0"}
...
^{:refer hara.common.grammar-xtalk/tf-set-key :added "4.0"}
(fact "set-key transform"

  (tf-set-key '(x:set-key obj "a" 1))
  => '(:= (. obj ["a"]) 1))

^{:refer hara.common.grammar-xtalk/tf-del-key :added "4.0"}
(fact "del-key transform"

  (tf-del-key '(x:del-key obj "a"))
  => '(x:del (. obj ["a"])))

^{:refer hara.common.grammar-xtalk/tf-copy-key :added "4.0"}
...
^{:refer hara.common.grammar-xtalk/tf-grammar-offset :added "4.0"}
(fact "del-key transform"

  (tf-grammar-offset)
  => 0)

^{:refer hara.common.grammar-xtalk/tf-grammar-end-inclusive :added "4.0"}
(fact "gets the end inclusive flag"

  (tf-grammar-end-inclusive)
  => nil)

^{:refer hara.common.grammar-xtalk/tf-offset-base :added "4.0"}
...
^{:refer hara.common.grammar-xtalk/tf-global-set :added "4.0"}
(fact "default global set transform"

  (tf-global-set '(x:global-set SYM 1))
  => '(x:set-key !:G "SYM" 1))

^{:refer hara.common.grammar-xtalk/tf-global-has? :added "4.0"}
(fact  "default global has transform"

  (tf-global-has? '(x:global-has SYM))
  => '(not (x:nil? (x:get-key !:G "SYM"))))

^{:refer hara.common.grammar-xtalk/tf-global-del :added "4.0"}
(fact "default global del transform"

  (tf-global-del '(x:global-del SYM))
  => '(x:set-key !:G "SYM" nil))

^{:refer hara.common.grammar-xtalk/tf-lu-eq :added "4.0"}
(fact "lookup equals transform"

  (tf-lu-eq '(x:lu-eq o1 o2))
  => '(== o1 o2))

^{:refer hara.common.grammar-xtalk/tf-bit-and :added "4.0"}
(fact "bit and transform"

  (tf-bit-and '(x:bit-and x y))
  => '(b:& x y))

^{:refer hara.common.grammar-xtalk/tf-bit-or :added "4.0"}
(fact "bit or transform"

  (tf-bit-or '(x:bit-or x y))
  => '(b:| x y))

^{:refer hara.common.grammar-xtalk/tf-bit-lshift :added "4.0"}
(fact "bit left shift transform"

  (tf-bit-lshift '(x:bit-lshift x y))
  => '(b:<< x y))

^{:refer hara.common.grammar-xtalk/tf-bit-rshift :added "4.0"}
(fact "bit right shift transform"

  (tf-bit-rshift '(x:bit-rshift x y))
  => '(b:>> x y))

^{:refer hara.common.grammar-xtalk/tf-bit-xor :added "4.0"}
