(ns hara.rt.jep-test
  (:require [hara.rt.jep :as jep :refer :all]
...
^{:refer hara.rt.jep/jep-bus :added "3.0"
...
^{:refer hara.rt.jep/make-interpreter :added "3.0"
  :setup [(mapv (fn [itp]
...
^{:refer hara.rt.jep/close-interpreter :added "3.0"}
(fact "closes the shared interpreter"
  (let [itp (jep/make-interpreter)]
    (close-interpreter itp)
    (contains? @jep/*interpreters* itp) => false))

^{:refer hara.rt.jep/eval-exec-interpreter :added "3.0"
  :setup [(mapv (fn [itp]
                  (f/suppress (jep/close-interpreter itp)))
                @jep/*interpreters*)]}
...
^{:refer hara.rt.jep/eval-get-interpreter :added "3.0"}
(fact "gets a value from the interpreter"
  (jep:temp-interpreter itp
                        (eval-get-interpreter itp "1 + 1"))
  => 2)

^{:refer hara.rt.jep/jep:temp-interpreter :added "3.0"}
(fact "gets a value from the interpreter"
  (jep:temp-interpreter itp
                        (eval-get-interpreter itp "1 + 1"))
  => 2)

^{:refer hara.rt.jep/jep-handler :added "3.0"}
...
^{:refer hara.rt.jep/eval-command-jep :added "3.0"
...
^{:refer hara.rt.jep/eval-command-fn :added "3.0"
  :setup    [(def +jep+ (component/start (jep/rt-jep:create {})))]
  :teardown [(component/stop +jep+)]}
...
^{:refer hara.rt.jep/start-jep :added "3.0"}
...
^{:refer hara.rt.jep/stop-jep :added "3.0"}
...
^{:refer hara.rt.jep/kill-jep :added "3.0"}
(fact "kills the jep runtime"
  (let [jep (component/start (rt-jep:create {}))]
    (kill-jep jep)))

^{:refer hara.rt.jep/invoke-ptr-jep :added "4.0"}
...
^{:refer hara.rt.jep/rt-jep:create :added "3.0"}
(fact "creates a componentizable runtime"
  (rt-jep:create {})
  => rt-jep?)

^{:refer hara.rt.jep/rt-jep :added "3.0"}
