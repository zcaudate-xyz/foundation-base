(ns hara.rt.fortran.grammar-test
  (:require [hara.rt.fortran.grammar :refer :all]
            [hara.lang :as l])
  (:use code.test))
...
(defn emit [form]
  (let [lib (hara.lang.base.library/library:create
             {:snapshot
              {:fortran {:book hara.rt.fortran.grammar/+book+}}})]
    (l/emit-as :fortran form {:library lib})))

^{:refer hara.rt.fortran.grammar/fortran-type :added "4.1"}
(fact "formats fortran types"
  (fortran-type "int") => "INTEGER"
  (fortran-type "float") => "REAL"
  (fortran-type "unknown") => "UNKNOWN")

^{:refer hara.rt.fortran.grammar/fortran-args :added "4.1"}
...
^{:refer hara.rt.fortran.grammar/fortran-decl :added "4.1"}
...
^{:refer hara.rt.fortran.grammar/fortran-defn :added "4.1"}
...
^{:refer hara.rt.fortran.grammar/fortran-defprogram :added "4.1"}
...
^{:refer hara.rt.fortran.grammar/fortran-def :added "4.1"}
...
^{:refer hara.rt.fortran.grammar/fortran-print :added "4.1"}
(fact "transforms print"
  (l/emit-as :fortran
    ['(print "Hello" 123)])
  => "PRINT *, 'Hello' , 123")

^{:refer hara.rt.fortran.grammar/fortran-module :added "4.1"}
...
^{:refer hara.rt.fortran.grammar/fortran-emit-if :added "4.1"}
...
^{:refer hara.rt.fortran.grammar/fortran-emit-for :added "4.1"}
