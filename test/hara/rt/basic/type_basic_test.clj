(ns hara.rt.basic.type-basic-test
  (:require [hara.rt.basic.impl.process-js :as js]
            [hara.rt.basic.impl.process-lua :as lua]
            [hara.rt.basic.server-basic :as server]
            [hara.rt.basic.type-basic :refer :all]
            [hara.rt.basic.type-bench :as bench]
            [hara.rt.basic.type-container :as container]
            [std.lib.component :as component])
  (:use code.test))

^{:refer hara.rt.basic.type-basic/start-basic :added "4.0"}
(fact "starts the basic rt"
  ...
  (with-redefs [server/start-server (fn [& _] {:port 1234})
                ...
                hara.rt.basic.type-oneshot/rt-oneshot-setup (fn [& _] [nil {} nil])
                hara.rt.basic.type-common/get-options (fn [& _] {})]
    (start-basic (rt-basic:create {:lang :test :id "test-start" :program nil :make nil :exec nil})))
  => map?)

^{:refer hara.rt.basic.type-basic/default-container-backup? :added "4.1"}
...
^{:refer hara.rt.basic.type-basic/local-exec-available? :added "4.1"}
(fact "local-exec-available? checks the first command token"

  (with-redefs [hara.rt.basic.type-common/program-exists? (fn [s] (= s "php"))]
...
^{:refer hara.rt.basic.type-basic/start-basic :added "4.1"}
(fact "start-basic falls back to container when local exec is unavailable and backup is enabled"
  ...
  (with-redefs [server/start-server (fn [& _] {:port 1234})
                ...
                hara.rt.basic.type-common/get-options (fn [& _] {})
                hara.rt.basic.type-oneshot/rt-oneshot-setup (fn [& _]
                                                         [:php
                                                          {:container {:image "foundation-base/rt-basic-php:latest"}
                                                           :container-backup true}
                                                          ["php" "-r"]])
                hara.rt.basic.type-common/program-exists? (fn [_] false)]
...
^{:refer hara.rt.basic.type-basic/stop-basic :added "4.0"}
...
^{:refer hara.rt.basic.type-basic/raw-eval-basic :added "4.0"}
...
^{:refer hara.rt.basic.type-basic/invoke-ptr-basic :added "4.0"}
(fact "invoke for basic rt")

^{:refer hara.rt.basic.type-basic/rt-basic-string :added "4.0"}
...
^{:refer hara.rt.basic.type-basic/rt-basic-port :added "4.0"}
...
^{:refer hara.rt.basic.type-basic/rt-basic:create :added "4.0"}
(fact "creates a basic rt"

  (with-redefs [hara.rt.basic.type-common/get-options (fn [& _] {})]
    (rt-basic:create {:lang :test}))
  => map?)

^{:refer hara.rt.basic.type-basic/rt-basic :added "4.0"}
