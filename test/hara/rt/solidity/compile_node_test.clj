...
^{:refer hara.rt.solidity.compile-node/rt-get-id :added "4.0"}
(fact "gets the rt node id"
  (with-redefs [l/rt (fn [& _] {:node {:id "id"}})]
    (compile-node/rt-get-id))
  => "id")

^{:refer hara.rt.solidity.compile-node/rt-get-contract-address :added "4.0"}
...
^{:refer hara.rt.solidity.compile-node/rt-get-contract :added "4.0"}
(fact "gets the current contract"
  ;; Complex setup
  )

^{:refer hara.rt.solidity.compile-node/rt-set-contract :added "4.0"}
(fact "sets the compiled contract"
  ;; side effect
  )

^{:refer hara.rt.solidity.compile-node/rt-get-caller-address :added "4.0"}
...
^{:refer hara.rt.solidity.compile-node/rt-get-caller-private-key :added "4.0"}
...
^{:refer hara.rt.solidity.compile-node/rt-get-node :added "4.0"}
(fact "gets the node runtime"
  (with-redefs [l/rt (fn [& _] {:node "node"})]
    (compile-node/rt-get-node))
  => "node")

^{:refer hara.rt.solidity.compile-node/rt-get-address :added "4.0"}
(fact "gets the address of the signer"
  ;; requires compile-rt-eval
  )

^{:refer hara.rt.solidity.compile-node/rt:node-get-block-number :added "4.0"}
(fact "gets the current block number"
  ;; requires compile-rt-eval
  )

^{:refer hara.rt.solidity.compile-node/rt:node-get-balance :added "4.0"}
(fact "gets the current balance"
  ;; requires compile-rt-eval
  )

^{:refer hara.rt.solidity.compile-node/rt:node-ping :added "4.0"}
(fact "pings the node"
...
^{:refer hara.rt.solidity.compile-node/rt:send-wei :added "4.0"}
(fact "sends wei to another address"
  ;; requires compile-rt-eval
  )

^{:refer hara.rt.solidity.compile-node/rt:node-eval :added "4.0"}
(fact "evaluates a form in the node runtime"
  ;; requires compile-rt-eval
  )

^{:refer hara.rt.solidity.compile-node/rt:node-past-events :added "4.0"}
(fact "gets past events"
  ;; requires compile-rt-eval
  )

^{:refer hara.rt.solidity.compile-node/with:measure :added "4.0"}
