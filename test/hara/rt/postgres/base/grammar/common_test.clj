(ns hara.rt.postgres.base.grammar.common-test
...
^{:refer hara.rt.postgres.base.grammar.common/pg-type-alias :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-deftype-ref-name :added "4.1"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-sym-meta :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-format :added "4.0"}
(fact "formats a form, extracting static components"

  (common/pg-format '(def ^{:- [:int]} hello 1))
  => vector?)

^{:refer hara.rt.postgres.base.grammar.common/pg-policy-format :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-hydrate-module-static :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-hydrate :added "4.0"}
(fact "hydrate function for top level entries"

  (common/pg-hydrate '(defn foo [] 1) {} {:module {:static {:application "app" :all {:schema ["schema"]}}}})
  => vector?)

^{:refer hara.rt.postgres.base.grammar.common/pg-string :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-uuid :added "4.1"}
(fact "constructs a pg uuid"

  (common/pg-uuid "123")
  => "'123'::uuid")

^{:refer hara.rt.postgres.base.grammar.common/pg-map :added "4.0"}
(fact "creates a postgres json object"
  (with-redefs [emit-common/*emit-fn* (fn [& _] "jsonb")]
    (common/pg-map {:a 1} nil nil))
  => "jsonb")

^{:refer hara.rt.postgres.base.grammar.common/pg-set :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-linked :added "4.0"}
(fact "emits the linked symbol"
  ;; Skipping complex setup.
  )

^{:refer hara.rt.postgres.base.grammar.common/block-do-block :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/block-do-suppress :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/block-loop-block :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/block-while-block :added "4.1"}
(fact "emits while block"
  (common/block-while-block '(= 1 1) '(do-something))
  => '[:while (= 1 1) :loop \\ (\| (do (do-something))) \\ :end-loop \;])

^{:refer hara.rt.postgres.base.grammar.common/block-case-block :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-defenum :added "4.0"}
(fact "defenum block"
...
^{:refer hara.rt.postgres.base.grammar.common/pg-defindex :added "4.0"}
(fact "defindex block"

  (common/pg-defindex '(defindex hello [table cols] body))
  => '[:create-index :if-not-exists hello :on table (quote ([cols])) body \;])

^{:refer hara.rt.postgres.base.grammar.common/pg-defpolicy :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-publication-format :added "4.1"}
(fact "formats publication"
  (common/pg-publication-format '(defpublication pub [:all]))
  => vector?)

^{:refer hara.rt.postgres.base.grammar.common/pg-defpublication :added "4.1"}
(fact "defpublication block"
  (common/pg-defpublication '(defpublication pub [:all]))
  => '(do [:drop-publication-if-exists pub]
          [:create-publication pub :for :all :tables]))

^{:refer hara.rt.postgres.base.grammar.common/pg-subscription-format :added "4.1"}
(fact "formats subscription"
  (common/pg-subscription-format '(defsubscription sub [conn pub] {}))
  => vector?)

^{:refer hara.rt.postgres.base.grammar.common/pg-defsubscription :added "4.1"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-deftrigger :added "4.0"}
...
^{:refer hara.rt.postgres.base.grammar.common/pg-defblock :added "4.0"}
