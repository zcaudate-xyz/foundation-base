...
(defn test-scaffold [config groups]
  (std.lib.component/with [board-fn (create-dispatch config)]
...
(fact "submits an entry to a board"
  ...
  => (contains ["t0" ["a"] std.lib.future/future?]) ^:hidden
...
(fact "polls the executor for more work"
  (def -ex- (doto (create-dispatch +test-config+)
              (start-dispatch)
              (submit-board {:id "a"} "t0")))
  (poll-dispatch -ex- ["a"]) => (contains [std.lib.future/future?]))
...
(fact "submits to the board executor"
  ...
  (submit-dispatch -ex- {:id "a"}) => std.lib.future/future?)
