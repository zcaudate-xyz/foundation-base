...
(fact "returns the output"
  ...
  (req:return (future/completed 1)
              false)
  => 1

  (req:return (future/completed 1) true)
  => future/future?)
...
(fact "prepares `req:single` options"
  ...
  (req:single-prep {:async true :transacted (future/incomplete)})
  => map?)
