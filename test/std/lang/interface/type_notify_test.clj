...
(fact:global
 {:setup [(std.lib.component/start +server+)]
  :teardown [(std.lib.component/stop +server+)]})
...
(fact "processes `print` id option"
  ...
  => (fn [s] (std.string.common/includes? s "world")))
...
(fact "starts http server"
  ...
  (do (def +value+ (std.lib.foundation/sid))
...
(fact "create and start notify server"
  (std.lib.component/stop (notify/notify-server {}))
  => map?)
...
(fact "returns a completable future"
  ...
  (notify/watch-oneshot +server+
                        10)
  => (contains [string? std.lib.future/future?]))
