(ns std.lang.model-annex.spec-erlang-test
...
^{:refer std.lang.model-annex.spec-erlang/capitalize-locals :added "4.1"}
(fact "capitalizes locals in form"
  (capitalize-locals '(+ a b) #{'a 'b})
  => '(+ A B))

^{:refer std.lang.model-annex.spec-erlang/emit-ast :added "4.1"}
(fact "emits ast using erlang context"
  (emit-ast '(+ 1 2)) => "+12")

^{:refer std.lang.model-annex.spec-erlang/wrap-raw :added "4.1"}
(fact "wraps string in erl-raw"
  (wrap-raw "foo") => '(erl-raw "foo"))

^{:refer std.lang.model-annex.spec-erlang/tf-erlang-defn :added "4.1"}
...
^{:refer std.lang.model-annex.spec-erlang/erlang-map-key :added "4.1"}
