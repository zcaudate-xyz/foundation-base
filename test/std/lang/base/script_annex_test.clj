...
(fact "adds a runtime to the annex"
  ...
  (annex/add-annex-runtime (std.lib.env/ns-sym)
                           :hello
                           :hello.rt)
  => [nil :hello.rt]

  (annex/get-annex-runtime (std.lib.env/ns-sym)
                           :hello)
  => :hello.rt

  (annex/remove-annex-runtime (std.lib.env/ns-sym)
                              :hello)
  => :hello.rt)

^{:refer std.lang.base.script-annex/get-annex-runtime :added "4.0"}
(fact "gets the annex rutime"
  (annex/get-annex-runtime (std.lib.env/ns-sym) :hello) => nil)

^{:refer std.lang.base.script-annex/remove-annex-runtime :added "4.0"}
(fact "removes the annex runtime"
  (annex/remove-annex-runtime (std.lib.env/ns-sym) :hello) => nil)

^{:refer std.lang.base.script-annex/register-annex-tag :added "4.0"
  :setup [(annex/deregister-annex-tag (std.lib.env/ns-sym) :redis.0)]}
(fact "registers a config for the tag"
  ^:hidden
  
  (annex/register-annex-tag (std.lib.env/ns-sym)
                            :redis.0
                            :lua
                            :redis
                            {})
  ...
  (annex/deregister-annex-tag (std.lib.env/ns-sym) :redis.0)
...
(fact "removes the config for the tag"
  (annex/deregister-annex-tag (std.lib.env/ns-sym) :redis.0) => nil)
...
(comment
  (./create-tests)
  (./import)
  (std.lib.context.registry/registry-list)

  

  (std.lib.context.registry/registry-get :lang.annex)

  (std.lib.context.space/space:context-list)
  (std.lib.context.space/space:context-get (std.lib.context.space/space) :lang.annex)

  (std.lib.context.space/space:context-get (std.lib.context.space/space) :lang/c)

  (std.lib.context.space/space:rt-start (std.lib.context.space/space) :lang.annex)
  (std.lib.context.space/space:rt-start (std.lib.context.space/space) :lang/lua)
