...
^{:refer std.pipe/main-function :added "4.0"}
(fact "creates a main function to be used for execution"
  ^:hidden

  (let [[main args?] (main-function ns-aliases 1)]
    (fn? main) => true
    args? => false))

^{:refer std.pipe/select-filter :added "4.0"}
...
^{:refer std.pipe/select-inputs :added "4.0"}
...
^{:refer std.pipe/wrap-execute :added "3.0"}
...
^{:refer std.pipe/task-inputs :added "4.0"}
