...
(fact "adds layout metadat to arglists"
  ...
  (binding [*print-meta* true]
    (pr-str (bind/layout-annotate-arglist
             '[{:keys [a b c d e]
                :as other}])))
  => "[^{:readable-len 30} {:keys ^{:tag :vector, :readable-len 10, :spec {:columns 1}} [a b c d e], :as other}]")
...
(fact"adds layout metadata to bindings"
  ...
  (binding [*print-meta* true]
    (pr-str (bind/layout-annotate-bindings
             '[{:keys [a b c d e]
                :as other} {:a 1 :b 2}])))
  => "^{:spec {:columns 2, :col-align true}} [^{:readable-len 30} {:keys ^{:tag :vector, :readable-len 10, :spec {:columns 1}} [a b c d e], :as other} {:a 1, :b 2}]")
  ...
  (binding [*print-meta* true]
    (pr-str (bind/layout-annotate-fn-named
             '(defn hello
                ([{:keys [a b c d e]
                   :as other}])))))
  => "(defn hello ([^{:readable-len 30} {:keys ^{:tag :vector, :readable-len 10, :spec {:columns 1}} [a b c d e], :as other}]))"
  ...
  (binding [*print-meta* true]
    (pr-str (bind/layout-annotate-fn-named
             '(defn hello
                ...
                [{:keys [a b c d e]
                    :as other}]))))
  => "(defn hello \"docstring\" {:a 1} [^{:readable-len 30} {:keys ^{:tag :vector, :readable-len 10, :spec {:columns 1}} [a b c d e], :as other}])")
