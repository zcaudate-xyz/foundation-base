(ns
 xtbench.dart.lang.util-xml-test
 (:use code.test)
 (:require [std.lang :as l] [std.string.prose :as prose]))

(l/script-
 :dart
 {:runtime :twostep, :require [[xt.lang.util-xml :as xml]]})
...
(fact
 "to node to tree"
 ^{:hidden true}
 (!.dt (xml/to-tree (xml/parse-xml "<a/>")))
 =>
 ["a"]
 (!.dt (xml/to-tree (xml/parse-xml "<a>1</a>")))
 =>
 ["a" "1"]
 (!.dt (xml/to-tree (xml/parse-xml "<a><b></b></a>")))
 =>
 ["a" ["b"]]
 (!.dt (xml/to-tree (xml/parse-xml "<a>1<b>2</b>3</a>")))
 =>
 ["a" "1" ["b" "2"] "3"]
 (!.dt
  (xml/to-tree
...
(fact
 "to node params"
 ^{:hidden true}
 (!.dt (xml/to-string-params (tab [:a 1])))
 =>
 " a=1")
...
(fact
 "node to string"
 ^{:hidden true}
 (!.dt (xml/to-string {"tag" "helo:test", "params" {}, "children" []}))
 =>
 "<helo:test></helo:test>"
 (!.dt
  (xml/to-string
 ...
 (!.dt (xml/to-string (xml/from-tree ["B"])))
 =>
 "<B></B>"
 (!.dt
  (xml/to-string
