...
(fact:global {:setup [(l/rt:restart) true], :teardown [(l/rt:stop)]})
...
(fact
 ...
 ^{:hidden true}
 (!.dt (v/tree-control-array {:limit 20, :order-by ["name"]}))
 =>
 +out+)
...
(fact
 ...
 ^{:hidden true}
 (!.dt (v/tree-count sample/Schema (@! +count+) {} {}))
 =>
 +tree+)
