(ns
 xtbench.r.lang.event-route-test
 (:require
  [std.json :as json]
  [std.lang :as l]
...
(fact
 ...
 ^{:hidden true}
 (!.R (route/path-to-tree ["hello" "world"]))
 =>
 {"[]" "hello", "[\"hello\"]" "world"})
...
(fact
 ...
 ^{:hidden true}
 (!.R (route/path-from-tree {"[]" "hello", "[\"hello\"]" "world"}))
 =>
 ["hello" "world"])
...
(fact
 ...
 ^{:hidden true}
 (!.R (route/get-param (route/make-route "hello?auth=sign_in") "auth"))
 =>
 "sign_in")
