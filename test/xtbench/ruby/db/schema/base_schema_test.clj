(ns xtbench.ruby.db.schema.base-schema-test
  (:use code.test)
...
(fact "get lookup"
  ...
  (!.rb (sch/get-cached-schema sample/Schema))
  => map?)
...
(fact "creates ref keys"
  ...
  (!.rb (sch/create-ref-keys sample/Schema "UserProfile"))
...
(fact "get all keys"
  ...
  (!.rb (sch/get-all-keys sample/Schema "Organisation"))
  => +all-org+)

^{:refer xt.db.schema.base-schema/data-keys :added "4.0"}
(fact "gets data keys"

  (!.rb (sch/data-keys sample/Schema "UserAccount"))
...
(fact "gets rev keys"
  ...
  (set (!.rb (sch/rev-keys sample/Schema "UserAccount")))
...
(fact "ges the table columns"
  ...
  (!.rb (sch/table-columns sample/Schema "UserProfile"))
  => +out+)
