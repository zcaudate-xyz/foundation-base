(ns xtbench.scheme.lang.common-data-test
...
(fact "merges objects at a nesting level"
  ...
  (!.scheme (xtd/obj-assign-nested {:a {:b 1}} {:a {:c 2}}))
  => {"a" {"b" 1, "c" 2}}
...
(fact "finds the intersection between map lookups"
  ...
  (!.scheme (xtd/obj-intersection {:a true, :b true} {:b true, :c true}))
  => ["b"])

^{:refer xt.lang.common-data/obj-keys-nested :added "4.1"}
(fact "gets nested keys"

  (!.scheme (xtd/obj-keys-nested {:a {:b 1, :c 2}} []))
  => (just [[["a" "b"] 1]
            [["a" "c"] 2]]
           :in-any-order))
...
(fact "finds the difference between two map lookups"
  ...
  (!.scheme (xtd/obj-difference {:a true, :b true} {:b true, :c true}))
  => ["c"])
...
(fact "performs reduce on an array"
  ...
  (!.scheme (xtd/arr-foldl [1 2 3] (fn [acc x] (return (+ acc x))) 0))
  => 6)

^{:refer xt.lang.common-data/arr-foldr :added "4.1"}
(fact "performs right reduce"

  (!.scheme (xtd/arr-foldr [1 2 3] (fn [acc x] (return (+ (* acc 10) x))) 0))
  => 321)
