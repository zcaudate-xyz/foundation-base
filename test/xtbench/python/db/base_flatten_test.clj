(ns
 xtbench.python.db.base-flatten-test
...
(fact
 "flatten links"
 ^{:hidden true}
 (!.py (f/flatten-get-links {:currencies [{:id "hello"}]}))
 =>
...
(fact
 ...
 ^{:hidden true}
 (!.py
  (f/flatten-merge {} {:id "hello"} {:profile {"profile-id" true}} {}))
 =>
 {"hello"
  {"ref_links" {"profile" {"profile-id" true}},
   "id" "hello",
   "rev_links" {},
   "data" {"id" "hello"}}})
...
^{:refer xt.db.base-flatten/flatten-node.account,
  :adopt true,
  :added "4.0",
  :setup
  [(def
     ...
     {"00000000-0000-0000-0000-000000000000"
      {"ref_links" {},
       ...
       {"is_official" false,
        ...
        "is_super" true}}}))]}
(fact
 "flatten node example for user account"
 ^{:hidden true}
 (!.py
  (->
   (f/flatten-node
...
^{:refer xt.db.base-flatten/flatten-node.profile,
  :adopt true,
  :added "4.0",
  :setup
  [(def
...
^{:refer xt.db.base-flatten/flatten-linked,
  :added "4.0",
  :setup
  [(def
    +table-linked+
  ...
  (f/flatten-linked
   ...
   {"profile"
    [{"id" "c4643895-b0ce-44cc-b07b-2386bf18d43b",
      "last_name" "User",
      "first_name" "Root",
      "picture" {"url" "static/user.jpg", "type" "base"},
      "language" "en"}],
    "organisations"
    [{"id" "ec088f52-310b-491b-a034-d4efc222fd00", "name" "root"}]}
   "hello"
   {}
   f/flatten-obj))
 =>
...
^{:refer xt.db.base-flatten/flatten-obj,
  :added "4.0",
  :setup
  [(def
    +user-full+
...
(fact
 ...
 ^{:hidden true}
 (!.py (f/flatten sample/Schema "UserAccount" sample/RootUserFull nil))
 =>
 +user-full+)
...
(fact
 ...
 ^{:hidden true}
 (!.py
  (f/flatten-bulk sample/Schema {"UserAccount" [sample/RootUserFull]}))
 =>
 +user-full+)
