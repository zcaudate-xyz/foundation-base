(ns
 xtbench.python.db.sql-manage-test
 (:require
  [std.lang :as l]
...
^{:refer xt.db.sql-manage/table-create.sqlite,
  :adopt true,
  :added "4.0"}
(fact
 "workflow for sqlite-wasm"
 (notify/wait-on
  :python
  (dbsql/connect
   {:constructor js-sqlite/connect-constructor}
   {:success (fn [conn] (:= (!:G DB) conn) (repl/notify true))}))
 =>
 true
 (!.py
  (dbsql/query-sync
 ...
 [{"values"
   [["UserAccount"]
    ...
    ["RegionCity"]],
   "columns" ["name"]}]
 (!.py (dbsql/query-sync DB "pragma table_info('Currency')"))
 =>
    ...
    (graph/select
     ...
     ["UserAccount"
      ["*/data"
       ...
       ["wallets"
        ["*/data"
         ["entries" ["*/data" ["asset" ["*/data" ["currency"]]]]]]]]]
     (ut/sqlite-opts nil)))
   xt/x:json-decode))
 =>
 [{"is_official" 0,
