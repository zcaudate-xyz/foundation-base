(ns
 xtbench.python.lang.util-http-test
 (:require
  [net.http :as nhttp]
...
(l/script-
 ...
 {:runtime :basic,
  :require
  [[xt.lang.util-http :as http] [xt.lang.common-repl :as repl]]})

(l/script+
 [:es :lua]
 {:runtime :nginx.instance,
  :require [[lua.nginx.websocket :as ws] [lua.nginx :as n]]})

(defonce +test-http+ (atom nil))

(defonce +es-ready+ (atom false))

(def CANARY-NGINX (not= "Mac OS X" (os/os)))
     ...
     (server/run-server
      (fn
       ...
       {:status 200,
        ...
        :body "OK"})
      {:port port})]
    (reset! +test-http+ {:port port, :stop stop-fn}))
   (Thread/sleep 250)
   (when
    CANARY-NGINX
    ...
    (try
     ...
     (catch Exception _ (reset! +es-ready+ false))))],
  :teardown
  [(l/rt:stop)
   (when-let
    [{:keys [stop]} @+test-http+]
    ...
    (reset! +test-http+ nil))
   (reset! +es-ready+ false)
   (when CANARY-NGINX (l/annex:stop-all))]})

^{:refer xt.lang.util-http/CANARY, :adopt true, :added "4.0"}
(fact "tests that scaffold is working" true => true)
 ...
 (when
  ...
  (notify/wait-on
   ...
   (var
    ...
    (http/es-connect
     (@!
      (str "http://localhost:" (:port (l/annex:get :es)) "/eval/es"))
     {:on-message (fn [msg] (repl/notify msg.data) (es.close))}))))
 =>
...
(comment
  ...
  (!.py
   ...
   (var
    ...
    (new
     EventSource
     (@!
      (str "http://localhost:" (:port (l/rt:inner :lua)) "/eval/es"))))
 ...
 (l/with:input
  (!.py
   ...
   (es.addEventListener "message" console.log)
   (es.addEventListener "open" console.log)
   (es.addEventListener "close" (fn [] (es.close))))))
