(ns
 xtbench.python.lang.common-repl-test
 (:require
  [std.json :as json]
  [std.lang :as l]
...
(fact
 "returns the encoded "
 ^{:hidden true}
 (json/read (!.py (k/return-encode {:data [1 2 3]} "<id>" "<key>")))
 =>
...
(fact
 ...
 ^{:hidden true}
 (json/read (!.py (k/return-wrap (fn:> 1))))
 =>
...
(fact
 ...
 ^{:hidden true}
 (json/read (!.py (k/return-eval "globals()[\"OUT\"] = 1")))
 =>
 {"key" nil, "id" nil, "value" 1, "type" "data"})
...
(fact
 ...
 ^{:hidden true}
 (notify/wait-on :python (k/notify 1))
 =>
 1)
...
(fact
 ...
 ^{:hidden true}
 (notify/wait-on :python ((. (k/<!) ["success"]) 1))
 =>
 1)
