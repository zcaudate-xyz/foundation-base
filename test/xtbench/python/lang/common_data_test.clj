(ns xtbench.python.lang.common-data-test (:require [std.lang :as l]) (:use code.test))

(l/script-
 :python
 {:runtime :basic, :require [[xt.lang.common-data :as xtd] [xt.lang.common-spec :as xt]]})
...
(fact "creates a lookup table" (!.py (var lu (xtd/lu-create)) (xtd/lu-get lu "missing")) => nil)

^{:refer xt.lang.common-data/lu-del, :added "4.1"}
(fact
 "deletes a lookup entry"
 (!.py (var lu (xtd/lu-create)) (xtd/lu-set lu "a" 1) (xtd/lu-del lu "a") (xtd/lu-get lu "a"))
 =>
 nil)
...
(fact
 "gets a lookup entry"
 (!.py (var lu (xtd/lu-create)) (xtd/lu-set lu "a" 1) (xtd/lu-get lu "a"))
 =>
 1
 (!.py (var lu (xtd/lu-create)) (xtd/lu-set lu lu 2) (xtd/lu-get lu lu))
 =>
 2)
...
(fact
 "sets a lookup entry"
 (!.py (var lu (xtd/lu-create)) (xtd/lu-set lu "a" 2) (xtd/lu-get lu "a"))
 =>
 2)
...
^{:refer xt.lang.common-data/first, :added "4.1"} (fact "first" (!.py (xtd/first [1 2 3])) => 1)

^{:refer xt.lang.common-data/second, :added "4.1"}
(fact "gets the second item" (!.py (xtd/second [1 2 3])) => 2)

^{:refer xt.lang.common-data/nth, :added "4.1"}
(fact "gets the nth item (index 0)" (!.py [(xtd/nth [1 2 3] 0) (xtd/nth [1 2 3] 2)]) => [1 3])

^{:refer xt.lang.common-data/last, :added "4.1"}
(fact "gets the last item" (!.py (xtd/last [1 2 3])) => 3)

^{:refer xt.lang.common-data/second-last, :added "4.1"}
(fact "gets the second-last item" (!.py (xtd/second-last [1 2 3])) => 2)

^{:refer xt.lang.common-data/arr-empty?, :added "4.1"}
(fact
 "checks that arrect is empty"
 (!.py [(xtd/arr-empty? nil) (xtd/arr-empty? []) (xtd/arr-empty? [1])])
 =>
 [true true false])
...
(fact
 ...
 (!.py [(xtd/arr-not-empty? nil) (xtd/arr-not-empty? []) (xtd/arr-not-empty? [1])])
 =>
 [false false true])

^{:refer xt.lang.common-data/arrayify, :added "4.1"}
(fact "makes something into an array" (!.py [(xtd/arrayify 1) (xtd/arrayify [1])]) => [[1] [1]])

^{:refer xt.lang.common-data/arr-lookup, :added "4.1"}
(fact
 "constructs a lookup given keys"
 (set (!.py (xtd/obj-keys (xtd/arr-lookup ["a" "b"]))))
 =>
 #{"a" "b"})

^{:refer xt.lang.common-data/arr-omit, :added "4.1"}
(fact "emits index from new array" (!.py (xtd/arr-omit [1 2 3] 1)) => [1 3])

^{:refer xt.lang.common-data/arr-reverse, :added "4.1"}
(fact "reverses the array" (!.py (xtd/arr-reverse [1 2 3])) => [3 2 1])

^{:refer xt.lang.common-data/arr-zip, :added "4.1"}
(fact "zips two arrays together into a map" (!.py (xtd/arr-zip ["a" "b"] [1 2])) => {"a" 1, "b" 2})

^{:refer xt.lang.common-data/arr-clone, :added "4.1"}
(fact
 "clones an array"
 (!.py (var src [1 2]) (var out (xtd/arr-clone src)) (xt/x:arr-push src 3) out)
 =>
 [1 2])

^{:refer xt.lang.common-data/arr-append, :added "4.1"}
(fact "appends to the end of an array" (!.py (xtd/arr-append [1 2] [3 4])) => [1 2 3 4])

^{:refer xt.lang.common-data/arr-slice, :added "4.1"}
(fact "slices an array" (!.py (xtd/arr-slice [1 2 3 4] 1 3)) => [2 3])

^{:refer xt.lang.common-data/arr-rslice, :added "4.1"}
(fact "gets the reverse of a slice" (!.py (xtd/arr-rslice [1 2 3 4] 1 3)) => [3 2])

^{:refer xt.lang.common-data/arr-tail, :added "4.1"}
(fact "gets the tail of the array" (!.py (xtd/arr-tail [1 2 3 4] 2)) => [4 3])

^{:refer xt.lang.common-data/arr-range, :added "4.1"}
(fact
 "creates a range array"
 (!.py [(xtd/arr-range 5) (xtd/arr-range [2 6]) (xtd/arr-range [2 7 2])])
 =>
 [[0 1 2 3 4] [2 3 4 5] [2 4 6]])

^{:refer xt.lang.common-data/arr-intersection, :added "4.1"}
(fact "gets the intersection of two arrays" (!.py (xtd/arr-intersection [1 2 3] [2 3 4])) => [2 3])

^{:refer xt.lang.common-data/arr-difference, :added "4.1"}
(fact "gets the difference of two arrays" (!.py (xtd/arr-difference [1 2] [2 3 4])) => [3 4])

^{:refer xt.lang.common-data/arr-union, :added "4.1"}
(fact "gets the union of two arrays" (set (!.py (xtd/arr-union [1 2] [2 3]))) => #{1 3 2})

^{:refer xt.lang.common-data/arr-shuffle, :added "4.1"}
(fact "shuffles the array" (set (!.py (xtd/arr-shuffle [1 2 3]))) => #{1 3 2})

^{:refer xt.lang.common-data/arr-pushl, :added "4.1"}
(fact "pushs an element into array" (!.py (xtd/arr-pushl [1 2 3] 4 3)) => [2 3 4])

^{:refer xt.lang.common-data/arr-pushr, :added "4.1"}
(fact "pushs an element into array" (!.py (xtd/arr-pushr [1 2 3] 0 3)) => [0 1 2])

^{:refer xt.lang.common-data/arr-interpose, :added "4.1"}
(fact "puts element between array" (!.py (xtd/arr-interpose [1 2 3] 0)) => [1 0 2 0 3])

^{:refer xt.lang.common-data/arr-random, :added "4.1"}
(fact "gets a random element from array" (!.py (xtd/arr-random [1])) => 1)

^{:refer xt.lang.common-data/arr-sample, :added "4.1"}
(fact "samples array according to probability" (!.py (xtd/arr-sample ["a" "b" "c"] [0 1 0])) => "b")

^{:refer xt.lang.common-data/obj-empty?, :added "4.1"}
(fact
 "checks that object is empty"
 (!.py [(xtd/obj-empty? {}) (xtd/obj-empty? {:a 1})])
 =>
 [true false])
...
(fact
 ...
 (!.py [(xtd/obj-not-empty? {}) (xtd/obj-not-empty? {:a 1})])
 =>
 [false true])

^{:refer xt.lang.common-data/obj-first-key, :added "4.1"}
(fact "gets the first key" (!.py (xtd/obj-first-key {:a 1})) => "a")

^{:refer xt.lang.common-data/obj-first-val, :added "4.1"}
(fact "gets the first val" (!.py (xtd/obj-first-val {:a 1})) => 1)
...
(fact
 ...
 (!.py (xtd/obj-assign-with {:a 1, :b 2} {:a 3, :c 4} (fn [x y] (return (+ x y)))))
 =>
 {"a" 4, "b" 2, "c" 4})

^{:refer xt.lang.common-data/obj-del, :added "4.1"}
(fact "deletes multiple keys" (!.py (xtd/obj-del {:a 1, :b 2, :c 3} ["a" "c"])) => {"b" 2})

^{:refer xt.lang.common-data/obj-pick, :added "4.1"}
(fact "select keys in object" (!.py (xtd/obj-pick {:a 1, :b 2} ["a"])) => {"a" 1})

^{:refer xt.lang.common-data/obj-omit, :added "4.1"}
(fact "new object with missing keys" (!.py (xtd/obj-omit {:a 1, :b 2} ["a"])) => {"b" 2})

^{:refer xt.lang.common-data/obj-transpose, :added "4.1"}
(fact "obj-transposes a map" (!.py (xtd/obj-transpose {:a "x", :b "y"})) => {"x" "a", "y" "b"})

^{:refer xt.lang.common-data/obj-nest, :added "4.1"}
(fact "creates a nested object" (!.py (xtd/obj-nest ["a" "b"] 1)) => {"a" {"b" 1}})

^{:refer xt.lang.common-data/obj-keys, :added "4.1"}
(fact "gets keys of an object" (set (!.py (xtd/obj-keys {:a 1, :b 2}))) => #{"a" "b"})

^{:refer xt.lang.common-data/obj-vals, :added "4.1"}
(fact "gets vals of an object" (set (!.py (xtd/obj-vals {:a 1, :b 2}))) => #{1 2})

^{:refer xt.lang.common-data/obj-pairs, :added "4.1"}
(fact
 "creates entry pairs from object"
 (set (!.py (xtd/obj-pairs {:a 1, :b 2})))
 =>
 #{["b" 2] ["a" 1]})
...
(fact
 "clones an object"
 (!.py (var src {:a 1}) (var out (xtd/obj-clone src)) (xt/x:set-key src "b" 2) out)
 =>
 {"a" 1})
...
(fact
 "gets item in object"
 (!.py [(xtd/get-in {:a {:b {:c 1}}} ["a" "b"]) (xtd/get-in {:a {:b {:c 1}}} ["a" "b" "c"])])
 =>
 [{"c" 1} 1])
...
(fact
 "sets item in object"
 [(!.py (var a {:a {:b {:c 1}}}) (xtd/set-in a ["a" "b"] 2) a)
  (!.py (var a {:a {:b {:c 1}}}) (xtd/set-in a ["a" "d"] 2) a)]
 =>
 [{"a" {"b" 2}} {"a" {"b" {"c" 1}, "d" 2}}])
...
(fact
 ...
 (set (!.py (xtd/obj-intersection {:a true, :b true} {:b true, :c true})))
 =>
 #{"b"})
...
(fact
 "gets nested keys"
 (set (!.py (xtd/obj-keys-nested {:a {:b 1, :c 2}} [])))
 =>
 #{[["a" "b"] 1] [["a" "c"] 2]})
...
(fact
 ...
 (set (!.py (xtd/obj-difference {:a true, :b true} {:b true, :c true})))
 =>
 #{"c"})
...
(fact
 ...
 (!.py (var out {:a 1}) (xtd/swap-key out "a" (fn [x y] (return (+ x y))) [2]) out)
 =>
 {"a" 3})

^{:refer xt.lang.common-data/to-flat, :added "4.1"}
(fact "flattens pairs of object into array" (!.py (xtd/to-flat [["a" 1] ["b" 2]])) => ["a" 1 "b" 2])

^{:refer xt.lang.common-data/from-flat, :added "4.1"}
(fact
 "creates object from flattened pair array"
 (!.py (xtd/from-flat ["a" 1 "b" 2] xtd/set-pair-step {}))
 =>
 {"a" 1, "b" 2})
...
(fact
 ...
 (!.py [(xtd/eq-nested-basic 1 1 nil nil nil) (xtd/eq-nested-basic 1 2 nil nil nil)])
 =>
 [true false])
...
(fact "diffs only keys within map" (!.py (xtd/tree-diff {:a 1, :b 2} {:a 1, :c 2})) => {"c" 2})
...
(fact
 ...
 (!.py (var out []) (xtd/arr-each [1 2 3] (fn [x] (xt/x:arr-push out (+ x 1)))) out)
 =>
 [2 3 4])
...
(fact
 ...
 (!.py (xtd/arr-keepf [1 2 3 4] (fn [x] (return (== 1 (mod x 2)))) (fn [x] (return (* x 10)))))
 =>
 [10 30])
...
(fact
 ...
 (!.py (xtd/arr-juxt [1 2] (fn [x] (return (xt/x:to-string x))) (fn [x] (return (* x x)))))
 =>
 {"1" 1, "2" 4})
...
(fact
 ...
 (!.py (xtd/arr-pipel [(fn [x] (return (+ x 1))) (fn [x] (return (* x 2)))] 1))
 =>
 4)
...
(fact
 ...
 (!.py (xtd/arr-piper [(fn [x] (return (+ x 1))) (fn [x] (return (* x 2)))] 1))
 =>
 3)
...
(fact "repeat function or value n times" (!.py (xtd/arr-repeat 1 3)) => [1 1 1])

^{:refer xt.lang.common-data/arr-normalise, :added "4.1"}
(fact "normalises array elements to 1" (!.py (xtd/arr-normalise [2 3 5])) => [0.2 0.3 0.5])

^{:refer xt.lang.common-data/arr-sort, :added "4.1"}
(fact
 "arr-sort using key function and comparator"
 (!.py (xtd/arr-sort [3 1 2] (fn [x] (return x)) (fn [a b] (return (< a b)))))
 =>
 [1 2 3])
...
(fact
 ...
 (!.py (xtd/arr-sorted-merge [1 3 5] [2 4 6] (fn [a b] (return (< a b)))))
 =>
 [1 2 3 4 5 6])
...
(fact
 ...
 (!.py [(xtd/clone-shallow nil) (xtd/clone-shallow 1)])
 =>
 [nil 1])
...
(fact
 ...
 (!.py (var src {:a {:b 1}}) (var out (xtd/clone-nested src)) (xtd/set-in src ["a" "b"] 2) out)
 =>
 {"a" {"b" 1}})
