(ns xtbench.julia.db.schema.base-schema-test
  (:use code.test)
...
(fact "get lookup"
  ...
  (!.julia (sch/get-cached-schema sample/Schema))
  => map?)
...
(fact "creates ref keys"
  ...
  (!.julia (sch/create-ref-keys sample/Schema "UserProfile"))
...
(fact "get all keys"
  ...
  (!.julia (sch/get-all-keys sample/Schema "Organisation"))
  => +all-org+)

^{:refer xt.db.schema.base-schema/data-keys :added "4.0"}
(fact "gets data keys"

  (!.julia (sch/data-keys sample/Schema "UserAccount"))
...
(fact "gets rev keys"
  ...
  (set (!.julia (sch/rev-keys sample/Schema "UserAccount")))
...
(fact "ges the table columns"
  ...
  (!.julia (sch/table-columns sample/Schema "UserProfile"))
  => +out+)
