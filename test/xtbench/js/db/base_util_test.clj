...
(fact
 "collect routes"
 ^{:hidden true}
 (!.js (ut/collect-routes (@! +routes+) "db"))
 =>
 +result+)
...
(fact
 "keeps given limit"
 ^{:hidden true}
 (!.js (ut/keepf-limit [1 2 3 4 5] xt/x:odd? k/identity 3))
 =>
 [1 3 5])
...
(fact
 "helper for lu-map"
 ^{:hidden true}
 (!.js (ut/lu-nested [{:id "1"}] (fn:> [e] e.id)))
 =>
 {"1" {"id" "1"}})
