...
(fact
 ...
 ^{:hidden true}
 (!.js
  [(dbsql/query INSTANCE "SELECT 1;" nil)
   (xtd/arr-sort
    (xtd/obj-keys
     (f/flatten-bulk sample/Schema {"UserAccount" [sample/RootUser]}))
    k/identity
    k/lt)])
 =>
...
(fact
 ...
 ^{:hidden true}
 (!.js
  (impl-sql/sql-gen-delete "HELLO" ["A" "B"] (ut/sqlite-opts nil)))
 =>
...
^{:refer xt.db.impl-sql/sql-process-event-remove,
  :added "4.0",
  :setup
  [(!.js
    (xtd/arr-sort
...
^{:refer xt.db.impl-sql/sql-pull-sync,
  :added "4.0",
  :setup
  [(def
    +account+
