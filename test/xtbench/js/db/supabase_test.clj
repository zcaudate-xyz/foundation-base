...
(fact
 "translates canonical supabase payloads to xt.db events"
 ^{:hidden true}
 (!.js
  (:= (!:G USD) @+usd-snake+)
...
(fact
 "applies supabase payload to local xt.db (sqlite)"
 ^{:hidden true}
 (!.js
  (:= (!:G USD) @+usd-snake+)
  (sup/apply-payload
   ...
   sample/SchemaLookup
   (ut/sqlite-opts nil))
  (xdb/db-pull-sync DBSQL sample/Schema ["Currency" ["id"]]))
 =>
 [{"id" "USD"}]
 (!.js
  (sup/apply-payload
...
(fact
 "attaches a stubbed supabase realtime client and applies payloads"
 ^{:hidden true}
 (!.js
  (:= (!:G USD) @+usd-snake+)
  (var handlers [])
  ...
  (var
   ...
   (sup/attach-events
    {"supabase" supabase,
     ...
     "channel-name" "test",
     "bindings"
     [{"event" "*", "schema" "public", "table" "Currency"}]}))
  ...
  ((x:get-idx handlers 0)
   {"type" "postgres_changes",
    ...
    "table" "Currency",
    "new" (!:G USD)})
  (var rows (xdb/db-pull-sync DBSQL sample/Schema ["Currency" ["id"]]))
  (detach-fn)
  rows)
 =>
 [{"id" "USD"}])

^{:refer xt.db.supabase/snake->kebab, :added "4.1"} (fact "TODO")

^{:refer xt.db.supabase/normalize-row, :added "4.1"} (fact "TODO")
