(ns xtbench.elisp.db.schema.base-schema-test
  (:use code.test)
...
(fact "get lookup"
  ...
  (!.elisp (sch/get-cached-schema sample/Schema))
  => map?)
...
(fact "creates ref keys"
  ...
  (!.elisp (sch/create-ref-keys sample/Schema "UserProfile"))
...
(fact "get all keys"
  ...
  (!.elisp (sch/get-all-keys sample/Schema "Organisation"))
  => +all-org+)

^{:refer xt.db.schema.base-schema/data-keys :added "4.0"}
(fact "gets data keys"

  (!.elisp (sch/data-keys sample/Schema "UserAccount"))
...
(fact "gets rev keys"
  ...
  (set (!.elisp (sch/rev-keys sample/Schema "UserAccount")))
...
(fact "ges the table columns"
  ...
  (!.elisp (sch/table-columns sample/Schema "UserProfile"))
  => +out+)
