(ns xtbench.elisp.lang.common-string-test
  (:use code.test)
...
(fact "splits a long string"
  ...
  (!.elisp (xts/split-long nil 10))
  => (any [] {})

  (!.elisp (xts/split-long "" 10))
  => (any [] {})

  (!.elisp (xts/split-long (@! +s+) 10))
  => +out+)
...
(fact "gets the string description for a given tag"
  ...
  (!.elisp (xts/tag-string "user.account/login"))
  => "account login")
