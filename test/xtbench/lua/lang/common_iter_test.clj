(ns
 xtbench.lua.lang.common-iter-test
 (:use code.test)
...
(fact "creates a null iterator" (!.lua (it/arr< (it/iter-null))) => {})
...
(fact
 ...
 =>
 [0 1 2]
 (set (!.lua (it/arr< (it/iter {:a 1, :b 2}))))
 =>
 #{["b" 2] ["a" 1]}
 (!.lua (it/arr< (it/iter nil)))
 =>
 {})
...
(fact
 ...
 (!.lua (it/arr< (it/take 4 (it/constantly 1))))
 =>
 [1 1 1 1])
...
(fact
 ...
 (!.lua (it/arr< (it/take 4 (it/iterate k/inc 1))))
 =>
 [1 2 3 4])
...
(fact
 "cycles a sequence"
 (!.lua (it/arr< (it/take 5 (it/cycle [1 2 3]))))
 =>
 [1 2 3 1 2])
...
(fact
 ...
 (!.lua (it/arr< (it/drop 3 (it/range 10))))
 =>
 [3 4 5 6 7 8 9])
...
(fact
 ...
 (!.lua (it/arr< (it/take 4 (it/range [10 60 5]))))
 =>
 [10 15 20 25])
...
(fact
 ...
 (!.lua (it/arr< (it/concat [(it/range 3) (it/range [4 6])])))
 =>
 [0 1 2 4 5])
...
(fact
 ...
 (!.lua (it/arr< (it/filter k/odd? [1 2 3 4])))
 =>
 [1 3])
...
(fact
 ...
 (!.lua (it/arr< (it/partition 3 (it/range 10))))
 =>
 [[0 1 2] [4 5 6] [8 9]])
