(ns
 xtbench.lua.lang.util-xml-test
 (:use code.test)
 (:require [std.lang :as l] [std.string.prose :as prose]))

(l/script-
 :lua
 {:runtime :basic, :require [[xt.lang.util-xml :as xml]]})
...
(fact
 ...
 =>
 [{"tag" "a"} {"tag" "b", "empty" true} {"tag" "a", "close" true}]
...
(fact
 ...
 ^{:hidden true}
 (xml/to-node [{"tag" "a", "empty" true}])
 =>
 {"tag" "a"}
 (xml/to-node [{"tag" "a"} {"tag" "a", "close" true}])
 =>
 {"tag" "a"}
 (xml/to-node [{"tag" "a"} {"tag" "a", "text" "1", "close" true}])
 =>
 {"children" ["1"], "tag" "a"}
 (xml/to-node
  [{"tag" "a"}
...
(fact
 "to node to tree"
 ^{:hidden true}
 (!.lua (xml/to-tree (xml/parse-xml "<a/>")))
 =>
 ["a"]
 (!.lua (xml/to-tree (xml/parse-xml "<a>1</a>")))
 =>
 ["a" "1"]
 (!.lua (xml/to-tree (xml/parse-xml "<a><b></b></a>")))
 =>
 ["a" ["b"]]
 (!.lua (xml/to-tree (xml/parse-xml "<a>1<b>2</b>3</a>")))
 =>
 ["a" "1" ["b" "2"] "3"]
 (!.lua
  (xml/to-tree
...
(fact
 "to node params"
 ^{:hidden true}
 (!.lua (xml/to-string-params (tab [:a 1])))
 =>
 " a=1")
...
(fact
 ...
 ^{:hidden true}
 (!.lua
  (xml/to-string {"tag" "helo:test", "params" {}, "children" []}))
 =>
 "<helo:test></helo:test>"
 (!.lua
  (xml/to-string
 ...
 (!.lua (xml/to-string (xml/from-tree ["B"])))
 =>
 "<B></B>"
 (!.lua
  (xml/to-string
