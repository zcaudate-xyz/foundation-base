(ns
 xtbench.lua.db.base-schema-test
...
(fact:global {:setup [(l/rt:restart) (l/rt:scaffold :lua)], :teardown [(l/rt:stop)]})

^{:refer xt.db.base-schema/list-tables, :added "4.0"}
(fact
 "list tables"
 ^{:hidden true}
 (def +tables+ (!.lua (sch/list-tables sample/Schema)))
 (!.lua (sch/list-tables sample/Schema))
 =>
 +tables+)

^{:refer xt.db.base-schema/get-cached-schema, :added "4.0"}
(fact "get lookup" ^{:hidden true} (!.lua (sch/get-cached-schema sample/Schema)) => map?)

^{:refer xt.db.base-schema/create-data-keys, :added "4.0"}
(fact
 "creates data keys"
 ^{:hidden true}
 (!.lua (sch/create-data-keys sample/Schema "Currency"))
 =>
...
(fact
 "creates rev keys"
 ^{:hidden true}
 (set (!.lua (sch/create-rev-keys sample/Schema "UserAccount")))
 =>
...
(fact
 "creates all keys"
 ^{:hidden true}
 (!.lua (sch/create-all-keys sample/Schema "Wallet"))
 =>
 +all-wallet+)

^{:refer xt.db.base-schema/get-all-keys,
  :added "4.0",
  :setup
  [(def
    +all-org+
...
(fact
 ...
 =>
 {"account_id" "account", "state_id" "state", "country_id" "country"})
...
^{:refer xt.db.base-schema/create-table-order,
  :added "4.0",
  :setup
  [(def
    +ordered+
...
(fact
 ...
 ^{:hidden true}
 (!.lua (sch/create-table-order sample/SchemaLookup))
 =>
 +ordered+)
...
(fact
 ...
 ^{:hidden true}
 (!.lua (sch/table-order sample/SchemaLookup))
 =>
 +ordered+)
