...
(fact:global
 {:setup [(l/rt:restart) (do (l/rt:scaffold :lua) true)],
  :teardown [(l/rt:stop)]})
...
(fact
 ...
 ^{:hidden true}
 (!.lua (v/tree-select sample/Schema (@! +select+) {} {}))
 =>
 (l/as-lua +tree+))
...
(fact
 ...
 ^{:hidden true}
 (!.lua (v/tree-control-array {:limit 20, :order-by ["name"]}))
 =>
 +out+)
...
(fact
 ...
 ^{:hidden true}
 (!.lua (v/tree-count sample/Schema (@! +count+) {} {}))
 =>
 (l/as-lua +tree+))
