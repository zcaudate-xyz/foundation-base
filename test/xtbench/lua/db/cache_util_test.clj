(ns
 xtbench.lua.db.cache-util-test
...
(fact
 ...
 ^{:hidden true}
 (!.lua (var rows {}) (data/merge-bulk rows (@! +full-core+) nil))
 =>
 map?
 (!.lua (var rows {}) (data/merge-bulk rows (@! +full-contact+) nil))
 =>
 map?
 (!.lua
  (var rows {})
 ...
 =>
 map?
 (!.lua (var rows {}) (data/merge-bulk rows (@! +full-wallets+) nil))
 =>
 map?
 (!.lua
  (var rows {})
...
(fact
 ...
 ^{:hidden true}
 (!.lua (var rows {}) (data/merge-bulk rows (@! +flattened-full+) nil))
 =>
 map?
 (!.lua
  (var rows {})
