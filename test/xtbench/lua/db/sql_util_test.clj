...
(fact
 ...
 ^{:hidden true}
 (!.lua (ut/sqlite-json-values "'[1,2,3,4]'"))
 =>
...
(fact
 ...
 ^{:hidden true}
 (!.lua [(ut/encode-bool true) (ut/encode-bool false)])
 =>
 ["TRUE" "FALSE"])
...
(fact
 ...
 ^{:hidden true}
 (!.lua [(ut/encode-number 100000000000)])
 =>
 ["'100000000000'"])
 ...
 (!.lua
  [(ut/encode-query-string {} "WHERE" {})
   (ut/encode-query-string
    ...
    "WHERE"
    {:column-fn (fn:> [col] (xt/x:cat "\"SCHEMA\"." col))})
   (ut/encode-query-string {:data {:a 1}} "WHERE" {})])
 =>
 ["" "WHERE \"SCHEMA\".name = 'hello'" "WHERE data = '{\"a\":1}'"])
...
(fact
 ...
 ^{:hidden true}
 (!.lua (ut/default-table-fn "hello" {:hello {:schema "test.schema"}}))
 =>
 "\"test.schema\".\"hello\"")
...
(fact
 ...
 ^{:hidden true}
 (!.lua (ut/postgres-wrapper-fn "SELECT * FROM <TABLE>" 2))
 =>
...
(fact
 ...
 ^{:hidden true}
 (!.lua [(ut/sqlite-to-boolean 0) (ut/sqlite-to-boolean 1)])
 =>
 [false true])
